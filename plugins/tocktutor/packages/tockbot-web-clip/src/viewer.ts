...
export const defaultReaderPreferences: Readonly<ReaderPreferences> = {
  appearance: 'system',
  spacing: 'md',
  textSize: 'md',
  width: 'md',
...
export function normalizeViewerPageUrl(value: string): string {
...
function tab(id: number): ViewerTab {
...
export function addViewerTab(state: ViewerState): ViewerState {
...
export function selectViewerTab(state: ViewerState, id: string): ViewerState {
  return state.tabs.some(item => item.id === id) ? { ...state, activeId: id } : state
}

export function navigateViewerTab(
  state: ViewerState,
  id: string,
  page: ValidatedViewerPage,
): ViewerState {
...
export function moveViewerTab(state: ViewerState, id: string, rawIndex: number): ViewerState {
...
export function closeViewerTab(state: ViewerState, id: string): ViewerState {
...
export function addViewerBookmark(state: ViewerState): ViewerState {
...
export function removeViewerBookmark(state: ViewerState, id: string): ViewerState {
...
function readerPreferences(value: unknown): ReaderPreferences {
...
export function restoreViewerState(raw: string | null): ViewerState {
  ...
  const bookmarks: ViewerBookmark[] = []
  ...
  const activeIndex = Number.isSafeInteger(input.activeIndex)
    ? Math.max(0, Math.min(tabs.length - 1, input.activeIndex as number))
    : 0
  return {
