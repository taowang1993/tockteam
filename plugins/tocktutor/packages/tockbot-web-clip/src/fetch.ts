...
async function fetchPublicResource<T>(
  ...
  accept: string,
  read: (response: Response, limits: PublicFetchLimits, signal: AbortSignal, url: string) => Promise<T>,
): Promise<T> {
...
export async function fetchPublicImage(value: string, options: FetchPublicTextOptions = {}): Promise<PublicImageResult> {
  return await fetchPublicResource(value, options, 'image/avif,image/webp,image/png,image/jpeg,image/gif', async (response, limits, signal, url) => {
    ...
    if (length !== null && (!/^\d+$/u.test(length) || Number(length) > limits.maxResponseBytes)) return fail('body', 'The image is too large.')
    const reader = response.body?.getReader()
    if (!reader) return fail('body', 'The image is empty.')
    ...
    try {
      while (true) {
        ...
        if (total > limits.maxResponseBytes) return fail('body', 'The image is too large.')
        chunks.push(value)
      }
    } catch (error) {
      await reader.cancel().catch(() => undefined)
    ...
    if (!signature) return fail('content-type', 'The image bytes do not match their content type.')
