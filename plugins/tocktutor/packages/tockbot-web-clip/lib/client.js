...
// src/reader.ts
var maximumReaderViewLimits = {
  maxParserInputChars: 2e6,
  maxParserTokens: 1e5,
  maxReaderOutputChars: 5e5,
...
function parseReaderViewResult(value) {
  ...
  const input = value;
  if (typeof input.content !== "string" || input.content.length > maximumReaderViewLimits.maxReaderOutputChars || typeof input.sourceUrl !== "string" || typeof input.title !== "string" || input.title.length > maximumReaderViewLimits.maxReaderTitleChars || !Array.isArray(input.warnings) || input.warnings.length > maximumReaderViewLimits.maxReaderWarnings || input.warnings.some((warning) => typeof warning !== "string" || warning.length > maximumReaderViewLimits.maxReaderWarningChars)) {
...
function parseViewerPageResult(value) {
  ...
  const input = value;
  if (typeof input.contentType !== "string" || !acceptedContentTypes.has(input.contentType) || typeof input.html !== "string" || input.html.length > MAX_VIEWER_HTML_CHARS || typeof input.title !== "string" || input.title.length > maximumReaderViewLimits.maxReaderTitleChars || typeof input.url !== "string") throw new Error("The Host returned an invalid viewer page.");
...
function WebViewer(props = {}) {
  ...
  const navigationIntent = (0, import_react3.useRef)(0);
  const pendingNavigation = (0, import_react3.useRef)(null);
  ...
  const [clipDestination, setClipDestination] = (0, import_react3.useState)(() => defaultClipDestination(props.webClipFolder));
  ...
  const loadBlankFrame = (0, import_react3.useCallback)((intent) => {
  ...
  const startNavigation = (0, import_react3.useCallback)((url, tabId, intent) => {
    if (!bridge) return;
    const element = webview.current;
  ...
  const clearFrame = (0, import_react3.useCallback)(() => {
    const intent = ++navigationIntent.current;
  ...
  const activate = (tab2) => {
    if (clipApplyingRef.current) return;
    const next = selectViewerTab(viewerRef.current, tab2.id);
    applyViewer(next);
    setDraft(tab2.url ?? "");
    invalidateReader();
