...
const positiveInteger = (value, max) => (Schema.number().step(1).min(1).max(max).default(value));
...
export class WebClipHost extends Service {
    ...
    active = new Set();
    activeFetches = 0;
    clipReviews = new ClipReviewStore();
    closing = false;
    fetchLimits;
    ...
    constructor(ctx, config) {
        ...
        ctx.inject(['noteVault'], runtimeCtx => {
            ...
            this.runtime = runtimeCtx.get('noteVault');
            this.clipReviews.dispose();
        ...
        ctx.inject(['webServer', 'tockTeamSurface'], browserCtx => {
            ...
            const surface = browserCtx.get('tockTeamSurface');
            if (!webServer || surface?.kind !== 'desktop')
                return;
            browserCtx.effect(() => {
                const removers = [];
                const removeAll = () => {
    ...
    assertOpen() {
        if (this.closing)
            throw new ClipRuntimeError('runtime-unavailable', 'The Web Clip Host is unloading');
    }
    async abortActive() {
    ...
    async trackOperation(signal, run) {
        this.assertOpen();
    ...
    async createClipReviewFromUrl(input, signal) {
