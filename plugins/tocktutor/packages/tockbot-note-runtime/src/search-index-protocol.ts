import { encodeIndexFrame, INDEX_FRAME_BYTES } from './search-index-channel.ts'
...
export type IndexReply = { items: unknown[]; meta: unknown }
type Pending = ReturnType<typeof Promise.withResolvers<IndexReply>> & { items: unknown[]; page: number; bytes: number; limit: number; maxBytes: number }
type Incoming = { id: number; action: IndexAction; args: unknown; controller: AbortController }
...
export function text(value: unknown, max = 32768): value is string {
...
export function documentRecord(value: unknown): value is { path: string; revision: string; modifiedAt: number } {
...
export class IndexProtocol {
  private readonly peer: IndexPeer
  private sequence = 0
  ...
  private ack: { id: number; page: number; gate: ReturnType<typeof Promise.withResolvers<void>> } | undefined
  private readonly handle: (action: IndexAction, args: unknown, signal: AbortSignal) => Promise<IndexReply>
  private readonly notice: (action: string, value: unknown) => void
  ...
  constructor(peer: IndexPeer, handle: IndexProtocol['handle'], notice: IndexProtocol['notice'], failed: IndexProtocol['failed']) {
    this.peer = peer; this.handle = handle; this.notice = notice; this.failed = failed
  ...
  private async send(body: Record<string, unknown>): Promise<void> {
    ...
    if (this.queuedWrites >= 10) throw new Error('Index protocol write queue exceeded')
    if (!Number.isSafeInteger(this.sequence + 1)) throw new Error('Index sequence exhausted')
    ...
    const snapshot: unknown = JSON.parse(bytes.subarray(4).toString('utf8'))
    this.queuedWrites += 1
    ...
    try { await write } finally { this.queuedWrites -= 1 }
  }
  ...
  request(action: IndexAction, args: unknown, limit: number, maxBytes: number, signal?: AbortSignal): Promise<IndexReply> {
    ...
    if (this.pending.size >= 4 || !Number.isSafeInteger(this.nextId + 1)) return Promise.reject(new Error('Index request limit exceeded'))
    const id = ++this.nextId
    const pending: Pending = { ...Promise.withResolvers<IndexReply>(), items: [], page: 0, bytes: 0, limit, maxBytes }
    this.pending.set(id, pending)
    void this.send({ type: 'request', id, action, args }).catch(error => this.close(error))
    const abort = (): void => {
      if (action === 'search' && this.pending.has(id)) void this.send({ type: 'cancel', id }).catch(error => this.close(error))
    }
  ...
  close(error = new Error('Index protocol is closed')): void {
    if (this.failure) return
    this.failure = error instanceof Error ? error : new Error('Index protocol failed')
    this.peer.channel.close()
    for (const pending of this.pending.values()) pending.reject(this.failure)
    this.pending.clear()
    for (const incoming of this.incoming.values()) incoming.controller.abort()
    this.incoming.clear()
    this.queue.length = 0
  ...
  private async receive(): Promise<void> {
    for await (const value of this.peer.channel.frames) {
      if (this.failure) return
      if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid index envelope')
      const message = value as Record<string, unknown>
      if (message.generation !== this.peer.generation || !integer(message.sequence, 1) || message.sequence !== ++this.receivedSequence) throw new Error('Invalid index generation or sequence')
      ...
      if (message.type === 'request' && exact(message, [...keys, 'id', 'action', 'args'])) {
        if (!integer(message.id, 1) || message.id !== this.incomingId + 1 || this.incoming.size >= 4
          || !['init', 'inventory', 'document', 'search'].includes(message.action as string)) throw new Error('Invalid index request')
        this.incomingId += 1
        const incoming = { id: this.incomingId, action: message.action as IndexAction, args: message.args, controller: new AbortController() }
        this.incoming.set(incoming.id, incoming); this.queue.push(incoming)
        void this.serve().catch(error => this.close(error))
      } else if (message.type === 'cancel' && exact(message, [...keys, 'id'])) {
        if (!integer(message.id, 1, this.incomingId)) throw new Error('Invalid index cancellation')
        const incoming = this.incoming.get(message.id)
        if (incoming && incoming.action !== 'search') throw new Error('Invalid index cancellation action')
        ...
        if (!['invalidate', 'ready', 'step', 'failure'].includes(message.action as string)) throw new Error('Invalid index notice')
        this.notice(message.action as string, message.value)
      } else if (message.type === 'ack' && exact(message, [...keys, 'id', 'page'])) {
        if (!this.ack || message.id !== this.ack.id || message.page !== this.ack.page) throw new Error('Invalid index acknowledgement')
        ...
        if (!pending || message.page !== pending.page++ || !Array.isArray(message.items) || message.items.length > 256
          || typeof message.done !== 'boolean' || (!message.done && (message.items.length === 0 || message.meta !== null))) throw new Error('Invalid index data page')
        pending.bytes += Buffer.byteLength(JSON.stringify(message.items))
        if (pending.items.length + message.items.length > pending.limit || pending.bytes > pending.maxBytes) throw new Error('Index response limit exceeded')
      ...
      } else throw new Error('Invalid index action')
    }
      ...
      while (this.queue.length && !this.failure) {
        ...
        do {
          ...
          while (offset < result.items.length && items.length < 256) {
            ...
            if (encoded === undefined) throw new Error('Invalid index record')
            const size = Buffer.byteLength(encoded) + (items.length ? 1 : 0)
            if (bytes + size > INDEX_FRAME_BYTES) {
              if (!items.length) throw new Error('Index record exceeds frame limit')
              break
            }
            bytes += size
