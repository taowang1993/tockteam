...
import { INDEX_FRAME_BYTES, IndexChannel } from './search-index-channel.ts'

const VERSION = 1
const AUTH_DEADLINE_MS = 15_000
const PEER_DEADLINE_MS = 2_000
const MAX_ATTEMPTS = 8
export type IndexPeer = Readonly<{ channel: IndexChannel; generation: string }>

type Bootstrap = { port: number; token: string; generation: string }
function fields(value: unknown, keys: string[]): value is Record<string, unknown> {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
    && Object.keys(value).length === keys.length && keys.every(key => Object.hasOwn(value, key))
}
const tokenValid = (value: unknown): value is string => typeof value === 'string' && /^[a-f0-9]{64}$/u.test(value)
const generationValid = (value: unknown): value is string => typeof value === 'string' && /^[a-f0-9]{32}$/u.test(value)

function parseBootstrap(json: string): Bootstrap {
  let value: unknown
  try {
    if (typeof json !== 'string' || Buffer.byteLength(json) > 1024) throw new Error()
    value = JSON.parse(json)
  } catch { throw new Error('Invalid index bootstrap') }
  if (!fields(value, ['port', 'token', 'generation']) || !Number.isSafeInteger(value.port)
    || (value.port as number) < 1 || (value.port as number) > 65535
    || !tokenValid(value.token) || !generationValid(value.generation)) throw new Error('Invalid index bootstrap')
  return value as Bootstrap
}

/** A fresh, private bootstrap goes only into the owned child's explicit environment. */
export async function listenForIndexPeer(signal: AbortSignal): Promise<Readonly<{
  bootstrap: string
  ...
  const peer = Promise.withResolvers<IndexPeer>()
  ...
  const stopListening = (): Promise<void> => {
    closing ??= new Promise<void>((resolve, reject) => server.close(error => {
  ...
  const close = (error = new Error('Index listener is closed')): Promise<void> => {
    closed = true
  ...
  const abort = (): void => { void close(new Error('Index authentication cancelled')) }
  ...
  server.on('connection', socket => {
    ...
    attempts += 1
    if (closed || authenticated || attempts > MAX_ATTEMPTS) { channel.close(); return }
    ...
    void (async () => {
      try {
        ...
        if (first.done || !fields(hello, ['type', 'version', 'generation', 'token'])
          ...
          || closed || authenticated) throw new Error('Invalid index authentication')
        authenticated = true
        admitted = true
        clearTimeout(deadline)
        void stopListening()
        ...
        if (closed) throw new Error('Index listener is closed')
        peer.resolve({ channel, generation })
      } catch {
        channel.close()
        if (admitted) await close(new Error('Index authentication failed'))
        else if (!authenticated && attempts >= MAX_ATTEMPTS) await close(new Error('Index authentication attempt limit exceeded'))
  ...
  try {
    ...
    if (address === null || typeof address === 'string') throw new Error('Index listener has no loopback address')
...
export async function connectIndexPeer(bootstrap: string, signal: AbortSignal): Promise<IndexPeer> {
  ...
  try {
    ...
    if (first.done || !fields(accepted, ['type', 'version', 'generation']) || accepted.type !== 'accepted'
      || accepted.version !== VERSION || accepted.generation !== generation) throw new Error('Invalid index authentication acknowledgement')
