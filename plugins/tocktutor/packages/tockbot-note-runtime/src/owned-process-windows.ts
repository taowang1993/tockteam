...
import { OWNED_PROCESS_EXIT_DEADLINE_MS, type OwnedProcess, type OwnedProcessOptions, type OwnedProcessResult } from './owned-process.ts'

// Structural subset of the pinned public API. The optional Windows dependency
// need not be installed on POSIX for the owning package to typecheck or build.
...
export type WindowsOwnedBindings = BaseBindings & Extensions & { address(buffer: Buffer): NativePtr }

export async function loadWindowsOwnedBindings(): Promise<WindowsOwnedBindings> {
  if (process.platform !== 'win32' || !['x64', 'arm64'].includes(process.arch)) throw new Error('Windows owned processes require a supported 64-bit Windows host')
  const entry = createRequire(import.meta.url).resolve('@deepseek-ai/dsh-win32-process')
  const publicApi = await import(pathToFileURL(entry).href) as {
    extendWin32ProcessBindings(create: (context: { kernel32: unknown; bind(library: unknown, name: string, result: string, args: string[]): unknown }) => Extensions): BaseBindings & Extensions
  }
  const koffi = createRequire(entry)('koffi') as { address(buffer: Buffer): NativePtr }
...
export function encodeWindowsInvocation(options: Pick<OwnedProcessOptions, 'executable' | 'args' | 'env' | 'windowsVerbatimArguments'>): { commandLine: Buffer; environment: Buffer } {
  if ([options.executable, ...options.args].some(value => value.includes('\0'))) throw new Error('Invalid Windows process argument')
  const keys = new Set<string>()
  const entries = Object.entries(options.env).filter((entry): entry is [string, string] => entry[1] !== undefined)
  for (const [key, value] of entries) {
    const normalized = key.toUpperCase()
    if (!key || /[=\0]/u.test(key) || value.includes('\0') || keys.has(normalized)) throw new Error('Invalid Windows process environment')
    keys.add(normalized)
  }
  ...
  if (commandLine.length > 32767 || environment.length > 32767) throw new Error('Windows process invocation limit exceeded')
...
export async function spawnWindowsOwnedProcess(options: OwnedProcessOptions, bindings?: WindowsOwnedBindings): Promise<OwnedProcess> {
  if (options.admit) {
  ...
  if (!Number.isSafeInteger(maxOutputBytes) || maxOutputBytes < 1 || maxOutputBytes > 1024 * 1024) throw new Error('Invalid owned process output limit')
  const encoded = encodeWindowsInvocation(options)
  const api = bindings ?? await loadWindowsOwnedBindings()
  if (options.signal.aborted) throw new Error('Owned process cancelled')
  const owned = new Set<NativePtr>()
  const held: Buffer[] = [encoded.commandLine, encoded.environment]
  const allocate = (size: number): Buffer => { const buffer = Buffer.alloc(size); held.push(buffer); return buffer }
  const error = (operation: string): Error => new Error(`${operation} failed (Win32 ${api.getLastError()})`)
  const check = (ok: number, operation: string): void => { if (!ok) throw error(operation) }
  const keep = (handle: NativePtr): NativePtr => { if (!handle) throw error('Acquire owned handle'); owned.add(handle); return handle }
  let releaseFailure: Error | undefined
  const close = (handle: NativePtr): void => {
    if (!owned.delete(handle)) return
    try { check(api.closeHandle(handle), 'CloseHandle') } catch (caught) {
      ...
      releaseFailure ??= caught as Error
      throw caught
    }
  }
  const closeAll = (): Error | undefined => {
    // The kill-on-close Job is acquired first and released last.
    for (const handle of [...owned].reverse()) { try { close(handle) } catch { /* retained above; keep attempting remaining handles */ } }
    return releaseFailure
  }
  const pipe = (): [NativePtr, NativePtr] => {
    const read = allocate(8), write = allocate(8)
    check(api.createPipe(api.address(read), api.address(write), null, 0), 'CreatePipe')
    const handles = [read.readBigUInt64LE() as NativePtr, write.readBigUInt64LE() as NativePtr] as const
    for (const handle of handles) if (handle) owned.add(handle)
    if (handles.some(handle => !handle)) throw new Error('CreatePipe returned a null handle')
    return [...handles]
  }
  let job: NativePtr | undefined
  let port: NativePtr | undefined
  let root: NativePtr | undefined
  let stdout: NativePtr | undefined
  let stderr: NativePtr | undefined
  let pid = 0
  let created = false
  let initializedAttributes: NativePtr | undefined
  ...
  try {
    ...
    stdout = output[0]; stderr = errors[0]
    const childHandles = [input[0], output[1], errors[1]]
    for (const handle of childHandles) check(api.setHandleInformation(handle, 1, 1), 'SetHandleInformation')
    const size = allocate(8)
    const sized = api.initializeProcThreadAttributeList(null, 2, 0, api.address(size))
    if (sized !== 0 || api.getLastError() !== 122) throw error('InitializeProcThreadAttributeList size query')
    const bytes = Number(size.readBigUInt64LE())
    if (!Number.isSafeInteger(bytes) || bytes < 1 || bytes > 1024 * 1024) throw new Error('Invalid process attribute size')
    ...
    const handleList = allocate(24); childHandles.forEach((handle, index) => handleList.writeBigUInt64LE(handle, index * 8))
    ...
    startup.writeUInt32LE(112, 0); startup.writeUInt32LE(0x100, 60)
    ...
    check(api.createProcessW(options.executable, api.address(encoded.commandLine), null, null, 1, 0x08080400,
      api.address(encoded.environment), options.cwd, api.address(startup), api.address(information)), 'CreateProcessW')
    created = true
    // Capture both handles before validating, so partial results cannot leak.
    const processHandle = information.readBigUInt64LE(0) as NativePtr
    const thread = information.readBigUInt64LE(8) as NativePtr
    if (processHandle) { root = processHandle; owned.add(root) }
    if (thread) owned.add(thread)
    pid = information.readUInt32LE(16)
    if (!root || !thread || !pid) throw new Error('CreateProcessW returned incomplete process information')
    for (const handle of [...input, output[1], errors[1], thread]) close(handle)
  } catch (caught) { launchError = caught as Error } finally {
    if (initializedAttributes) {
      try { api.deleteProcThreadAttributeList(initializedAttributes) } catch (caught) { launchError ??= caught as Error }
    }
  ...
  if (!created) {
    ...
    if (cleanupError) throw new Error('Owned process cleanup could not be verified', { cause: cleanupError })
    throw launchError ?? new Error('Owned process could not start')
  }

  const result = Promise.withResolvers<OwnedProcessResult>()
  const cleanup = Promise.withResolvers<void>()
  void result.promise.catch(() => {}); void cleanup.promise.catch(() => {})
  ...
  const processes = new Map<number, NativePtr>()
  let trackingFailure: Error | undefined
  const observeBirths = (): boolean => {
    try {
      ...
      for (let index = 0; index < 64; index++) {
        ...
        if (!api.getQueuedCompletionStatus(port!, api.address(message), api.address(key), api.address(value), 0)) {
          ...
          if (code === 258 && value.readBigUInt64LE() === 0n) return true
          throw new Error(`Job completion queue failed (Win32 ${code})`)
        }
        if (key.readBigUInt64LE() !== job) throw new Error('Unexpected Job completion key')
        if (message.readUInt32LE() !== 6) continue
        const born = Number(value.readBigUInt64LE())
        if (!Number.isSafeInteger(born) || born <= 0 || born > 0xffffffff || processes.has(born) || processes.size >= 1024) throw new Error('Owned process birth tracking limit or identity violation')
        const handle = born === pid ? root! : api.openProcess(0x100000, 0, born)
        processes.set(born, handle ?? 0n)
        if (!handle) throw error('Open owned process for settlement')
      ...
      return false
    } catch (caught) { trackingFailure ??= caught as Error; throw caught }
  }
  const finish = (cleanupError?: Error): void => {
    ...
    if (cleanupError) { const rejected = new Error('Owned process cleanup could not be verified', { cause: cleanupError }); cleanup.reject(rejected); result.reject(rejected) }
    else { cleanup.resolve(); if (failure) result.reject(failure); else result.resolve({ code, signal: null, stdoutBytes, stderrBytes }) }
  }
  const stop = (reason?: Error): void => {
    failure ??= reason
    if (finished || stoppingAt !== undefined) return
    stoppingAt = performance.now()
    try { check(api.terminateJobObject(job!, 1), 'TerminateJobObject') } catch (caught) { failure ??= caught as Error }
  }
  const cancel = (): void => stop(new Error('Owned process cancelled'))
  const read = (handle: NativePtr): number | null => {
    if (!api.peekNamedPipe(handle, null, 0, null, api.address(count), null)) {
      if (api.getLastError() === 109) return null
    ...
    if (bytes > available) throw new Error('Invalid owned pipe read count')
    return bytes
  }
  const poll = (): void => {
    if (finished) return
    try {
      ...
      if (total < 1 || total > 1024) { trackingFailure ??= new Error('Owned process birth tracking limit exceeded'); throw trackingFailure }
      ...
      if (!stdoutClosed) { const bytes = read(stdout!); stdoutClosed = bytes === null; stdoutBytes += bytes ?? 0 }
      if (!stderrClosed) { const bytes = read(stderr!); stderrClosed = bytes === null; stderrBytes += bytes ?? 0 }
      if (stdoutBytes + stderrBytes > maxOutputBytes) stop(new Error('Owned process output limit exceeded'))
      if (!rootExited) {
        if (!root) throw new Error('Owned root handle is unavailable')
        const waited = api.waitForSingleObject(root, 0)
        if (waited === 0) { check(api.getExitCodeProcess(root, api.address(count)), 'GetExitCodeProcess'); code = count.readUInt32LE(); rootExited = true; stop() }
        else if (waited !== 258) { trackingFailure ??= error('WaitForSingleObject'); throw trackingFailure }
      }
      if (stoppingAt !== undefined && rootExited && stdoutClosed && stderrClosed) {
        if (queueEmpty && total > 0 && total === processes.size && processes.has(pid) && accounting.readUInt32LE(40) === 0) {
          ...
          for (const handle of processes.values()) {
            if (!handle) continue // sticky trackingFailure prevents successful cleanup
            const waited = api.waitForSingleObject(handle, 0)
            if (waited === 258) signaled = false
            else if (waited !== 0) { trackingFailure ??= error('Wait for owned process settlement'); throw trackingFailure }
          }
          if (signaled) { finish(); return }
        }
      }
    } catch (caught) { stop(caught as Error) }
    if (stoppingAt !== undefined && performance.now() - stoppingAt >= OWNED_PROCESS_EXIT_DEADLINE_MS) finish(new Error('Owned Job did not settle'))
  }
  timer = setInterval(poll, 10)
  options.signal.addEventListener('abort', cancel, { once: true })
  if (options.signal.aborted) cancel()
  if (launchError) stop(launchError)
  poll()
  if (launchError) { await cleanup.promise; throw launchError }
