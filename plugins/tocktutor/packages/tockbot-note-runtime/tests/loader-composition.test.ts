...
import { SearchIndexProcess, type SearchIndexProcessOptions } from '../src/search-index-process.ts'
...
// Historical SQLite monkeypatches exercise the algorithm in this test Host, not isolation.
function useInProcessIndex(t: TestContext): void {
  t.mock.method(NoteVaultRuntime.prototype as unknown as { createSearchIndex(options: SearchIndexProcessOptions): unknown }, 'createSearchIndex', (options: SearchIndexProcessOptions) => new PersistentSearchIndex({
    directory: options.directory, identity: options.identity, vaultId: options.vaultId,
    list: signal => options.list(signal, () => {}),
    read: (path, signal) => options.read(path, signal, () => {}),
  }))
}
...
for (const stage of ['mount', 'commit'] as const) {
  test(`in-process search index disposal drains ${stage} before closing its unpublished native connection`, async t => {
    useInProcessIndex(t)
...
test('in-process search index remount waits for its prior native owner before reusing a vault', async t => {
  useInProcessIndex(t)
  const { Document } = createRequire(import.meta.url)('flexsearch') as typeof import('flexsearch')
...
for (const failure of ['open', 'lock', 'insert'] as const) {
  test(`in-process search index native ${failure} failure falls back, disposes, and rebuilds on reopen`, async t => {
    useInProcessIndex(t)
    const { Document } = createRequire(import.meta.url)('flexsearch') as typeof import('flexsearch')
...
test('in-process search index retained epoch does not imply publication during a full retry', async t => {
  useInProcessIndex(t)
...
test('Keyword search reconciles state-owned indexed candidates through the exact verifier', async t => {
  ...
  await writeFile(join(vaultRoot, 'FalsePositive.md'), '# False\nproject is plain text\n')
  await writeFile(join(vaultRoot, 'Other.md'), '# Other\nunrelated\n')
  const legacy = join(stateRoot, 'search-index', 'tocktutor-search-v2')
  await mkdir(legacy, { recursive: true })
  const sentinel = join(legacy, 'legacy.sqlite')
  await writeFile(sentinel, 'legacy cache must remain untouched')
  ...
  try {
    ...
    const actor = Reflect.get(loaded.context.noteVault, 'searchIndex').index
    assert.ok(actor instanceof SearchIndexProcess)
    childPid = actor.pid
    assert.ok(childPid && childPid !== process.pid)
    ...
    assert.deepEqual((await indexedSearch(1)).matches, [])
    assert.equal(await readFile(sentinel, 'utf8'), 'legacy cache must remain untouched')
  } finally {
    await dispose(loaded.context, loaded.root)
    try {
      if (childPid) {
        assert.throws(() => process.kill(childPid!, 0), { code: 'ESRCH' })
        if (process.platform !== 'win32') assert.throws(() => process.kill(-childPid!, 0), { code: 'ESRCH' })
      }
    } finally { await rm(fixture, { recursive: true, force: true, maxRetries: 10, retryDelay: 100 }) }
  }
})
...
test('persistent FlexSearch SQLite indexes reopen outside the user vault', async () => {
  ...
  try {
    ...
    loaded = await load(config)
    await verifyIndexedSearch()
    const databaseName = (await readdir(join(stateRoot, 'search-index', 'tocktutor-search-v3'), { recursive: true })).find(name => name.endsWith('.sqlite'))
    if (databaseName === undefined) assert.fail('persistent search database must be present')
    const databasePath = join(stateRoot, 'search-index', 'tocktutor-search-v3', databaseName)
    const second = loaded
    loaded = null
    await dispose(second.context, second.root)
    ...
    await verifyIndexedSearch()
    assert.equal(await readFile(sentinel, 'utf8'), 'legacy cache must remain untouched')
    assert.deepEqual(await readdir(legacy), ['legacy.sqlite'])
    assert.equal((await lstat(join(stateRoot, 'search-index'))).isDirectory(), true)
    await assert.rejects(lstat(join(vaultRoot, 'search-index')), { code: 'ENOENT' })
  } finally {
