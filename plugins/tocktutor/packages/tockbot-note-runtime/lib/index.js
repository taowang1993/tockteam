...
async function assertNoDirectorySymlinks(root, candidate) {
...
async function resolveDocumentTarget(root, requestedPath) {
...
async function readBounded(handle, limit, expectedSize, signal) {
    const chunks = [];
    let offset = 0;
...
async function inspectTreeFile(root, candidate, relativePath) {
...
async function scanVaultTree(root, config, signal) {
    const entries = [];
    const warnings = [];
    let scanned = 0;
