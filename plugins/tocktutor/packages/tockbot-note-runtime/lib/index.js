...
import { closeSync, constants as fsConstants, fstatSync, fsyncSync, lstatSync, mkdirSync, openSync, readSync, realpathSync, renameSync, unlinkSync, writeSync, } from 'node:fs';
...
import { SearchIndexProcess, ISOLATED_SEARCH_SCHEMA } from "./search-index-process.js";
import { adoptSearchIndex, retireSearchIndex, awaitSearchIndexSettlement, searchIndexOwnershipFailure } from "./search-index-ownership.js";
...
export const TOCKTEAM_DESKTOP_VAULT_SELECTION_SERVICE = 'tockTeamDesktopVaultSelection';
export class TockTeamDesktopVaultSelection extends Service {
    constructor(ctx) {
        super(ctx, TOCKTEAM_DESKTOP_VAULT_SELECTION_SERVICE);
    }
    async adopt(input, _signal) {
...
function passiveBackupRoot(name) {
...
export function isPassiveBackupPath(relativePath) {
...
function normalizePassiveBackupPath(requestedPath) {
...
function passiveBackupAliasKey(relativePath) {
    return relativePath.normalize('NFKC').toLocaleLowerCase('en-US');
}
function preMoveReferrerPath(postMovePath, oldPath, newPath, isDirectory) {
    if (isDirectory) {
        if (postMovePath === newPath)
            return oldPath;
...
function normalizeEntryPath(requestedPath) {
...
function entryRevision(alias, aliasEntry, targetEntry) {
    if (!alias)
        return fileRevision(targetEntry);
    return `entry:${createHash('sha256')
        .update(`${fileRevision(aliasEntry)}:${fileRevision(targetEntry)}`)
        .digest('hex')}`;
}
async function assertNoDirectorySymlinks(root, candidate, progress) {
...
async function bindDestinationParent(root, candidate) {
...
async function assertClaimedEntryConfined(root, candidate, claimed) {
    await assertNoDirectorySymlinks(root, candidate);
...
async function assertDesktopVaultSelectionTargetBound(target) {
    try {
...
async function resolveRevealTarget(root, requestedPath) {
    const relativePath = normalizeEntryPath(requestedPath);
    if (Buffer.byteLength(relativePath, 'utf8') > MAX_REVEAL_PATH_BYTES) {
...
async function assertRevealTargetBound(root, target) {
    try {
        ...
        if (error instanceof NoteVaultError && error.code === 'changed')
            throw error;
        throw new NoteVaultError('changed', 'Vault reveal target changed during the operation');
    }
}
...
async function resolveDocumentTarget(root, requestedPath, progress) {
    const relativePath = normalizeDocumentPath(requestedPath);
    const candidate = path.join(root, ...relativePath.split('/'));
    assertInside(root, candidate);
    await assertNoDirectorySymlinks(root, candidate, progress);
    ...
    if (alias && documentKind(relativePath) !== documentKind(canonical)) {
        throw new NoteVaultError('unsupported-type', 'Vault document aliases must target the same supported file type');
    }
    return { alias, aliasEntry, candidate, canonical, relativePath, targetEntry };
}
async function readBounded(handle, limit, expectedSize, signal, progress) {
    const chunks = [];
    let offset = 0;
...
async function readVaultDocument(root, requestedPath, maxBytes, signal, bindAliasEntry = false, progress) {
    signal.throwIfAborted();
    const target = await resolveDocumentTarget(root, requestedPath, progress);
    signal.throwIfAborted();
    ...
    try {
        ...
        if (opened.size > BigInt(maxBytes)) {
            throw new NoteVaultError('too-large', `Vault document exceeds the configured ${String(maxBytes)}-byte limit`);
        }
        const data = await readBounded(handle, maxBytes, Number(opened.size), signal, progress);
        if (data === null) {
            throw new NoteVaultError('too-large', `Vault document exceeds the configured ${String(maxBytes)}-byte limit`);
        }
        ...
        await assertNoDirectorySymlinks(root, target.candidate, progress);
        ...
        await handle.close().then(() => progress?.(), () => undefined);
    }
}
async function resolveAttachmentTarget(root, requestedPath) {
    const relativePath = normalizeEntryPath(requestedPath);
...
async function inspectVaultAttachment(root, requestedPath, maxBytes, signal, readData) {
    signal.throwIfAborted();
...
function encodeDocumentContent(content, maxBytes) {
    if (typeof content !== 'string') {
        throw new NoteVaultError('invalid-content', 'Document content must be a string');
    }
    const data = Buffer.from(content, 'utf8');
    if (data.byteLength > maxBytes) {
        throw new NoteVaultError('too-large', `Document content exceeds the configured ${String(maxBytes)}-byte limit`);
    }
    return data;
}
async function resolveNewDocumentTarget(root, requestedPath) {
    const relativePath = normalizeDocumentPath(requestedPath);
    const candidate = path.join(root, ...relativePath.split('/'));
    assertInside(root, candidate);
...
async function assertWriteTargetUnchanged(root, target, expectedRevision) {
    try {
        await assertNoDirectorySymlinks(root, target.candidate);
        if (await realpath(target.candidate) !== target.canonical)
            throw new Error();
        const currentTarget = await lstat(target.canonical, { bigint: true });
...
function temporaryPathFor(targetPath) {
    ...
    return path.join(path.dirname(targetPath), `.${basename}.${String(process.pid)}.${Date.now().toString()}.${randomBytes(6).toString('hex')}.tmp`);
}
async function bestEffortFsync(directory) {
    try {
...
async function writeDocumentAtomic(targetPath, data, exclusive, beforeCommit) {
...
async function resolveFileDestination(root, requestedPath, source) {
    const relativePath = normalizeDocumentPath(requestedPath);
...
async function assertFileEntryUnchanged(root, target, expectedRevision, afterHardLink = false) {
...
async function rollbackCreatedEntry(pathToRemove, created) {
    try {
        const current = await lstat(pathToRemove, { bigint: true });
        if (!sameStableFile(created, current))
            return false;
...
async function createMovedFileEntry(source, destinationPath) {
    if (source.alias) {
...
async function createDuplicatedFileEntry(source, destinationPath) {
    if (source.alias) {
        const relativeTarget = path.relative(path.dirname(destinationPath), source.canonical);
        await symlink(relativeTarget, destinationPath, 'file');
    }
...
function supportedFileKind(filePath) {
...
function folderTargetKey(vaultRoot, subtreeRoot, targetPath) {
...
async function hashStableManifestFile(vaultRoot, candidate, maxBytes, signal) {
    signal.throwIfAborted();
...
async function buildFolderManifest(vaultRoot, subtreeRoot, config, signal) {
    ...
    const visit = async (directory, relativeDirectory, depth) => {
        ...
        for (const dirent of dirents) {
            ...
            const inspected = await hashStableManifestFile(vaultRoot, candidate, config.maxBytes - bytes, signal);
...
function folderManifestFingerprint(manifest) {
    return createHash('sha256').update(JSON.stringify(manifest)).digest('hex');
}
function folderContentFingerprint(manifest) {
...
async function syncCopiedFile(filePath) {
...
async function copyFolderManifest(manifest, vaultRoot, sourceRoot, destinationRoot, signal, assertCurrent, onRootCreated) {
...
function snapshotDirectoryParts(vault, relativePath) {
    return [
        'snapshots',
...
async function stateDirectory(stateRoot, parts, create) {
...
async function draftFilePath(stateRoot, vault, relativePath, create) {
    const directory = await stateDirectory(stateRoot, ['drafts', createHash('sha256').update(vault.id).digest('hex')], create);
    return directory === null
        ? null
        : path.join(directory, `${createHash('sha256').update(relativePath).digest('hex')}.json`);
}
async function readDraftRecord(stateRoot, vault, relativePath, maxBytes) {
    const filePath = await draftFilePath(stateRoot, vault, relativePath, false);
    if (filePath === null)
        return null;
    try {
        const parsed = JSON.parse((await readStateBytes(filePath, maxBytes)).toString('utf8'));
        const now = Date.now();
        if (parsed.path !== relativePath
...
async function inspectTreeFile(root, candidate, relativePath, progress) {
    ...
    if (alias && (requestedDocument !== documentKind(canonical)
...
async function scanVaultTree(root, config, signal, progress) {
    const entries = [];
    const warnings = [];
    let scanned = 0;
    ...
    const visit = async (directory, depth) => {
        ...
        // The iterator owns closure on success, break, abort, and callback failure.
        for await (const entry of stream) {
            progress?.();
            signal.throwIfAborted();
        ...
        progress?.(); // Successful end-of-directory also advances an empty traversal.
        dirents.sort((left, right) => compareVaultPaths(left.name, right.name));
        for (const dirent of dirents) {
            signal.throwIfAborted();
            ...
            if (dirent.isSymbolicLink()) {
                ...
                try {
                    const entry = await inspectTreeFile(root, candidate, relativePath, progress);
                    if (entry !== null)
                        entries.push(entry);
                }
                catch (error) {
...
async function scanPassiveBackupEntries(root, config, signal) {
    const output = [];
    ...
    for await (const dirent of stream) {
        if (passiveBackupRoot(dirent.name))
            roots.push(dirent.name);
    }
...
async function readPassiveBackupFile(root, request, signal) {
    signal.throwIfAborted();
    if (NOFOLLOW === 0) {
...
async function assertPassiveDestinationUnaliased(root, relativePath, maxEntries) {
    const parts = relativePath.split('/');
    let cursor = root;
...
async function ensurePassiveBackupParent(root, relativePath, maxEntries) {
    const parts = relativePath.split('/');
    let cursor = root;
...
function treeCursorKey(vault, maxEntries, maxDepth) {
    return createHash('sha256')
        .update(`${vault.id}:${String(vault.generation)}:${String(maxEntries)}:${String(maxDepth)}`)
        .digest('hex');
}
function treeFingerprint(scan) {
...
function encodeTreeCursor(offset, key, fingerprint) {
    return Buffer.from(JSON.stringify({ fingerprint, key, offset, version: 1 })).toString('base64url');
}
function decodeTreeCursor(cursor, key, maxOffset) {
    if (cursor == null)
        return { fingerprint: null, offset: 0 };
    try {
        ...
        if (parsed.version !== 1
            ...
            || parsed.offset < 0
            || parsed.offset > maxOffset)
            throw new Error();
        return { fingerprint: parsed.fingerprint, offset: parsed.offset };
    }
    catch {
...
function sameSyncFile(left, right) {
...
function readStateTextSync(filePath, maxBytes) {
    let descriptor;
    try {
        ...
        if (!opened.isFile() || !sameSyncFile(entry, opened))
            return null;
        ...
        if (!sameSyncFile(opened, final) || !sameSyncFile(opened, current))
            return null;
        ...
        if (descriptor !== undefined) {
            ...
            catch { /* fail-closed read cleanup */ }
        }
    }
}
function writeStateTextSync(filePath, content) {
...
function vaultStateDirectorySync(stateRoot) {
...
function searchIndexDirectorySync(stateRoot) {
    ...
    for (const segment of ['search-index', ISOLATED_SEARCH_SCHEMA]) {
...
function parsePersistedRecentVaults(value, limit) {
    ...
    for (const candidate of value) {
        ...
        let root;
        try {
            root = resolveVaultRoot(record.root);
        }
        catch {
            continue;
        }
...
function loadPersistedRecentVaults(stateRoot, limit) {
    const directory = vaultStateDirectorySync(stateRoot);
    const raw = readStateTextSync(path.join(directory, 'recent.json'), 1024 * 1024);
    if (raw === null)
        return [];
    try {
        return parsePersistedRecentVaults(JSON.parse(raw), limit);
    }
    catch {
        return [];
    }
}
function loadPersistedActiveVault(stateRoot) {
    const directory = vaultStateDirectorySync(stateRoot);
    ...
    if (value.length === 0 || value.length > 32_768 || value.includes('\0'))
        return null;
    try {
        return resolveVaultRoot(value);
    }
    catch {
        return null;
    }
}
function loadLegacyRecentVaults(stateRoot, limit) {
...
function loadLegacyActiveVault(stateRoot) {
...
function loadPersistedVaultSelection(stateRoot, limit) {
    const directory = vaultStateDirectorySync(stateRoot);
...
function persistVaultSelection(stateRoot, activeRoot, recents) {
    const directory = vaultStateDirectorySync(stateRoot);
    ...
    for (const legacyName of ['active', 'recent.json']) {
        try {
            unlinkSync(path.join(directory, legacyName));
        }
...
export class NoteVaultRuntime extends Service {
    ...
    searchIndexDisposed = false;
    searchIndexFailure;
    searchIndexDiagnostic;
    searchIndexWarned = false;
    vaultIdentity;
    vaultRoot;
    vaultTransitionPending = false;
    watcher = null;
    watcherStartup = new WeakMap();
    ...
    constructor(ctx, config) {
        ...
        ctx.effect(() => {
            ...
            return async () => {
                ...
                if (searchIndex !== null)
                    this.retireIndex(searchIndex.index);
                await this.searchIndexReplacement;
                ...
                if (this.searchIndexFailure)
                    throw new Error('Search-index ownership could not be verified; indexing remains disabled.');
            };
        });
    ...
    async [Service.init]() {
        if (this.watcher)
            await this.watcherStartup.get(this.watcher)?.promise;
    }
    ...
    async awaitWatcherStartup(signal) {
        signal.throwIfAborted();
        if (!this.watcherActive)
        ...
        signal.addEventListener('abort', onAbort, { once: true });
        try {
            await Promise.race([startup, cancelled.promise]);
        }
        finally {
            signal.removeEventListener('abort', onAbort);
        }
    ...
    async emitWatcherChange(root, state, token, eventType, filename) {
        if (!this.watcherActive
            || this.watcherToken !== token
            || this.currentState !== state
            || this.vaultRoot !== root)
    ...
    async synchronizeDesktopSelection(signal) {
        const state = this.currentState;
        const root = this.vaultRoot;
        if (!state.active || root === null)
            throw new NoteVaultError('inactive', 'No vault is active');
        ...
        try {
            ...
            operationSignal.throwIfAborted();
            this.assertCapturedVault(state, root);
            if (cause instanceof NoteVaultError)
                throw cause;
    ...
    removeRecentVault(id, expectedGeneration) {
        ...
        if (state.generation !== expectedGeneration)
            throw new NoteVaultError('stale-vault', 'The active vault changed before recent-vault removal');
    ...
    openSandboxVault(expectedGeneration) {
        ...
        if (this.stateRoot === null)
            throw new NoteVaultError('unavailable', 'Sandbox storage is unavailable');
        ...
        try {
            ...
            try {
                ...
                while (offset < data.byteLength) {
                    const written = writeSync(descriptor, data, offset, data.byteLength - offset);
                    if (written <= 0)
                        throw new Error('sandbox seed write stopped');
                    offset += written;
                }
                fsyncSync(descriptor);
            }
    ...
    createManagedVault(name, expectedGeneration) {
        ...
        try {
            ...
            if (error.code === 'EEXIST')
                throw new NoteVaultError('exists', 'Managed vault already exists');
            throw error;
        }
        return this.activate(root, expectedGeneration);
    }
    ...
    activateRecentVault(id, expectedGeneration) {
        ...
        if (recent === undefined)
            throw new NoteVaultError('not-found', 'Recent vault not found');
        return this.activate(recent.root, expectedGeneration);
    }
    invalidateSearchIndex(vault, changedPath) {
        if (this.searchIndex?.vault.id === vault.id
    ...
    warnIndexFailure() {
        if (this.searchIndexWarned)
            return;
        this.searchIndexWarned = true;
        try {
            this.context.logger.warn('Search indexing is unavailable; using exact scanning.');
        }
        catch { /* Diagnostics cannot interrupt cleanup. */ }
    }
    retireIndex(index) {
        const cleanup = retireSearchIndex(index);
        this.searchIndexCleanup.add(cleanup);
        void cleanup.then(() => this.searchIndexCleanup.delete(cleanup), error => {
            this.searchIndexCleanup.delete(cleanup);
            this.searchIndexFailure ??= error;
            this.warnIndexFailure();
        });
    }
    createSearchIndex(options) {
        return new SearchIndexProcess(options);
    }
    replaceSearchIndex() {
        const previous = this.searchIndex;
        this.searchIndex = null;
        if (previous !== null)
            this.retireIndex(previous.index);
        if (this.searchIndexDisposed || searchIndexOwnershipFailure())
            return;
        this.searchIndexRequested = true;
        if (this.searchIndexReplacement)
            return;
        this.searchIndexReplacement = (async () => {
            await Promise.resolve(); // Coalesce synchronous vault transitions before admission.
            while (this.searchIndexRequested && !this.searchIndexDisposed) {
                this.searchIndexRequested = false;
                await awaitSearchIndexSettlement(() => {
                    if (!this.searchIndexRequested && !this.searchIndexDisposed)
            ...
            this.searchIndexReplacement = undefined;
            if (this.searchIndexRequested && !this.searchIndexDisposed)
                this.replaceSearchIndex();
        });
    }
    installSearchIndex() {
        if (!searchIndexOwnershipFailure()
            && this.stateRoot !== null
            ...
            if (!isInside(root, this.stateRoot)) {
                try {
                    directory = searchIndexDirectorySync(this.stateRoot);
                }
                catch { /* scanner fallback */ }
            }
            if (directory !== null) {
                const index = this.createSearchIndex({
                    ...
                    failed: error => {
                        this.searchIndexDiagnostic = error;
                        if (this.searchIndex?.index === index)
                            this.searchIndex = null;
                        this.retireIndex(index);
                        this.warnIndexFailure();
                    },
                    list: async (signal, progress) => {
                        signal.throwIfAborted();
                        this.assertCapturedVault(state, root);
                    ...
                    read: async (requestedPath, signal, progress) => {
                        try {
                            this.assertCapturedVault(state, root);
                            const document = await readVaultDocument(root, requestedPath, this.maxReadBytes, signal, true, progress);
                            this.assertCapturedVault(state, root);
                            return {
                ...
                this.searchIndex = { vault, index };
            }
        }
    }
    async searchCandidates(expectedVault, request, signal) {
        const current = this.searchIndex;
        if (searchIndexOwnershipFailure() !== null
            || current === null
            || current.vault.id !== expectedVault.id
            || current.vault.generation !== expectedVault.generation)
            return null;
        const result = await current.index.search(request, signal);
        return searchIndexOwnershipFailure() || current !== this.searchIndex ? null : result;
    }
    createInspection(expectedVault, maxResults = Math.min(DEFAULT_MAX_INSPECTION_RESULTS, this.maxTreeResults)) {
        const input = {
            list: async (request, signal) => {
                ...
                const truncationReason = page.truncationReason === 'depth-limit'
                    ? 'entry-limit'
        ...
        const limits = {
            maxReadBytes: this.maxReadBytes,
            maxSearchBytes: Math.min(DEFAULT_MAX_INSPECTION_BYTES, this.maxReadBytes * this.treeConfig.maxEntries),
    ...
    async runInspection(expectedVault, signal, operation) {
    ...
    async runDraftOperation(key, operation) {
        ...
        try {
            ...
            if (this.draftOperations.get(key) === settled)
                this.draftOperations.delete(key);
        }
    }
    ...
    async openDocument(requestedPath, expectedVault, signal) {
        await this.awaitWatcherStartup(signal);
        const { root, state } = this.captureExpectedVault(expectedVault);
        let document;
        try {
    ...
    async listPassiveBackupEntries(request, signal) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        signal.throwIfAborted();
        try {
    ...
    async readPassiveBackupEntry(request, signal) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        try {
    ...
    async restorePassiveBackupEntry(request, signal) {
        ...
        try {
            ...
            if (committed)
                throw new NoteVaultError('partial', 'Passive backup entry was restored but could not be inspected');
            if (error instanceof NoteVaultError || (error instanceof Error && error.name === 'AbortError'))
                throw error;
            if (error.code === 'EEXIST') {
    ...
    async moveFileInternal(request, signal, emitEvent) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        signal.throwIfAborted();
        let source;
        let destination;
        ...
        try {
            ...
            if (emitEvent)
                this.emitFileMutation('moved', result.fromPath, result.path, state);
            return result;
        }
        catch {
    ...
    async moveFile(request, signal) {
        return await this.moveFileInternal(request, signal, true);
    }
    async duplicateFile(request, signal) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        signal.throwIfAborted();
        let source;
        let destination;
        ...
        try {
            ...
            try {
                ...
                this.assertCapturedVault(state, root);
                await assertFileEntryUnchanged(root, source, request.expectedRevision);
                const sourceDocument = await readVaultDocument(root, source.relativePath, this.maxReadBytes, signal);
                const destinationDocument = await readVaultDocument(root, destination.relativePath, this.maxReadBytes, signal);
                this.assertCapturedVault(state, root);
                ...
                if (!await rollbackCreatedEntry(destination.candidate, created)) {
                    throw new NoteVaultError('partial', 'The file duplicate did not finish and its destination was retained');
                }
                throw error;
            }
            await bestEffortFsync(path.dirname(destination.candidate));
    ...
    async mutateFolder(request, signal, status, emitEvent = true) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        signal.throwIfAborted();
        ...
        try {
            await copyFolderManifest(sourceManifest, root, sourceRoot, destinationCandidate, signal, () => this.assertCapturedVault(state, root), () => { destinationCreated = true; });
            this.assertCapturedVault(state, root);
            ...
            if (status === 'moved') {
                ...
                try {
                    ...
                    try {
                        ...
                        throw new NoteVaultError('partial', `Folder move was interrupted; partial destination ${toPath} and source recovery remain`);
                    }
                    throw error;
                }
            ...
            if (destinationCreated) {
                throw new NoteVaultError('partial', `Folder ${status === 'moved' ? 'move' : 'duplicate'} was interrupted; partial destination ${toPath} was retained`);
            }
            if (error.code === 'EEXIST') {
        ...
        if (emitEvent)
            this.emitFileMutation(status, result.fromPath, result.path, state);
        return result;
    }
    async duplicateFolder(request, signal) {
        return await this.mutateFolder(request, signal, 'duplicated');
    }
    async moveFolder(request, signal) {
        return await this.mutateFolder(request, signal, 'moved');
    }
    async completePathRewritePlan(args, expectedVault, signal) {
        const { root, state } = this.captureExpectedVault(expectedVault);
        ...
        while (true) {
            signal.throwIfAborted();
            const page = await inspection.planPathRewrite({
                ...args,
    ...
    async documentPathsByCanonicalTarget(expectedVault, signal) {
        const paths = new Map();
        const seenCursors = new Set();
        let cursor;
        while (true) {
        ...
        for (const logicalPaths of paths.values())
            logicalPaths.sort(compareVaultPaths);
        return paths;
    }
    async preparePathRewrites(updates, args, expectedVault, signal) {
        const { root, state } = this.captureExpectedVault(expectedVault);
        ...
        for (const update of updates) {
            ...
            const preMovePath = normalizeDocumentPath(preMoveReferrerPath(postMovePath, args.oldPath, args.newPath, args.isDirectory));
        ...
        for (const [canonicalPath, group] of updatesByCanonical) {
            ...
            const desiredContents = new Set(logicalPaths.map(logicalPath => (updateByPreMovePath.get(logicalPath)?.newContent ?? originalContent)));
            let writer;
            let rewritten;
            if (desiredContents.size === 1) {
                writer = [...group].sort((left, right) => compareVaultPaths(left.postMovePath, right.postMovePath))[0];
                rewritten = group;
            }
            else {
    ...
    async moveWithLinkRewrite(request, signal, isDirectory) {
        const { root, state } = this.captureExpectedVault(request.expectedVault);
        ...
        if (!isDirectory && /\.md$/iu.test(oldPath)) {
            ...
            try {
                ...
                if (sidecar.isDirectory() && !sidecar.isSymbolicLink()) {
                    throw new NoteVaultError('unsupported-type', 'Move with link rewrites cannot leave a Markdown sidecar folder behind');
                }
            }
            catch (error) {
                if (error.code !== 'ENOENT')
        ...
        for (const rewrite of prepared.selected) {
            try {
                ...
                if (saved.status !== 'saved')
                    throw new Error('Link rewrite did not save an existing document');
    ...
    async moveFileWithLinkRewrite(request, signal) {
        return await this.moveWithLinkRewrite(request, signal, false);
    }
    ...
    async trashEntry(request, signal) {
        ...
        if (sourceEntry.isDirectory() && !sourceEntry.isSymbolicLink()) {
            ...
            throw new NoteVaultError('unsupported-type', 'Trash supports only vault documents, attachments, aliases, and folders');
        }
    ...
    async inspectAttachment(requestedPath, expectedVault, signal) {
        const { root, state } = this.captureExpectedVault(expectedVault);
        let attachment;
        try {
            attachment = await inspectVaultAttachment(root, requestedPath, this.maxAttachmentBytes, signal, false);
        }
        catch (error) {
            if (error instanceof NoteVaultError || (error instanceof Error && error.name === 'AbortError')) {
                throw error;
    ...
    async previewAttachment(requestedPath, expectedVault, signal) {
        const { root, state } = this.captureExpectedVault(expectedVault);
        let attachment;
        try {
            attachment = await inspectVaultAttachment(root, requestedPath, this.maxAttachmentBytes, signal, true);
        }
        catch (error) {
            if (error instanceof NoteVaultError || (error instanceof Error && error.name === 'AbortError')) {
                throw error;
        ...
        if (attachment.data === null)
            throw new Error('unreachable attachment preview without data');
        return {
