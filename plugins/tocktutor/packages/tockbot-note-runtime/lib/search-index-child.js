...
import { connectIndexPeer } from "./search-index-auth.js";
import { IndexProtocol, documentRecord, exact, integer, relativePath, text, ISOLATED_SEARCH_SCHEMA } from "./search-index-protocol.js";
import { NativeOperationProgress } from "./search-index-progress.js";
import { PersistentSearchIndex, acquireSearchIndexLease } from "./search-index-native.js";
function searchRequest(value) {
    ...
    return Object.keys(request).every(key => ['directory', 'groups', 'limit', 'modifiedFrom', 'modifiedTo'].includes(key))
        ...
        && request.groups.every(group => Array.isArray(group) && group.length > 0 && group.length <= 256
            && group.every(anchor => exact(anchor, ['field', 'value']) && ['property', 'tag'].includes(anchor.field) && text(anchor.value, 4096)))
...
async function main() {
    ...
    if (!bootstrap || process.versions.electron)
        throw new Error('Invalid index child bootstrap');
    ...
    protocol = new IndexProtocol(peer, async (action, args, signal) => {
        if (action === 'init') {
            if (initialized || !exact(args, ['directory', 'identity', 'vaultId', 'maxReadBytes'])
                ...
                || !integer(args.maxReadBytes, 1, 2 * 1024 * 1024))
                throw new Error('Invalid index initialization');
            initialized = true;
            const { identity, vaultId, maxReadBytes } = args;
            const directory = await progress.run(() => realpath(args.directory));
            ...
            index = new PersistentSearchIndex({ directory, identity, vaultId, schema: ISOLATED_SEARCH_SCHEMA, progress,
                list: async () => {
                    ...
                    if (reply.meta === null && reply.items.length === 0)
                        return null;
                    if (reply.meta !== true || !reply.items.every(documentRecord))
                        throw new Error('Invalid index inventory response');
                    return reply.items;
                },
                read: async (requestedPath) => {
                    ...
                    if (reply.meta === null && reply.items.length === 0)
                        return null;
                    if (!documentRecord(reply.meta) || reply.meta.path !== requestedPath)
                        throw new Error('Invalid index document response');
                    const chunks = [];
                    let bytes = 0;
                    for (const item of reply.items) {
                        if (typeof item !== 'string' || item.length > 43692)
                            throw new Error('Invalid index document chunk');
                        const buffer = Buffer.from(item, 'base64');
                        bytes += buffer.length;
                        if (buffer.length > 32768 || buffer.toString('base64') !== item || bytes > 3 * maxReadBytes)
                            throw new Error('Invalid index document encoding');
                        chunks.push(buffer);
                    }
                ...
                changed: value => {
                    ready = value;
                    if (value)
                        void protocol.notify('ready', { revision }).catch(die);
                },
        ...
        if (action !== 'search' || !index || !lease || !exact(args, ['revision', 'request']) || !integer(args.revision)
            || args.revision > revision || !searchRequest(args.request))
            throw new Error('Invalid index search');
        const captured = args.revision;
        if (!ready || captured !== revision || signal.aborted)
        ...
        if (!result || !ready || captured !== revision)
            return { items: [], meta: null };
        ...
        if (action !== 'invalidate' || !index || !lease || !exact(value, ['revision', 'paths']) || !integer(value.revision, revision + 1)
            || (value.paths !== null && (!Array.isArray(value.paths) || value.paths.length < 1 || value.paths.length > 256
                || !value.paths.every(relativePath) || Buffer.byteLength(JSON.stringify(value.paths)) > 32770)))
            throw new Error('Invalid index invalidation');
        revision = value.revision;
        ready = false;
        if (value.paths === null)
            index.invalidate();
        else
            for (const changedPath of value.paths)
                index.invalidate(changedPath);
    }, die);
}
