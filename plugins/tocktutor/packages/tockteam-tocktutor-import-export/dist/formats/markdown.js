...
export const MARKDOWN_ARCHIVE_LIMITS = {
...
function planMarkdown(files) {
    if (!Array.isArray(files) || files.length === 0)
        throw new ImportExportError('unsupported-type');
    if (files.length > MARKDOWN_MAX_ENTRIES)
    ...
    for (const source of files.toSorted((left, right) => left.path.localeCompare(right.path))) {
        ...
        if (size > MARKDOWN_MAX_TOTAL_BYTES)
            throw new ImportExportError('limit-exceeded');
        planned.push({
...
export function planMarkdownFolder(files) {
    return planMarkdown(files);
}
export function planMarkdownZip(bytes, signal) {
