...
export function parseZip(input, limits, options = {}) {
    ...
    for (const entry of central) {
        ...
        if (directory && (entry.uncompressedSize !== 0 || entry.crc !== 0))
            return invalidArchive();
        if (!directory && !options.allowNestedArchives && nestedArchive(entry.path))
            return invalidArchive();
        ...
        for (const [field, expected] of [[14, entry.crc], [18, entry.compressedSize], [22, entry.uncompressedSize]]) {
        ...
        if (hasDescriptor) {
            ...
            if (dataEnd + 4 <= centralOffset && archive.readUInt32LE(dataEnd) === 0x08074b50)
                offsets.push(dataEnd + 4);
            if (!offsets.some(start => start + 12 <= centralOffset
                ...
                && archive.readUInt32LE(start + 8) === entry.uncompressedSize))
                return invalidArchive();
        }
