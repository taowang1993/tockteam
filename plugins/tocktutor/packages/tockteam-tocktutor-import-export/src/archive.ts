...
function safeInteger(value: number, maximum: number): boolean {
...
function validateLimits(limits: ArchiveLimits): void {
  if (!safeInteger(limits.maxArchiveBytes, Number.MAX_SAFE_INTEGER)
    ...
    || limits.maxCompressionRatio < 1) limitExceeded()
}
...
function findEnd(archive: Buffer): number {
  ...
  for (let offset = archive.byteLength - 22; offset >= minimum; offset -= 1) {
    if (archive.readUInt32LE(offset) === END_SIGNATURE
      && offset + 22 + archive.readUInt16LE(offset + 20) === archive.byteLength) return offset
  }
  return invalidArchive()
}

function decodeName(bytes: Buffer, flags: number): string {
  if ((flags & UTF8_FLAG) === 0 && bytes.some(byte => byte > 0x7f)) return invalidArchive()
  const value = bytes.toString('utf8')
  if (Buffer.from(value, 'utf8').compare(bytes) !== 0 || value.includes('\0')) return invalidArchive()
  return value
}

function confinedArchivePath(raw: string, limits: ArchiveLimits): { directory: boolean; path: string } {
  ...
  if (segments.length > limits.maxDepth) limitExceeded()
  if (segments.some(segment => segment === '__MACOSX' || segment.startsWith('._'))) return invalidArchive()
  return { directory, path }
}

function nestedArchive(path: string): boolean {
  return /\.(?:bear2bk|textpack|zip)$/iu.test(path)
}

function unixKind(entry: CentralEntry): number {
  return (entry.externalAttributes >>> 16) & 0xf000
}

function assertEntryKind(entry: CentralEntry, directory: boolean): void {
  const kind = unixKind(entry)
  if (kind === UNIX_SYMLINK) return invalidArchive()
  if (kind !== 0 && kind !== UNIX_FILE && kind !== UNIX_DIRECTORY) return invalidArchive()
  if (!directory && kind === UNIX_FILE && ((entry.externalAttributes >>> 16) & 0o111) !== 0) return invalidArchive()
  if (directory && kind !== 0 && kind !== UNIX_DIRECTORY) return invalidArchive()
  if (!directory && kind === UNIX_DIRECTORY) return invalidArchive()
}

export function parseZip(
  input: Uint8Array,
  limits: ArchiveLimits,
  options: { allowNestedArchives?: boolean; signal?: AbortSignal } = {},
): ArchiveEntry[] {
  validateLimits(limits)
  const startedAt = Date.now()
  const checkpoint = (): void => {
    options.signal?.throwIfAborted()
    if (Date.now() - startedAt > limits.maxParserMs) limitExceeded()
  }
  checkpoint()
  if (!(input instanceof Uint8Array) || input.byteLength > limits.maxArchiveBytes) limitExceeded()
  const archive = Buffer.from(input.buffer, input.byteOffset, input.byteLength)
  if (archive.byteLength < 22) return invalidArchive()
  const end = findEnd(archive)
  if (archive.readUInt16LE(end + 4) !== 0
    || archive.readUInt16LE(end + 6) !== 0
    || archive.readUInt16LE(end + 8) !== archive.readUInt16LE(end + 10)) return invalidArchive()
  ...
  if (end + 22 + commentLength !== archive.byteLength
    ...
    || centralOffset > archive.byteLength) return invalidArchive()

  const central: CentralEntry[] = []
  let offset = centralOffset
  for (let index = 0; index < count; index += 1) {
    checkpoint()
    if (offset + 46 > end || archive.readUInt32LE(offset) !== CENTRAL_SIGNATURE) return invalidArchive()
    ...
    if (next > end || diskStart !== 0 || nameLength === 0
      ...
      || uncompressedSize > limits.maxEntryBytes) return invalidArchive()
  ...
  if (offset !== end) return invalidArchive()

  const aliases = new Set<string>()
  for (const entry of central) {
    const alias = destinationAliasKey(entry.path)
    if (aliases.has(alias)) return invalidArchive()
    aliases.add(alias)
  }
  ...
  for (const entry of central) {
    ...
    if (entry.localOffset + 30 > centralOffset
      ...
      || archive.readUInt16LE(entry.localOffset + 8) !== entry.method) return invalidArchive()
