  ...
  for (const file of files.slice(0, 500)) {
    ...
    if (date !== undefined) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/u.test(date) || Number.isNaN(parsed.getTime())
        || parsed.toISOString().slice(0, 10) !== date) throw new ImportExportError('unsupported-type')
    }
...
export function planGoogleKeep(bytes: Uint8Array): PlannedSourceResult {
  ...
  for (const entry of entries) {
    ...
    const labels = Array.isArray(note.labels)
      ? note.labels.flatMap(label => label !== null && typeof label === 'object' && typeof (label as { name?: unknown }).name === 'string' ? [(label as { name: string }).name] : [])
      : []
    const tasks = Array.isArray(note.listContent)
      ? note.listContent.flatMap(item => item !== null && typeof item === 'object'
        ? [`- ${(item as { isChecked?: unknown }).isChecked === true ? '[x]' : '[ ]'} ${String((item as { text?: unknown }).text ?? '')}`]
        : [])
      : []
