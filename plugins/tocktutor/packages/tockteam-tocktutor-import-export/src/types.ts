...
export const IMPORT_INSPECT_FORMATS = [
  ...
  'textbundle',
  'restore-backup',
] as const satisfies readonly DesktopSourcePurpose[]

export type ImportInspectFormat = (typeof IMPORT_INSPECT_FORMATS)[number]

export function isImportInspectFormat(value: unknown): value is ImportInspectFormat {
  return typeof value === 'string' && (IMPORT_INSPECT_FORMATS as readonly string[]).includes(value)
}
...
export type ReviewPlanView = ReviewedPlanSummary & { reviewToken: string }
...
export type CleanupEvidenceView =
  | { status: 'complete' }
  | { residualLabels: string[]; status: 'residual' | 'retained' | 'scrubbed' }
