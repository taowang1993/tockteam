import { MAX_EXECUTABLE_BASE_FILE_BYTES } from "./base-query.js";
import { inferPropertyType, parseFrontmatterProperties, setFrontmatterProperty } from "./properties.js";
const EDITABLE_COLUMN = /^note\.([A-Za-z_][\w-]*)$/u;
const REVISION = /^file:[0-9a-f]{64}$/u;
export function executableBasePropertyIdentity(property, value) {
...
export function createExecutableBaseFrontmatterEdit(file, column, rawValue) {
    ...
    if (value === null || Object.is(value, previous.value))
        return null;
    let source;
    try {
        source = setFrontmatterProperty(file.source, previous.key, value);
    }
    catch {
        return null;
    }
    if (source === file.source || new TextEncoder().encode(source).byteLength > MAX_EXECUTABLE_BASE_FILE_BYTES)
        return null;
    return {
...
//# sourceMappingURL=base-edit.js.map