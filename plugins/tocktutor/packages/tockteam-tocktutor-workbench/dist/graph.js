...
export function projectGraph(result, options) {
    if (!Array.isArray(result.nodes) || result.nodes.length > MAX_GRAPH_NODES)
        throw new Error('Graph node limit exceeded.');
    if (!Array.isArray(result.edges) || result.edges.length > MAX_GRAPH_EDGES)
...
function hash(value) {
    ...
    for (const character of value)
        result = Math.imul(result ^ character.codePointAt(0), 16777619) >>> 0;
    return result;
}
...
export function layoutGraph(graph, options) {
    if (graph.nodes.length > MAX_GRAPH_NODES || graph.edges.length > MAX_GRAPH_EDGES)
        throw new Error('Graph layout limit exceeded.');
