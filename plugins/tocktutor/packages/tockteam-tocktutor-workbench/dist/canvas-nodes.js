import { parseCanvasForMutation, serializeCanvasDocument, } from "./canvas.js";
import { CANVAS_GRID_SIZE, MIN_CANVAS_NODE_HEIGHT, MIN_CANVAS_NODE_WIDTH, calculateCanvasPointerValue, isBoundedCanvasGeometry, } from "./canvas-geometry.js";
import { normalizeCanvasLinkUrl } from "./canvas-links.js";
const SUPPORTED_CANVAS_CARD_TYPES = new Set(['text', 'file', 'link']);
export const CANVAS_DEFAULT_TEXT_CARD_SIZE = { width: 260, height: 140 };
export function isSupportedCanvasCard(node) {
    return typeof node.type === 'string' && SUPPORTED_CANVAS_CARD_TYPES.has(node.type);
}
function nextCanvasId(document, prefix) {
    const existingIds = new Set([
        ...document.nodes.map(node => node.id),
        ...(document.edges ?? []).map(edge => edge.id),
    ]);
    let index = 1;
    while (existingIds.has(`${prefix}-${String(index)}`))
        index += 1;
    return `${prefix}-${String(index)}`;
}
function createCanvasNode(content, prefix, fields, size = CANVAS_DEFAULT_TEXT_CARD_SIZE) {
    const document = parseCanvasForMutation(content);
    const rightmost = document.nodes.reduce((right, node) => Math.max(right, node.x + Math.max(MIN_CANVAS_NODE_WIDTH, node.width)), 0);
...
export function createCanvasTextNode(content, position) {
...
export function createCanvasGroupNode(content) {
...
export function createCanvasGroupFromSelection(content, nodeIds) {
    const document = parseCanvasForMutation(content);
    ...
    if (!isBoundedCanvasGeometry(geometry))
...
export function createCanvasFileNode(content, relativePath) {
...
export function createCanvasLinkNode(content, value) {
    return createCanvasNode(content, 'link', { type: 'link', url: normalizeCanvasLinkUrl(value) });
}
export function updateCanvasLinkNode(content, nodeId, value) {
    const document = parseCanvasForMutation(content);
    const node = document.nodes.find(candidate => candidate.id === nodeId);
    if (node?.type !== 'link')
        throw new Error('The selected Canvas link card no longer exists.');
    node.url = normalizeCanvasLinkUrl(value);
    return serializeCanvasDocument(document);
}
export function updateCanvasTextNode(content, nodeId, text) {
    const document = parseCanvasForMutation(content);
    const node = document.nodes.find(candidate => candidate.id === nodeId);
    if (node?.type !== 'text')
        throw new Error('The selected Canvas text card no longer exists.');
    node.text = text;
    return serializeCanvasDocument(document);
}
/** Validate every selected card before applying one atomic geometry update. */
export function updateCanvasNodeGeometries(content, updates) {
    const document = parseCanvasForMutation(content);
    ...
    for (const update of updates) {
        ...
        if (!isBoundedCanvasGeometry(update.geometry))
            throw new Error('The Canvas card geometry is invalid.');
    }
    for (const update of updates) {
...
export function moveCanvasNodes(content, nodeIds, deltaX, deltaY) {
    const document = parseCanvasForMutation(content);
...
export function updateCanvasGroupGeometry(content, nodeId, geometry) {
    const document = parseCanvasForMutation(content);
...
export function updateCanvasNodeGeometry(content, nodeId, geometry) {
    const document = parseCanvasForMutation(content);
    const node = document.nodes.find(candidate => candidate.id === nodeId);
    if (node?.type === 'group')
        return updateCanvasGroupGeometry(content, nodeId, geometry);
    return updateCanvasNodeGeometries(content, [{ nodeId, geometry }]);
}
export function updateCanvasGroupLabel(content, nodeId, label) {
    const document = parseCanvasForMutation(content);
    const node = document.nodes.find(candidate => candidate.id === nodeId);
    if (node?.type !== 'group')
        throw new Error('The selected Canvas group no longer exists.');
    ...
    if (!normalizedLabel)
        throw new Error('The Canvas group label cannot be empty.');
    node.label = normalizedLabel;
    return serializeCanvasDocument(document);
}
/** Delete a group boundary without deleting contained cards or edges. */
export function deleteCanvasGroup(content, nodeId) {
    const document = parseCanvasForMutation(content);
...
export function deleteCanvasNodes(content, nodeIds) {
    const document = parseCanvasForMutation(content);
    const selectedIds = new Set(nodeIds);
    const selectedNodes = document.nodes.filter(node => selectedIds.has(node.id));
...
export function deleteCanvasNode(content, nodeId) {
...
export function duplicateCanvasGroup(content, nodeId, geometry) {
    const document = parseCanvasForMutation(content);
...
export function duplicateCanvasNodes(content, updates) {
    const document = parseCanvasForMutation(content);
    ...
    for (const update of updates)
        copiedNodeIds.set(update.nodeId, nextCopyId(update.nodeId, ids));
    for (const update of updates) {
