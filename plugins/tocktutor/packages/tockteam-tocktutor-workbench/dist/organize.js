...
export function buildHighlightNote(input) {
    ...
    if (title === '' || input.highlights.length === 0 || input.highlights.length > 1_000)
        throw new Error('Highlight input is invalid.');
    const url = safeUrl(input.sourceUrl);
    if (input.sourceUrl !== undefined && input.sourceUrl.trim() !== '' && url === null)
        throw new Error('Highlight source URL is invalid.');
    const body = input.highlights.map(highlight => {
        if (bytes(highlight) > MAX_ORGANIZE_BYTES)
            throw new Error('Highlight input is too large.');
    ...
    if (bytes(content) > MAX_ORGANIZE_BYTES)
        throw new Error('Highlight input is too large.');
    return { content, path: `Highlights/${date(input.now)}-${slug(title)}.md` };
}
export function buildOrganizationProposal(input) {
    if (input.captures.length === 0 || input.captures.length > MAX_ORGANIZE_CAPTURES)
        throw new Error('Organization requires bounded captures.');
    const title = input.title.trim().slice(0, 200);
    if (title === '')
        throw new Error('Organization title is required.');
    const seen = new Set();
    for (const capture of input.captures) {
        if (!isSafeVaultRelativePath(capture.path) || !/^Inbox\/.+\.md$/iu.test(capture.path))
            throw new Error('Capture path is invalid.');
        if (seen.has(capture.path) || bytes(capture.content) > MAX_ORGANIZE_BYTES)
            throw new Error('Capture input is invalid or too large.');
    ...
    const content = `# ${title}\n\n${sections.join('\n\n')}\n`;
    if (bytes(content) > MAX_ORGANIZE_BYTES)
        throw new Error('Organization output is too large.');
