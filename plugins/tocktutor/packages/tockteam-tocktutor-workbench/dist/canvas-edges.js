...
import { isCanvasSide } from "./canvas-geometry.js";
import { CANVAS_DEFAULT_TEXT_CARD_SIZE, createCanvasTextNode, isSupportedCanvasCard, } from "./canvas-nodes.js";
export function isConnectableCanvasNode(node) {
...
export function createCanvasEdge(content, connection) {
    ...
    let index = 1;
    while (existingIds.has(`edge-${String(index)}`))
        index += 1;
...
export function createCanvasConnectedTextNode(content, connection) {
    if (!isCanvasSide(connection.fromSide))
        throw new Error('Canvas connections require valid node sides.');
...
export function reconnectCanvasEdge(content, update) {
    const document = parseCanvasForMutation(content);
    const edge = document.edges?.find(candidate => candidate.id === update.edgeId);
    if (edge === undefined)
        throw new Error('The selected Canvas edge no longer exists.');
...
function edgeDocument(content) {
    ...
    if (document.edges === undefined)
        throw new Error('This .canvas file does not contain Canvas edges.');
    return document;
}
export function updateCanvasEdgeLabel(content, edgeId, label) {
...
export function updateCanvasEdgeColor(content, edgeId, color) {
    const document = edgeDocument(content);
    const edge = document.edges.find(candidate => candidate.id === edgeId);
    if (edge === undefined)
        throw new Error('The selected Canvas edge no longer exists.');
...
export function deleteCanvasEdge(content, edgeId) {
    const document = edgeDocument(content);
...
//# sourceMappingURL=canvas-edges.js.map