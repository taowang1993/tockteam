...
import { BookmarkPlus, ChevronLeft, ChevronRight, FileClock, FileCode2, FileText, Globe2, Link2, ListTree, MessageSquare, Network, PanelsTopLeft, Paperclip, Pencil, Plus, Search, SlidersHorizontal, Tags, Trash2, Wrench, X, } from 'lucide-react';
...
import { addPaneGroup, closeNoteTab, createWorkbenchSession, focusPaneGroup, isSafeVaultRelativePath, markTabDirty, MAX_NOTE_TABS, MAX_PANE_GROUPS, moveNoteTab, openNoteTab, renameNoteTabPath, setActiveNoteTab, setNoteTabMode, setTabPinned, hydrateWorkbenchSession, } from "./session.js";
...
export class WorkbenchRouteController {
    ...
    async renameActiveTitle(title) {
        ...
        if (!isSafeVaultRelativePath(toPath))
            return false;
        if (toPath === fromPath)
            return true;
...
function WorkbenchNoteSearchPalette(props) {
    ...
    return (_jsxs("section", { "aria-label": "Search Notes", className: "tocktutor-search-sidebar grid h-full min-h-0 grid-rows-[48px_40px_minmax(0,1fr)] overflow-hidden border-b border-[var(--tt-border)] bg-[var(--tt-panel)] text-[var(--tt-text)] [--tt-accent:var(--dsw-alias-brand-primary,#533afd)] [--tt-border:var(--dsw-alias-border-l1,var(--dsw-alias-border-subtle,#e1e3e7))] [--tt-muted:var(--dsw-alias-label-secondary,#71717a)] [--tt-panel:var(--dsw-alias-bg-layer-1,#fff)] [--tt-selected:color-mix(in_srgb,var(--tt-text)_6%,var(--tt-panel))] [--tt-text:var(--dsw-alias-label-primary,#27272a)]", children: [_jsxs("header", { ref: searchInputContainer, className: "flex min-w-0 items-center gap-1.5 border-b border-[var(--tt-border)] px-2 text-[var(--tt-muted)] [&>svg]:size-4", children: [_jsx(Search, { "aria-hidden": "true" }), _jsx(Input, { unstyled: true, "aria-label": "Search Notes Query", autoFocus: true, className: "h-full min-w-0 flex-1 border-0 bg-transparent p-0 text-[13px] font-medium text-[var(--tt-text)] outline-none placeholder:text-[var(--tt-muted)]", maxLength: 1_000, onChange: event => { props.onSearchChange?.(event.target.value); }, onKeyDown: event => {
                            if (event.key === 'Escape') {
                                event.preventDefault();
                                props.onClose();
                                return;
                            ...
                            event.preventDefault();
                            props.onRunSearch?.();
                        }, placeholder: "Search notes...", type: "search", value: snapshot.searchQuery }), (snapshot.searchMode ?? 'query') === 'query' && (_jsxs(Popover, { open: searchOptionsOpen, onOpenChange: setSearchOptionsOpen, children: [_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx(PopoverTrigger, { asChild: true, children: _jsx(Button, { unstyled: true, "aria-label": "Search Options", className: "flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-md border-0 bg-transparent p-0 text-[var(--tt-muted)] hover:bg-[var(--tt-selected)] hover:text-[var(--tt-text)] data-[state=open]:bg-[var(--tt-selected)] data-[state=open]:text-[var(--tt-text)] [&_svg]:size-[14px]", type: "button", children: _jsx(SlidersHorizontal, { "aria-hidden": "true", strokeWidth: 1.75 }) }) }) }), _jsx(TooltipContent, { children: "Search Options" })] }), _jsxs(PopoverContent, { unstyled: true, align: "start", "aria-label": "Search Options", className: "z-[2147483647] box-border flex w-[300px] flex-col gap-2 rounded-xl border border-[var(--dsw-alias-border-l1,#e1e3e7)] bg-[var(--dsw-alias-bg-layer-1,#fff)] p-2.5 text-sm text-[var(--dsw-alias-label-primary,#27272a)] shadow-xl outline-none", onCloseAutoFocus: event => {
                                ...
                                }, role: "dialog", sideOffset: 8, children: [_jsxs(PopoverHeader, { className: "gap-0.5 px-1.5 pt-0.5", children: [_jsx(PopoverTitle, { className: "text-xs font-semibold", children: "Search syntax" }), _jsx(PopoverDescription, { className: "m-0 text-xs text-[var(--dsw-alias-label-secondary,#71717a)]", children: "Insert an operator at the cursor." })] }), _jsx("ul", { className: "m-0 grid list-none gap-1 p-0", children: SEARCH_OPTIONS.map(option => (_jsx("li", { children: _jsxs(Button, { unstyled: true, className: "grid w-full cursor-pointer grid-cols-[76px_1fr] items-start gap-2 rounded-lg border-0 bg-transparent px-2.5 py-2 text-left hover:bg-[var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,0.05))] focus-visible:bg-[var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,0.05))] focus-visible:outline-none", onClick: () => { insertSearchOption(option.value); }, type: "button", children: [_jsx("code", { className: "font-mono text-xs font-semibold leading-4 text-[var(--dsw-alias-brand-primary,#533afd)]", children: option.label }), _jsx("span", { className: "text-xs leading-4 text-[var(--dsw-alias-label-secondary,#71717a)]", children: option.description })] }) }, option.label))) })] })] })), _jsx(Button, { unstyled: true, "aria-label": "Close Search", className: "flex size-6 shrink-0 items-center justify-center rounded-md border-0 bg-transparent p-0 text-[var(--tt-muted)] hover:bg-[var(--tt-selected)] hover:text-[var(--tt-text)] [&_svg]:size-[14px]", onClick: props.onClose, type: "button", children: _jsx(X, { "aria-hidden": "true" }) })] }), _jsxs("div", { className: "flex min-w-0 items-center justify-between gap-1 border-b border-[var(--tt-border)] px-2 text-[11px] font-medium text-[var(--tt-muted)]", children: [_jsxs(ToggleGroup, { unstyled: true, type: "single", "aria-label": "Search Mode", className: "flex min-w-0 items-center gap-0.5", value: snapshot.searchMode ?? 'query', onValueChange: value => { if (value === 'query' || value === 'related')
                            props.onSearchMode?.(value); }, children: [_jsx(ToggleGroupItem, { unstyled: true, className: "rounded-md border-0 bg-transparent px-1.5 py-1 hover:bg-[var(--tt-selected)] data-[state=on]:bg-[var(--tt-selected)] data-[state=on]:text-[var(--tt-text)]", value: "query", children: "Keyword" }), _jsx(ToggleGroupItem, { unstyled: true, className: "rounded-md border-0 bg-transparent px-1.5 py-1 hover:bg-[var(--tt-selected)] data-[state=on]:bg-[var(--tt-selected)] data-[state=on]:text-[var(--tt-text)]", value: "related", children: "Related" })] }), _jsx(Button, { unstyled: true, className: "shrink-0 rounded-md border-0 bg-transparent px-1.5 py-1 hover:bg-[var(--tt-selected)] hover:text-[var(--tt-text)] disabled:opacity-40", disabled: snapshot.searchLoading === true || snapshot.searchQuery.trim() === '', onClick: props.onRunSearch, type: "button", children: snapshot.searchLoading === true ? 'Searching…' : 'Search' }), _jsx(Button, { unstyled: true, className: "shrink-0 rounded-md border-0 bg-transparent px-1.5 py-1 hover:bg-[var(--tt-selected)] hover:text-[var(--tt-text)]", onClick: props.onCommands, type: "button", children: "Commands" }), _jsx(Alert, { unstyled: true, "aria-live": "polite", className: "min-w-0 truncate text-[10px] font-normal text-[var(--tt-muted)]", role: "status", children: matches.length > 0 ? `${String(matches.length)} results` : `${String(pathResults.length)} paths` })] }), _jsxs("section", { className: "grid min-h-0 grid-rows-[28px_minmax(0,1fr)] px-1.5 pb-1.5", "aria-label": "Search Results", children: [_jsx("div", { className: "flex items-end px-1.5 pb-1 text-[10px] font-medium text-[var(--tt-muted)]", children: "Results" }), _jsx(NoteSearchResultList, { matches: matches, onPreview: setPreviewChoice, onSelect: props.onSelect, pathResults: pathResults, previewMatchIndex: previewMatchIndex, previewResultPath: previewResultPath, query: snapshot.searchQuery })] })] }));
}
