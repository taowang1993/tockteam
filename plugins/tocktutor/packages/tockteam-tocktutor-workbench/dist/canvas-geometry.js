...
export function calculateCanvasResizeGeometry(start, delta, aspectRatioLocked, snappingDisabled) {
    ...
    if (widthDominant) {
        const width = Math.max(MIN_CANVAS_NODE_WIDTH, MIN_CANVAS_NODE_HEIGHT * aspectRatio, calculateCanvasPointerValue(start.width, delta.x, snappingDisabled));
        return { ...start, width, height: width / aspectRatio };
    }
    const height = Math.max(MIN_CANVAS_NODE_HEIGHT, MIN_CANVAS_NODE_WIDTH / aspectRatio, calculateCanvasPointerValue(start.height, delta.y, snappingDisabled));
    return { ...start, width: height * aspectRatio, height };
}
//# sourceMappingURL=canvas-geometry.js.map