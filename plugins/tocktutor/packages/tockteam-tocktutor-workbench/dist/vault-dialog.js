...
import { Dialog, DialogClose, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger, } from '@tockteam/ui/dialog';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger, } from '@tockteam/ui/dropdown-menu';
...
function vaultMenuIcon(icon) {
    switch (icon) {
        case 'move': return _jsx(FolderTree, { "aria-hidden": "true" });
        case 'remove': return _jsx(X, { "aria-hidden": "true" });
        case 'rename': return _jsx(PencilLine, { "aria-hidden": "true" });
        case 'reveal': return _jsx(FolderOpen, { "aria-hidden": "true" });
        default: return null;
    }
}
function renderVaultMenuItem(item) {
    return (_jsxs(_Fragment, { children: [item.separatorBefore && _jsx(DropdownMenuSeparator, {}), _jsxs(DropdownMenuItem, { ...(item.live ? { 'aria-live': 'polite' } : {}), className: item.destructive ? 'text-destructive focus:text-destructive' : '', disabled: item.disabled === true, onSelect: event => { event.preventDefault(); item.select(); }, children: [vaultMenuIcon(item.icon), _jsx("span", { children: item.label })] })] }));
}
...
export function WorkbenchVaultDialog(props) {
    ...
    return (_jsxs(Dialog, { onOpenChange: changeOpen, open: open, children: [_jsx(DialogTrigger, { asChild: true, children: _jsxs(Button, { unstyled: true, "aria-expanded": open, className: "tocktutor-vault-switcher grid grid-cols-[14px_minmax(0,1fr)_16px] items-center gap-1.5 border-0 border-t border-[var(--tt-border)] bg-[var(--tockteam-shell-chrome,var(--tt-panel))] px-2.5 text-left [&>span]:truncate [&_svg]:size-[13px]", type: "button", children: [_jsx(WorkbenchGlyph, { kind: "collapse" }), _jsx("span", { children: props.vault === null ? 'Choose Vault' : props.vaultName ?? 'TockTutor Vault' }), _jsx(WorkbenchGlyph, { kind: "more" })] }) }), _jsxs(DialogContent, { unstyled: true, className: "fixed top-1/2 left-1/2 z-[2147483647] grid w-full -translate-x-1/2 -translate-y-1/2 gap-0 overflow-y-auto rounded-xl border border-[var(--tt-border)] bg-[var(--tt-panel)] p-0 text-[var(--tt-text)] shadow-[0_18px_48px_rgba(0,0,0,0.16),0_2px_8px_rgba(0,0,0,0.08)] outline-none sm:overflow-hidden [--tt-accent:var(--dsw-alias-brand-primary,#533afd)] [--tt-border:var(--dsw-alias-border-l1,var(--dsw-alias-border-subtle,#e1e3e7))] [--tt-muted:var(--dsw-alias-label-secondary,#71717a)] [--tt-panel:var(--dsw-alias-bg-layer-1,#fff)] [--tt-selected:color-mix(in_srgb,var(--tt-accent)_14%,var(--tt-panel))] [--tt-text:var(--dsw-alias-label-primary,#27272a)] [font:14px/1.45_ui-sans-serif,-apple-system,BlinkMacSystemFont,'Segoe_UI',sans-serif]", overlayClassName: "z-[2147483646]", showCloseButton: false, style: { height: '560px', maxHeight: 'calc(100vh - 2rem)', maxWidth: '860px', width: 'calc(100% - 2rem)' }, children: [_jsx(DialogClose, { asChild: true, children: _jsx(Button, { unstyled: true, "aria-label": "Close", className: "absolute top-2 right-2 z-10 inline-flex size-7 cursor-pointer appearance-none items-center justify-center rounded-md border-0 bg-transparent p-0 text-[var(--tt-muted)] hover:bg-transparent hover:text-[var(--tt-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tt-accent)]", type: "button", children: _jsx(X, { "aria-hidden": "true", className: "size-4" }) }) }), _jsxs("div", { className: "grid min-h-0 sm:h-full sm:grid-cols-[270px_minmax(0,1fr)]", children: [_jsxs("section", { "aria-label": "Vault List", className: "flex min-h-0 flex-col border-b border-[var(--tt-border)] bg-[var(--tockteam-shell-chrome,var(--tt-panel))] p-5 sm:border-r sm:border-b-0", children: [_jsxs(DialogHeader, { className: "sr-only", children: [_jsx(DialogTitle, { children: "Vault Switcher" }), _jsx(DialogDescription, { children: "Open a local Markdown vault or create a new one." })] }), _jsx("p", { className: "mb-2 text-[10px] font-semibold tracking-[.08em] text-[var(--tt-muted)] uppercase", children: "Current Vault" }), _jsxs("div", { className: "flex min-w-0 items-start gap-3 rounded-lg bg-[var(--tt-selected)] p-2", "data-active-vault": "true", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [rename === null
                                                        ...
                                                        : (_jsxs("form", { onSubmit: submitRename, children: [_jsx(Input, { "aria-label": "Vault Name", autoFocus: true, disabled: renaming, maxLength: 80, onChange: event => { setRenameName(event.target.value); }, onKeyDown: event => {
                                                                    ...
                                                                    }, value: renameName }), _jsx("button", { className: "sr-only", type: "submit", children: "Rename Vault" }), renameError && _jsx("p", { className: "mt-1 text-xs text-destructive", role: "alert", children: "The vault could not be renamed." })] })), (props.vault === null || props.vaultDisplayPath !== null) && (_jsx("p", { className: "mt-0.5 break-words text-xs text-[var(--tt-muted)]", title: props.vaultDisplayPath ?? undefined, children: props.vault === null ? 'Open or create a local vault' : props.vaultDisplayPath })), copyError && _jsx("p", { className: "mt-1 text-xs text-destructive", role: "alert", children: "The vault ID could not be copied." })] }), props.vault !== null && rename === null && (_jsxs(DropdownMenu, { onOpenChange: setMenuOpen, open: menuOpen, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx(Button, { unstyled: true, "aria-label": "More Vault Actions", className: "inline-flex size-7 shrink-0 appearance-none items-center justify-center border-0 bg-transparent p-0 text-[var(--tt-muted)] hover:text-[var(--tt-text)] focus-visible:rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tt-accent)]", type: "button", children: _jsx(Ellipsis, { "aria-hidden": "true", className: "size-4" }) }) }), _jsxs(DropdownMenuContent, { unstyled: true, align: "start", alignOffset: 16, className: "max-h-(--radix-dropdown-menu-content-available-height) w-52 origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-lg border border-[var(--tt-border)] bg-[var(--tt-panel)] p-1 text-sm text-[var(--tt-text)] shadow-md", "data-vault-menu-align-offset": "16", portalled: false, children: [_jsxs(DropdownMenuItem, { onSelect: copyVaultId, children: [_jsx(Copy, { "aria-hidden": "true" }), _jsx("span", { children: "Copy Vault ID" })] }), menuOpen && props.renderVaultActions?.('menu', () => { changeOpen(false); }, () => { setMenuOpen(false); }, beginRename, renderVaultMenuItem)] })] }))] })] }), _jsx("section", { "aria-label": "Vault Actions", className: "flex min-h-0 flex-col overflow-y-auto p-8 sm:p-12", children: _jsxs("div", { className: "my-auto w-full", children: [_jsxs("div", { className: "mb-10 flex flex-col items-center text-center", children: [_jsx("span", { className: "mb-4 flex size-16 items-center justify-center rounded-2xl bg-primary/10 text-primary", children: _jsx(TockTeamLogo, {}) }), _jsx("h2", { className: "m-0 text-2xl font-semibold", children: "TockTutor" }), _jsx("p", { className: "mt-1 text-sm text-[var(--tt-muted)]", children: "Your local Markdown notes, kept together." })] }), _jsxs("div", { className: "divide-y divide-[var(--tt-border)] rounded-xl border border-[var(--tt-border)] bg-[color-mix(in_srgb,var(--tt-panel)_82%,var(--tockteam-shell-chrome,var(--tt-panel)))]", children: [_jsx("div", { className: "p-4", "data-vault-action-row": true, children: creating
