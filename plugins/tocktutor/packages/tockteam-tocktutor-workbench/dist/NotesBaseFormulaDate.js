...
function formatTimezoneOffset(date, separator) {
...
function localIsoCalendar(date) {
    ...
    if (!Number.isFinite(weekday))
        return { week: Number.NaN, year: Number.NaN };
...
function localDefaultEnglishCalendar(date) {
    const calendarDate = createUtcCalendarDate(date.getFullYear(), date.getMonth(), date.getDate());
    const weekday = calendarDate.getUTCDay();
    if (!Number.isFinite(weekday))
        return { week: Number.NaN, year: Number.NaN };
...
export function formatNotesTemplateDate(now, format) {
    ...
    return format.replace(/\[([^\]]*)\]|YYYYYY|YYYYY|YYYY|YY|Y|yyyy|yyy|yy|yo|y|NNNNN|NNNN|NNN|NN|N|GGGGG|GGGG|GG|ggggg|gggg|gg|LTS|LT|LLLL|LLL|LL|L|llll|lll|ll|l|MMMM|MMM|Mo|MM|M|DDDo|DDDD|DDD|Do|DD|D|dddd|ddd|dd|do|d|HH|H|kk|k|hh|h|mm|m|ss|s|S{1,9}|A|a|ZZ|Z|zz|z|Qo|Q|e|E|wo|ww|w|Wo|WW|W|X|x/gu, (token, literal) => {
        if (literal !== undefined)
            return literal;
        if (token.startsWith("S"))
            return fractionalSeconds.slice(0, token.length).padEnd(token.length, "0");
        return values.get(token) ?? token;
    });
}
//# sourceMappingURL=NotesBaseFormulaDate.js.map