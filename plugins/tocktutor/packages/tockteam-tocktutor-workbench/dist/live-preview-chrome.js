...
function widgetButton(region, folded) {
...
function staticWidget(kind, content, from, to) {
    ...
    state.doc.descendants((node, pos) => {
        if (node.type.name === 'code_block') {
            ...
            if ((language === 'base' || language === 'mermaid') && !(state.selection.from <= pos + node.nodeSize && state.selection.to >= pos)) {
                values.push(Decoration.node(pos, pos + node.nodeSize, { class: 'hidden' }), Decoration.widget(pos, () => staticWidget(language, node.textContent, pos, pos + node.nodeSize), { side: -1 }));
            }
            return false;
        }
        ...
        for (const match of node.text.matchAll(/\$\$(.{1,20000})\$\$/gu)) {
            ...
            if (state.selection.from <= to && state.selection.to >= from)
                continue;
            values.push(Decoration.inline(from, to, { class: 'hidden' }), Decoration.widget(from, () => staticWidget('math', match[1], from, to), { side: -1 }));
        }
        let cursor = 0;
        if (commentOpen) {
...
export function buildLivePreviewChromePlugin(options) {
    ...
    const toggleTaskControl = (view, event) => {
        ...
        if (task === null || !activatesControl(event))
            return false;
    ...
    const toggleCalloutControl = (view, event) => {
        const callout = event.target instanceof Element ? event.target.closest('[data-callout-fold-pos]') : null;
        if (callout === null || !activatesControl(event))
            return false;
        event.preventDefault();
    ...
    const toggleFoldControl = (view, event) => {
        const fold = event.target instanceof Element ? event.target.closest('[data-fold-from]') : null;
        if (fold === null || !activatesControl(event))
            return false;
        event.preventDefault();
        const from = Number(fold.dataset.foldFrom);
        if (Number.isSafeInteger(from))
            view.dispatch(view.state.tr.setMeta(chromeKey, { toggle: from }));
        return true;
    };
    return new Plugin({
        key: chromeKey,
        state: {
            ...
            apply(transaction, value) {
                ...
                const toggle = transaction.getMeta(chromeKey);
                if (!Number.isSafeInteger(toggle?.toggle))
                    return value;
                const folded = new Set(value.folded);
                if (folded.has(toggle.toggle))
                    folded.delete(toggle.toggle);
                else
                    folded.add(toggle.toggle);
                return { folded };
            },
        },
        props: {
            ...
            handleDOMEvents: {
                ...
                keydown(view, event) {
                    if (toggleTaskControl(view, event) || toggleCalloutControl(view, event) || toggleFoldControl(view, event))
                        return true;
                    if (!options.isProtected())
                        return false;
...
//# sourceMappingURL=live-preview-chrome.js.map