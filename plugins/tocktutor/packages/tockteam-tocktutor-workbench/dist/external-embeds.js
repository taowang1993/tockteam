...
function parsePublicUrl(value) {
    if (typeof value !== 'string' || value.length === 0 || new TextEncoder().encode(value).byteLength > MAX_EXTERNAL_URL_BYTES || value !== value.trim() || /[\u0000-\u001f\u007f]/u.test(value))
        return null;
    try {
        const url = new URL(value);
        if ((url.protocol !== 'http:' && url.protocol !== 'https:') || url.username !== '' || url.password !== '' || url.hostname === '' || isPrivateHostname(url.hostname))
            return null;
        return url;
    }
    catch {
...
export function classifyExternalEmbed(value) {
...
export function externalEmbedButtonHtml(alt, target) {
...
export function externalEmbedInertHtml(alt, target) {
...
export function viewerExternalUrl(value) {
    return classifyExternalEmbed(value)?.viewerUrl ?? null;
}
//# sourceMappingURL=external-embeds.js.map