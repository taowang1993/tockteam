...
function parseNotesBaseRegexpLiteralPrefix(value, allowedFlags = NOTES_BASE_REGEXP_FLAGS, maxCaptureGroups = 0) {
    ...
    for (let index = 1; index < value.length && index <= MAX_NOTES_BASE_REGEXP_PATTERN_LENGTH + 1; index += 1) {
        ...
        if (!inCharacterClass && character === "(") {
            if (captureGroupDepth > 0 || captureGroupCount >= maxCaptureGroups)
                return null;
            captureGroupDepth = 1;
            captureGroupCount += 1;
        ...
        if (!inCharacterClass && character === ")") {
            if (captureGroupDepth !== 1 || index === captureGroupStart + 1)
                return null;
            captureGroupDepth = 0;
            captureGroupStart = -1;
            escapedAtom = false;
        ...
        if (!inCharacterClass && NOTES_BASE_UNSAFE_REGEXP_TOKEN.test(character))
            return null;
        if (/[\n\r\u2028\u2029]/u.test(character))
            return null;
        escapedAtom = false;
    }
    if (closingSlash < 1 || escaped || inCharacterClass)
        return null;
    const source = value.slice(1, closingSlash);
    let flagEnd = closingSlash + 1;
    while (/[A-Za-z]/u.test(value[flagEnd] ?? ""))
        flagEnd += 1;
    const flags = value.slice(closingSlash + 1, flagEnd);
    if (source.length > MAX_NOTES_BASE_REGEXP_PATTERN_LENGTH
...
function tokenizeNotesBaseCaptureReplacement(replacement, captureGroupCount) {
    ...
    for (let index = 0; index < replacement.length; index += 1) {
        ...
        if (captureIndex >= 1 && captureIndex <= captureGroupCount) {
            if (literalStart < index)
                tokens.push(replacement.slice(literalStart, index));
    ...
    if (literalStart < replacement.length)
        tokens.push(replacement.slice(literalStart));
    return tokens;
}
...
export function evaluateNotesBaseRegexpReplaceCall(call, resolveProperty, splitArgs, evaluateArg, maxOutputLength) {
    const argsSource = call.args.trim();
    const parsed = parseNotesBaseRegexpLiteralPrefix(argsSource, NOTES_BASE_REGEXP_FLAGS, MAX_NOTES_BASE_REGEXP_CAPTURE_GROUPS);
    ...
    const replacementTokens = tokenizeNotesBaseCaptureReplacement(replacementText, parsed.captureGroupCount);
    ...
    const projectMatch = (match) => {
        ...
        if (captureExpansionWork > MAX_NOTES_BASE_REGEXP_MATCH_WORK)
            return false;
    ...
    if (parsed.expression.global) {
        ...
        if (match && !projectMatch(match))
            return { supported: false };
    }
    if (projectedLength > maxOutputLength)
        return { supported: false };
...
export function splitNotesBaseRegexpIsTypeCall(value) {
    ...
    const marker = ".isType(";
    if (!parsed || !value.startsWith(marker, parsed.length) || !value.endsWith(")"))
        return null;
    return {
...
export function evaluateNotesBaseRegexpMatchesCall(call, resolveProperty, splitArgs, evaluateArg) {
    const args = splitArgs(call.args);
    if (!args || args.length !== 1 || /,\s*$/u.test(call.args))
        return { supported: false };
    const expression = parseNotesBaseRegexpLiteral(call.receiver, MAX_NOTES_BASE_REGEXP_CAPTURE_GROUPS);
...
//# sourceMappingURL=NotesBaseFormulaRegex.js.map