...
export function applyEditorCommand(source, command, selectionStart, selectionEnd) {
...
function parseTable(source) {
    ...
    if (width < 1 || cells.some(row => row.length !== width))
        return null;
    if (!cells[1]?.every(cell => /^:?-{3,}:?$/u.test(cell)))
        return null;
...
export function applyTableCommand(source, command) {
    ...
    if (command.kind === 'add-row') {
        ...
        if (index < 2 || index >= cells.length)
            return source;
        if (command.kind === 'delete-row')
            cells.splice(index, 1);
        else {
            const destination = index + (command.kind === 'move-row-up' ? -1 : 1);
            if (destination < 2 || destination >= cells.length)
                return source;
            const [row] = cells.splice(index, 1);
            if (row !== undefined)
                cells.splice(destination, 0, row);
        }
    }
    else {
        if (!('column' in command))
            return source;
        if (!Number.isSafeInteger(command.column) || command.column < 0 || command.column >= width)
            return source;
        if (command.kind === 'delete-column') {
            if (width === 1)
                return source;
            for (const row of cells)
                row.splice(command.column, 1);
        }
...
export function resolveSlashCommand(value) {
    return SLASH_COMMANDS.get(value.trim().toLocaleLowerCase()) ?? null;
}
export function resolvePlatformEditorCommand(event, isMac) {
...
export function internalLinkDropMarkdown(path, label) {
...
export function pagePreviewTargetAtOffset(source, offset) {
    if (!Number.isSafeInteger(offset) || offset < 0 || offset > source.length || byteLength(source) > MAX_EDITOR_COMMAND_SOURCE_BYTES)
        return null;
    if (codeRanges(source).some(([start, end]) => offset >= start && offset < end))
        return null;
    for (const match of source.matchAll(/\[\[([^\]|\r\n]{1,2000})(?:\|[^\]\r\n]{0,2000})?\]\]/gu)) {
        if (match.index === undefined || offset < match.index || offset >= match.index + match[0].length)
            continue;
        const [path, fragment] = match[1].split('#', 2);
        if (!isSafeVaultRelativePath(/\.md$/iu.test(path) ? path : `${path}.md`))
            return null;
