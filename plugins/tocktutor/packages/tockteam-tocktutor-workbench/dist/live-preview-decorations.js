...
class Preview extends WidgetType {
    ...
    toDOM(view) {
        ...
        dom.querySelectorAll('input[data-task-index]').forEach((input, index) => {
            if (markers[index] !== undefined)
                input.dataset.liveTaskFrom = String(markers[index]);
        ...
        for (const nested of dom.querySelectorAll('li > ul, li > ol')) {
            ...
            while (item.firstChild && item.firstChild !== nested)
        ...
        if (fold && callout) {
            ...
            details.addEventListener('toggle', () => {
                ...
                if (!view.state.readOnly && view.state.sliceDoc(from, from + 1) !== marker)
                    view.dispatch({ changes: { from, to: from + 1, insert: marker } });
                view.requestMeasure();
    ...
    updateDOM(dom) {
        ...
        dom.dataset.previewFrom = String(this.from);
        dom.dataset.previewTo = String(this.to);
        for (const task of dom.querySelectorAll('[data-live-task-from]'))
            task.dataset.liveTaskFrom = String(Number(task.dataset.liveTaskFrom) + shift);
        return true;
    }
    destroy(dom) {
        const owner = owners.get(dom);
        owner?.disposeImages?.();
        owner?.controls?.abort();
        owners.delete(dom);
    }
    ignoreEvent(event) { return event.target instanceof Element && event.target.closest('audio,video,summary') !== null; }
}
class ImagePreview extends Preview {
    url;
    alt;
    constructor(source, from, to, url, alt) {
...
export function buildLivePreviewExtension(getEmbeds, openUrl) {
    const focus = StateEffect.define();
    ...
    const build = state => {
        ...
        const active = (from, to) => state.field(focused) && state.selection.ranges.some(range => range.from <= to && range.to >= from);
        const hide = (from, to) => { if (to > from)
            values.push(Decoration.replace({}).range(from, to)); };
        const mark = (from, to, className, attributes = {}) => { if (to > from)
            values.push(Decoration.mark({ class: className, attributes }).range(from, to)); };
        const line = (from, className) => values.push(Decoration.line({ class: className }).range(state.doc.lineAt(from).from));
        ...
        tree.iterate({ enter: ref => {
                ...
                const label = ref.node.getChild('LinkLabel'), url = ref.node.getChild('URL');
                if (label && url)
                    references.set(source.slice(label.from + 1, label.to - 1).toLowerCase(), source.slice(url.from, url.to).replace(/^<|>$/gu, ''));
            } });
        tree.iterate({ enter: ref => {
                const { from, to, name, node } = ref;
                if (name === 'Document')
                    return;
                if (covered.some(range => from >= range.from && to <= range.to))
                    return false;
                ...
                if (/^ATXHeading[1-6]$|^SetextHeading[12]$/u.test(name))
                    line(from, `cm-live-heading-${name.slice(-1)}`);
                if (name === 'HeaderMark' && !active(node.parent.from, node.parent.to)) {
                    hide(from, Math.min(to + (source[to] === ' ' ? 1 : 0), node.parent.to));
                    return false;
                }
                ...
                if (['EmphasisMark', 'StrikethroughMark', 'CodeMark'].includes(name) && !active(node.parent.from, node.parent.to))
                    hide(from, to);
                if (name === 'TaskMarker') {
                    values.push(Decoration.replace({ widget: new Task(from, source.slice(from, to) !== '[ ]') }).range(from, to));
                    return false;
                }
                if (name === 'ListMark') {
                    if (/^\s*\[[ xX]\]/u.test(source.slice(to, state.doc.lineAt(to).to)) && !active(node.parent.from, node.parent.to))
                        hide(from, to + 1);
                    else
                        mark(from, to, 'cm-live-list-mark');
                }
                if (name === 'Link' || name === 'Image') {
                    ...
                    if (name === 'Image' && !active(from, to)) {
                        ...
                        if (widget)
                            values.push(Decoration.replace({ block: wholeLine, widget }).range(from, to));
                    }
                    else if (name === 'Link') {
                ...
                if (name === 'Paragraph') {
                    ...
                    for (const match of text.matchAll(/==([^=\n]+)==|%%[\s\S]*?%%/gu)) {
                        ...
                        if (match[1] !== undefined) {
                            ...
                            mark(start, end, 'cm-live-comment');
                        else
                            hide(start, end);
                    }
                    let targets = [];
                    try {
                        targets = collectEmbedTargets(text);
                    }
                    catch { /* Source remains editable when preview budgets are exceeded. */ }
                    let offset = 0;
                    for (const target of targets) {
                        ...
                        if (start < 0 || active(from + start, from + offset) || !embeds.some(embed => embed.target.source === target.source))
                            continue;
    ...
    const activate = (event, view) => {
        ...
        if (event.type === 'mousedown' && control) {
            event.preventDefault();
            return true;
        }
        if (event.type === 'click' && !control)
            return false;
        ...
        if (task) {
            ...
            if (!Number.isSafeInteger(from) || !/^\[[^\]]\]$/u.test(view.state.sliceDoc(from, from + 3)))
                return false;
            event.preventDefault();
        ...
        if (!widget || target?.closest('audio,video,summary'))
            return false;
        const from = Number(widget.dataset.previewFrom), to = Number(widget.dataset.previewTo);
        if (!Number.isSafeInteger(from) || !Number.isSafeInteger(to) || from < 0 || to > view.state.doc.length)
            return false;
        event.preventDefault();
        view.focus();
...
//# sourceMappingURL=live-preview-decorations.js.map