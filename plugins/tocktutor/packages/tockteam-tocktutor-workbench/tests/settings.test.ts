...
test('named workspaces round-trip nested split ratios and independent modes through the existing store', async () => {
  const { splitPaneGroup, resizePaneSplit, setNoteTabMode } = await import('../dist/session.js')
  const storage = new MemoryStorage()
  const initial = openNoteTab(createWorkbenchSession('route', { id: vault, generation: 2 }), 'group-1', 'Note.md', { mode: 'source' })
  const right = splitPaneGroup(initial, 'group-1', 'horizontal')
  const down = splitPaneGroup(right.session, right.groupId, 'vertical')
  const session = setNoteTabMode(resizePaneSplit(down.session, [1], .65), down.groupId, 'Note.md', 'reading')
  const workspaces = createNamedWorkspace([], 'Nested', session, 1, true)
  saveWorkbenchState(storage, vault, { focusMode: true, session, workspaces })
  const restored = loadWorkbenchState(storage, vault)
  assert.deepEqual(restored.session.layout, session.layout)
  assert.deepEqual(restored.workspaces[0]?.session.layout, session.layout)
  assert.equal(restored.workspaces[0]?.session.groups[2]?.tabs[0]?.mode, 'reading')
  assert.equal(restored.workspaces[0]?.session.groups[0]?.tabs[0]?.mode, 'source')
})
