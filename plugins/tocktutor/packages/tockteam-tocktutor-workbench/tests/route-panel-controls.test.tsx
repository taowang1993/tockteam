...
describe('TockTutor titlebar panel controls', () => {
  ...
  it('defaults to formatted editing and toggles reading and source without a Live Preview menu item', async () => {
    expect(DEFAULT_TOCKTUTOR_SETTINGS.defaultEditingMode).toBe('live-preview')
    const onEdit = vi.fn()
    function Harness() {
      const [mode, setMode] = useState<'live-preview' | 'reading' | 'source'>(DEFAULT_TOCKTUTOR_SETTINGS.defaultEditingMode)
      return <TockTutorRouteView snapshot={{ ...snapshot, documentKind: 'markdown', path: 'Lesson.md', phase: 'ready', source: '# Lesson\n\nKeep this draft.', mode, settings: DEFAULT_TOCKTUTOR_SETTINGS }} onMode={setMode} onEdit={onEdit} onActivateTab={() => {}} onAddPane={() => {}} onFocusPane={() => {}} onMoveCanvas={() => {}} onSave={() => {}} onSelect={() => {}} onToggleTask={() => {}} />
    }
    render(<Harness />)
    await screen.findByLabelText('Live Preview Editor')
    const choose = async (name: string, role: 'menuitem' | 'menuitemcheckbox' = 'menuitem') => {
      openNoteActions()
      const menu = await screen.findByRole('menu', { name: 'More Note Actions' })
      expect(within(menu).queryByText('Live Preview', { exact: true })).toBeNull()
      fireEvent.click(within(menu).getByRole(role, { name, exact: true }))
      await waitFor(() => expect(screen.queryByRole('menu')).toBeNull())
    }
    await choose('Source Mode', 'menuitemcheckbox')
    await screen.findByLabelText('Markdown Source')
    openNoteActions()
    expect(screen.getByRole('menuitemcheckbox', { name: 'Source Mode' }).getAttribute('aria-checked')).toBe('true')
    fireEvent.click(screen.getByRole('menuitem', { name: 'Reading View' }))
    await screen.findByLabelText('Reading View')
    expect(document.querySelector('.cm-content[contenteditable="true"]')).toBeNull()
    await choose('Editing View')
    await screen.findByLabelText('Markdown Source')
    await choose('Source Mode', 'menuitemcheckbox')
    await screen.findByLabelText('Live Preview Editor')
    await choose('Reading View')
    await choose('Editing View')
    await screen.findByLabelText('Live Preview Editor')
    fireEvent.click(screen.getByRole('button', { name: 'Switch to Reading View' }))
    await screen.findByLabelText('Reading View')
    fireEvent.click(screen.getByRole('button', { name: 'Switch to Editing View' }))
    await screen.findByLabelText('Live Preview Editor')
    expect(onEdit).not.toHaveBeenCalled()
  })

  it.each(['live-preview', 'source'] as const)('returns a restored Reading View to the saved %s editing preference', async defaultEditingMode => {
    const onMode = vi.fn()
    renderRoute({ documentKind: 'markdown', path: 'Lesson.md', phase: 'ready', mode: 'reading', settings: { ...DEFAULT_TOCKTUTOR_SETTINGS, defaultEditingMode } }, { onMode })
    fireEvent.click(screen.getByRole('button', { name: 'Switch to Editing View' }))
    expect(onMode).toHaveBeenLastCalledWith(defaultEditingMode)
    openNoteActions()
    fireEvent.click(screen.getByRole('menuitem', { name: 'Editing View' }))
    expect(onMode).toHaveBeenLastCalledWith(defaultEditingMode)
  })
  ...
  it('renders editable Live Preview with exact-source task and callout controls', async () => {
    ...
    expect(within(menu).queryByText('Live Preview', { exact: true })).toBeNull()
    await waitFor(() => expect(document.activeElement).toBe(screen.getByRole('menuitem', { name: 'Reading View' })))
    fireEvent.keyDown(document.activeElement!, { key: 'ArrowDown' })
    await waitFor(() => expect(document.activeElement).toBe(screen.getByRole('menuitemcheckbox', { name: 'Source Mode' })))
