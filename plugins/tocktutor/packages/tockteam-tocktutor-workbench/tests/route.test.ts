...
test('loads, edits, reads, toggles, and snapshot-saves one exact note', async () => {
  ...
  assert.match(html, /<header[^>]+aria-label="TockTutor Title Bar"/u)
  ...
  assert.match(html, /padding-top: var\(--tockteam-titlebar-height, 40px\)/u)
  assert.match(html, /\.tocktutor-titlebar\s*\{[^}]*height: var\(--tockteam-titlebar-height, 40px\)/su)
  assert.match(html, /\.tocktutor-titlebar\s*\{[^}]*top: var\(--tockteam-titlebar-height, 40px\)/su)
