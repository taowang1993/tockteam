...
test('loads, edits, reads, toggles, and snapshot-saves one exact note', async () => {
  ...
  assert.match(html, /grid-template-columns:280px minmax\(0, 1fr\)/u)
  assert.match(html, /\.tocktutor-titlebar svg \{[^}]*height: 20px; width: 20px/u)
  assert.match(html, /class="lucide lucide-panel-left"/u)
  assert.match(html, /\.tocktutor-titlebar-sidebar &gt; span:last-child \{ margin-left: auto; \}/u)
  ...
  assert.match(html, /<aside[^>]+aria-label="Files"/u)
  assert.match(html, /<aside[^>]+aria-label="Assistant Panel"[^>]+hidden=""/u)
