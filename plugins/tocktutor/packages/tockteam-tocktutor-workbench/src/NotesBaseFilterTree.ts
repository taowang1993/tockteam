...
export type NotesBaseFilterTree =
  ...
  | { kind: "and"; children: NotesBaseFilterTree[] }
  | { kind: "or"; children: NotesBaseFilterTree[] }
  | { kind: "not"; children: NotesBaseFilterTree[] }
  | { kind: "unsupported"; raw: string };

export type NotesBaseFilterStatementResult =
  | { supported: true; matched: boolean }
  | { supported: false; kind: "filter" | "formula"; expression: string };

type FilterBlockLine = { indent: number; text: string };

function filterBlockLines(lines: string[]): FilterBlockLine[] {
  const block: FilterBlockLine[] = [];
...
function filterBlockText(block: FilterBlockLine[]) {
...
function parseFilterItems(
  block: FilterBlockLine[],
  depth: number,
  budget: { nodes: number },
): NotesBaseFilterTree[] | null {
  if (block.length === 0) return null;
  const itemIndent = block[0]?.indent ?? 0;
  const children: NotesBaseFilterTree[] = [];
  let index = 0;
  while (index < block.length) {
    ...
    if (conjunction) {
      ...
      children.push({ kind: conjunction[1] as "and" | "or" | "not", children: nested });
    } else {
...
function parseFilterBlock(
  ...
  if (conjunction) {
    ...
    return { kind: conjunction[1] as "and" | "or" | "not", children };
  }
...
export function parseNotesBaseFilterBlock(lines: string[]): NotesBaseFilterTree {
 ...
 */
export function evaluateNotesBaseFilterTree(
  tree: NotesBaseFilterTree,
  evaluateStatement: (statement: string) => NotesBaseFilterStatementResult,
): NotesBaseFilterStatementResult {
