...
async function imageData(url: string, signal: AbortSignal): Promise<string> {
...
export function attachInlineImages(root: HTMLElement, onSizeChange: () => void = () => {}): () => void {
  ...
  for (const placeholder of Array.from(root.querySelectorAll<HTMLElement>('[data-external-embed-kind="image"]'))) {
    ...
    const load = () => {
      void imageData(target.sourceUrl, controller.signal).then(src => {
        if (!controller.signal.aborted) image.src = src
      }).catch(failed)
    }
    if (observer) { pending.set(image, load); observer.observe(image) }
    else load()
  }
