import {
  MAX_BASE_BYTES,
  MAX_BASE_FIELDS,
  MAX_BASE_LINE_LENGTH,
  MAX_BASE_LINES,
  MAX_BASE_VIEWS,
} from './base.ts'
import { parseNotesBaseFilterBlock, type NotesBaseFilterTree } from './NotesBaseFilterTree.ts'
...
export type ExecutableBaseParseResult =
  | ExecutableBaseDocument
  | { reason: string; status: 'unsupported' }

function unsupported(reason: string): ExecutableBaseParseResult {
...
function boundedList(values: string[]): boolean {
...
export function parseExecutableBase(source: string): ExecutableBaseParseResult {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (field !== null) {
      ...
      if (key === 'type') {
        ...
        if (!Number.isSafeInteger(limit) || limit > 2_000) return unsupported('Base view limit is invalid.')
        ...
        if (inline !== null) {
          if (!boundedList(inline)) return unsupported('Base view list exceeds its limit.')
          if (currentList === 'summaries') {
            currentView.summaries.push(...inline.map(expression => ({ expression, label: expression })))
          } else if (currentList === 'order') currentView.order.push(...inline)
  ...
  for (const view of views) {
    ...
    if (name === '' || names.has(name)) return unsupported('Base view names must be unique.')
    names.add(name)
  }
  return {
    filters: [...filters],
