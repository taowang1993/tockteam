                  ...
                  <div className="contents" key={property.key}>
                    ...
                    <dd className={`m-0 flex min-h-6 min-w-0 items-center text-[var(--tt-text)] ${tags === null ? 'truncate' : 'flex-wrap gap-1'}`}>
                      {tags === null
                        ...
                        : tags.map((tag, index) => (
                            <span className="inline-flex h-5 items-center gap-1 rounded-full bg-[color-mix(in_srgb,var(--dsw-specific-markdown-accent)_10%,transparent)] px-2 text-[color-mix(in_srgb,var(--dsw-specific-markdown-accent)_85%,var(--tt-text))]" key={tag}>
                              {tag}
