...
import { authoredHistoryKey, buildAuthoredHistory, withoutGeneratedHeadingIds } from './live-preview-authored-history.ts'
...
function replaceAuthoredSourceMatches(
  source: string,
  query: string,
  replacement: string,
  ...
  if (selectedIndexes.length === 0) return source
  const normalizedSource = normalizeSource(source)
  const { body, prefix } = splitLivePreviewSource(source)
  const result = searchEditorMatches(body, query)
  if (result.error !== undefined || result.truncated || result.matches.length !== totalMatches) return null
...
function liveSearchProjection(document: { descendants(callback: (node: { isText: boolean; text?: string }, position: number) => void): void; resolve(position: number): { parent: unknown } }): { positions: number[]; text: string } {
  const positions: number[] = []
  let text = ''
  let previousParent: unknown = null
  document.descendants((node, position) => {
    if (!node.isText || node.text === undefined || node.text === '') return
...
function liveSearchResult(document: { descendants(callback: (node: { isText: boolean; text?: string }, position: number) => void): void; resolve(position: number): { parent: unknown } }, query: string): { error?: string; ranges: { from: number; to: number }[]; truncated: boolean } {
  const checked = searchEditorMatches('', query)
  if (query === '' || checked.error !== undefined) return { ...checked, ranges: [] }
  ...
  return {
    ...
    ranges: result.matches.flatMap(match => {
      ...
      if (from === undefined || end === undefined) return []
...
function liveSearchDecorations(document: { descendants(callback: (node: { isText: boolean; text?: string }, position: number) => void): void; resolve(position: number): { parent: unknown } }, query: string, current: number | null): { decorations: DecorationSet; total: number; truncated?: boolean } {
  const result = liveSearchResult(document, query)
  return {
    decorations: DecorationSet.create(document as never, result.ranges.map((range, index) => Decoration.inline(range.from, range.to, { class: `tocktutor-find-match${index === current ? ' tocktutor-find-current' : ''}` }))),
...
function LivePreviewEditorInner(props: LivePreviewEditorProps): ReactNode {
  ...
  const publishSearch = (view: EditorView, error?: string): EditorSearchState => {
    ...
    const search = $prose(() => new Plugin({
      ...
      state: {
        ...
        apply: (transaction, value, _oldState, state) => {
          ...
          if (refresh || transaction.docChanged) return liveSearchDecorations(state.doc, searchQueryRef.current, searchCurrentIndexRef.current)
  ...
  useEffect(() => {
    if (loading || props.content === lastContentRef.current) return
    lastContentRef.current = props.content
    const instance = editor.get()
    const view = onEditorViewRef.current
    if (!instance || view === null || props.content === authoredHistoryKey.getState(view.state)) return
    ...
    try {
      ...
      if (view.state.doc.eq(doc)) {
        ...
        const selection = TextSelection.between(
          doc.resolve(Math.min(view.state.selection.anchor, doc.content.size)),
          doc.resolve(Math.min(view.state.selection.head, doc.content.size)),
        )
  ...
  useEffect(() => {
    ...
    for (const range of [...selectedRanges].reverse()) transaction = transaction.insertText(replacement, range.from, range.to)
    ...
    if (request.action === 'replace') view.focus()
    publishSearch(view)
