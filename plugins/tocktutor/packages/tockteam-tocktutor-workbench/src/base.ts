...
export type BaseProjection =
  | { status: 'ready'; views: BaseViewProjection[]; warnings: string[] }
  | { status: 'unsupported'; reason: string }

function isUnsafeYamlLine(line: string): boolean {
...
function unsupported(reason: string): BaseProjection {
  return { status: 'unsupported', reason }
}

export function projectBase(content: string): BaseProjection {
  ...
  let inViews = false
  let sawViews = false
  let current: BaseViewProjection | null = null
  const views: BaseViewProjection[] = []
...
function assignField(view: BaseViewProjection, key: string, rawValue: string): void {
