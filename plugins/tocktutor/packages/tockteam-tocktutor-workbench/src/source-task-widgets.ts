...
interface TaskMarker { checked: boolean; from: number; to: number }

function taskMarkers(source: string): TaskMarker[] {
  ...
  for (const line of lines) {
    ...
    if (fence === null) {
      ...
      if (commentOpen) {
        ...
        if (close < 0) { offset += line.length; continue }
      ...
      if (open >= 0) {
        ...
        if (close < 0) {
          ...
          commentOpen = true
        } else visible = `${visible.slice(0, open)}${visible.slice(close + 2)}`
      }
...
class TaskWidget extends WidgetType {
  constructor(readonly marker: TaskMarker) { super() }
  eq(other: TaskWidget): boolean { return this.marker.from === other.marker.from && this.marker.checked === other.marker.checked }
...
function taskDecorations(view: EditorView) {
...
export function buildSourceTaskWidgetExtension() {
  const plugin = ViewPlugin.fromClass(class {
    decorations
    constructor(view: EditorView) { this.decorations = taskDecorations(view) }
    update(update) { if (update.docChanged) this.decorations = taskDecorations(update.view) }
  }, {
    decorations: value => value.decorations,
    provide: value => EditorView.atomicRanges.of(view => view.plugin(value)?.decorations ?? Decoration.none),
  })
  const toggle = (event, view): boolean => {
    const input = event.target instanceof Element ? event.target.closest<HTMLInputElement>('[data-task-from]') : null
    if (input === null || view.state.readOnly) return false
    if (event.type === 'keydown' && event.key !== 'Enter' && event.key !== ' ') return false
    event.preventDefault()
    const from = Number(input.dataset.taskFrom)
    if (!Number.isSafeInteger(from) || from < 0 || from + 3 > view.state.doc.length) return true
