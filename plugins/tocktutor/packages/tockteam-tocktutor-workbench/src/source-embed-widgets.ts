...
function buildDecorations(state, embeds: readonly ResolvedEmbedNode[]) {
  ...
  for (const target of projectEditorWidgets(state.doc.toString())) {
    if (state.selection.ranges.some(range => range.from <= target.to && range.to >= target.from)) continue
