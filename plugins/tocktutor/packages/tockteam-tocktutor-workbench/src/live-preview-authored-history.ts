...
export class AuthoredSourceStep extends Step {
  constructor(readonly from: number, readonly removed: string, readonly inserted: string) { super() }
  static between(before: string, after: string): AuthoredSourceStep {
    ...
    while (endBefore > from && endAfter > from && before[endBefore - 1] === after[endAfter - 1]) { endBefore -= 1; endAfter -= 1 }
  ...
  update(source: string): string { return source.slice(0, this.from) + this.inserted + source.slice(this.from + this.removed.length) }
}

export function buildAuthoredHistory(initialSource: () => string, serialize: (doc, authored: string) => string) {
  return new Plugin({
    ...
    filterTransaction(transaction, state) {
      ...
      if (transaction.docChanged && !isHistoryTransaction(transaction)
        ...
        && sameAuthoredDocument(state.doc, transaction.doc)) transaction.setMeta('addToHistory', false)
      return true
    },
    state: {
      init: initialSource,
      apply(transaction, source) {
        for (const step of transaction.steps) if (step instanceof AuthoredSourceStep) source = step.update(source)
        return source
      },
    },
    appendTransaction(transactions, oldState, state) {
      ...
      if (!transactions.some(transaction => transaction.docChanged)
        || transactions.some(transaction => transaction.steps.some(step => step instanceof AuthoredSourceStep))) return null
      // No serializer publication for DOM-only IDs (including after undo).
      if (sameAuthoredDocument(oldState.doc, state.doc)) return null
