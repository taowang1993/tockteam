...
export type LivePreviewProjection =
  | { status: 'ready'; lines: readonly LivePreviewLine[] }
...
function fenceMarker(content: string): { character: '`' | '~'; length: number } | null {
...
function heading(content: string): number | null {
  const match = content.match(/^ {0,3}(#{1,6})(?:\s+|$)/u)
  return match?.[1]?.length ?? null
}

function listIndent(content: string): number | null {
  const match = content.match(/^(\s*)(?:[-+*]|\d{1,9}[.)])\s+/u)
  return match === null ? null : match[1]!.replaceAll('\t', '    ').length
}

export function projectLivePreview(source: string): LivePreviewProjection {
  ...
  const projected: LivePreviewLine[] = []
  ...
  for (const line of raw) {
    ...
    if (openFence !== null) {
      ...
      if (close !== undefined && close[0] === openFence.character && close.length >= openFence.length) openFence = null
      continue
    }
...
export function replaceLivePreviewLine(source: string, index: number, replacement: string): string {
  if (!Number.isSafeInteger(index) || index < 0 || /[\r\n]/u.test(replacement)) return source
  if (byteLength(replacement) > MAX_LIVE_PREVIEW_LINE_BYTES) return source
