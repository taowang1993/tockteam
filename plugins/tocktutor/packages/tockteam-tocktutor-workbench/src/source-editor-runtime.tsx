  ...
  parseInline: [
    {
      name: 'Highlight',
      parse(cx, char, pos) {
        if (char !== 61 || cx.char(pos + 1) !== 61 || cx.char(pos - 1) === 61 || cx.char(pos + 2) === 61) return -1
        return cx.addDelimiter(highlightDelimiter, pos, pos + 2,
          !/\s/u.test(cx.slice(pos + 2, pos + 3)), !/\s/u.test(cx.slice(pos - 1, pos)))
      },
    },
    {
      name: 'WikiLink', before: 'Link',
      parse(cx, char, pos) {
        if (char !== 91 || cx.char(pos + 1) !== 91) return -1
...
function applySelectionRequest(view: EditorView, request: SourceEditorProps['selectionRequest']): boolean {
  if (request === null || request === undefined || !Number.isSafeInteger(request.id) || request.id < 0) return false
...
function sourceDecorations(state: EditorState) {
  ...
  for (let number = 1; number <= state.doc.lines; number += 1) {
    ...
    if (heading !== null) {
      ...
      decorations.push(
        Decoration.line({ class: `cm-tock-heading-line cm-tock-heading-${heading[1].length}` }).range(line.from),
    ...
    if (commentOpen) {
      ...
      if (close < 0) {
        if (line.from < line.to) decorations.push(Decoration.mark({ class: 'cm-tock-comment' }).range(line.from, line.to))
      } else {
...
export function SourceEditorRuntime(props: SourceEditorProps): ReactNode {
  ...
  useEffect(() => {
    ...
    if (request.action !== 'replace-all') view.focus()
    publishSearch(view, query, current)
  }, [props.searchRequest])
  ...
  useEffect(() => {
    ...
    if (!view || request === null || request === undefined || !Number.isSafeInteger(request.id) || request.id < 0) return
    const latestId = lastSelectionRequestIdRef.current
    if ((latestId !== null && request.id < latestId) || (request.id === latestId && appliedSelectionViewRef.current === view)) return
    lastSelectionRequestIdRef.current = request.id
    if (applySelectionRequest(view, request)) appliedSelectionViewRef.current = view
