...
export function collectEmbedTargets(source: string): EmbedTarget[] {
  ...
  for (const line of lines) {
    ...
    for (const match of line.matchAll(/!\[\[([^\]\r\n]{1,4096})\]\]/gu)) {
      if (match.index === undefined || code.some(([start, end]) => match.index! >= start && match.index! < end)) continue
      let slashes = 0
      for (let index = match.index - 1; index >= 0 && line[index] === '\\'; index -= 1) slashes += 1
...
export function resolveEmbedTargetPath(entries: readonly { path: string }[], targetPath: string): string | null {
