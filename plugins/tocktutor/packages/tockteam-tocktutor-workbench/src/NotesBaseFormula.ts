import { formatNotesTemplateDate } from './NotesBaseFormulaDate.ts'
import {
  evaluateNotesBaseBooleanAnd,
  evaluateNotesBaseBooleanNot,
  evaluateNotesBaseBooleanOr,
  evaluateNotesBaseComparison,
  evaluateNotesBaseUnaryNumeric,
  isNotesBaseDateOffsetExpression,
  isNotesBaseDurationScaleExpression,
  evaluateNotesBaseNumberTransform,
  evaluateNotesBaseMultiplicative,
  evaluateNotesBaseAdditive,
} from './NotesBaseFormulaArithmetic.ts'
import {
  notesBaseObjectHasProperty,
  notesBaseObjectKeys,
  notesBaseObjectSnapshot,
  notesBaseObjectValue,
  notesBaseObjectValues,
} from './NotesBaseFormulaObject.ts'
import {
  evaluateNotesBaseFileHasLinkCall,
  evaluateNotesBaseFileInFolderCall,
  evaluateNotesBaseLinkLinksToCall,
  normalizeNotesBaseFilePath,
  normalizeNotesBaseLinkPath,
  notesBaseFileLinksSnapshot,
  notesBaseFilePathField,
} from './NotesBaseFormulaPath.ts'
import {
  createNotesBaseLinkValue,
  notesBaseLinkPath,
  notesBaseLinkText,
} from './NotesBaseFormulaLink.ts'
import { createNotesBaseIconValue, notesBaseIconName } from './NotesBaseFormulaIcon.ts'
import { createNotesBaseImageValue, notesBaseImageText } from './NotesBaseFormulaImage.ts'
import { createNotesBaseHtmlValue, notesBaseHtmlText } from './NotesBaseFormulaHtml.ts'
import {
  evaluateNotesBaseRegexpAnyValueCall,
  evaluateNotesBaseRegexpMatchesCall,
  evaluateNotesBaseRegexpReplaceCall,
  evaluateNotesBaseRegexpSplitCall,
  splitNotesBaseRegexpIsTypeCall,
} from './NotesBaseFormulaRegex.ts'
import {
  evaluateNotesBaseFileHasTagCall,
  notesBaseTagsSnapshot,
} from './NotesBaseFormulaTags.ts'
import { countNotesBaseUniqueValues } from './NotesBaseSummaryUnique.ts'
import { isNotesBaseFormulaQuoteEscaped } from './NotesBaseFormulaSyntax.ts'

export type NotesBaseFormulaResult = { supported: true; value: unknown } | { supported: false };
...
const NOTES_BASE_HTML_ESCAPE_BY_CHARACTER = new Map<string, string>(Object.entries({
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  "\"": "&quot;",
...
const NOTES_BASE_FORMULA_LIST_COLLATOR = new Intl.Collator(
  undefined,
  { usage: "sort", sensitivity: "base", numeric: true },
);

export function notesBaseFileTimestamp(timestamp: unknown) {
  if (typeof timestamp !== "number" || !Number.isFinite(timestamp)) return undefined;
  const date = new Date(timestamp);
  return Number.isFinite(date.getTime()) ? date.toISOString() : undefined;
}

export function notesBaseFormulaExpression(value: string) {
  return /^formula\((['"])([\s\S]*)\1\)$/u.exec(value.trim())?.[2] ?? null;
}

function evaluateNotesBaseValueEmptiness(value: unknown): NotesBaseFormulaResult {
...
function splitFormulaArgs(value: string) {
  const args: string[] = [];
  let current = "";
  let quote = "";
  let depth = 0;
...
function splitMemberCall(value: string, method: string) {
  const marker = `.${method}(`;
  let quote = "";
  let depth = 0;
  let markerIndex = -1;
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (depth === 0 && char === ".") {
      ...
      if (memberCall) lastMemberCallIndex = index;
      if (value.startsWith(marker, index)) markerIndex = index;
    }
    if (char === "(") depth += 1;
    if (char === ")") depth -= 1;
    if (depth < 0) return null;
...
function unwrapBoundedFormulaGroup(value: string): string | null {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (char === "(") {
      ...
      depth -= 1;
      if (depth < 0) return null;
      if (depth === 0 && outerClose < 0) outerClose = index;
    }
  }
  if (quote !== "" || depth !== 0) return null;
...
function unwrapNotesBaseOperand(value: string) {
...
function isNotesBaseDateOperand(value: string) {
...
function isNotesBaseDateOffsetOperand(value: string) {
  const current = unwrapNotesBaseOperand(value);
  return current !== null && isNotesBaseDateOffsetExpression(current, isNotesBaseDateOperand);
}

function isDirectNotesBaseDurationOperand(value: string) {
  const current = unwrapNotesBaseOperand(value);
...
function isNotesBaseDurationOperand(value: string) {
  const current = unwrapNotesBaseOperand(value);
  return current !== null && (
    isDirectNotesBaseDurationOperand(current)
    || isNotesBaseDurationScaleExpression(current, isDirectNotesBaseDurationOperand)
  );
}

function rejectNotesBaseUnaryNumericOperand(operator: "+" | "-", operand: string) {
  return operator === "-" && isNotesBaseDurationOperand(operand);
}

function containsMemberCall(value: string, methods: string[]) {
  let quote = "";
  for (let index = 0; index < value.length; index += 1) {
    const char = value[index] ?? "";
    if ((char === "\"" || char === "'") && !isNotesBaseFormulaQuoteEscaped(value, index)) {
    ...
    if (methods.some((method) => value.startsWith(`.${method}(`, index))) return true;
  }
  return false;
}

function isNotesBaseSafeShortCircuitOperand(value: string) {
...
function escapeNotesBaseHtml(value: string) {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (escaped !== undefined) projectedLength += escaped.length - 1;
...
function notesBaseNumberValue(value: unknown, operand: string) {
...
function localClockTime(now: Date) {
...
function parseTimezoneOffsetDate(value: string) {
...
function parseFixedDuration(value: string) {
  if (value.length > MAX_NOTES_BASE_FORMULA_STRING_LENGTH) return null;
  const match = /^([+-]?(?:\d+(?:\.\d+)?|\.\d+))\s*(ms|s|seconds?|m|minutes?|h|hours?|d|days?|w|weeks?)$/u.exec(value.trim());
  if (!match) return null;
  const magnitude = Number(match[1]);
  const unit = match[2] as keyof typeof NOTES_BASE_FIXED_DURATION_MULTIPLIERS;
  const milliseconds = magnitude * NOTES_BASE_FIXED_DURATION_MULTIPLIERS[unit];
  return Number.isSafeInteger(milliseconds) ? milliseconds : null;
}

function parseFixedDateDuration(value: unknown, operand: string) {
  if (typeof value === "number") {
...
function offsetLocalCalendarMonths(timestamp: number, months: number) {
...
const NOTES_BASE_DATE_FIELD_ACCESSORS = {
  ...
  millisecond: (date: Date) => date.getMilliseconds(),
} as const;

function notesBaseDateValue(value: unknown) {
...
function notesBaseDateTimestamp(value: unknown, operand: string) {
  return isNotesBaseDateOperand(operand) || isNotesBaseDateOffsetOperand(operand)
    ? notesBaseDateValue(value)?.getTime() ?? null
    : null;
}

function notesBaseFileOperandPath(value: unknown, operand: string) {
...
function isNotesBaseFileOperand(value: unknown, operand: string) {
  return notesBaseFileOperandPath(value, operand) !== null;
}

function notesBaseRelativeTime(date: Date, now = new Date()) {
...
function isNotesBaseValueType(value: unknown, expectedType: string, receiverSource: string) {
...
function evaluateNotesBaseIsTypeCall(
...
function stringifyNotesBaseValue(value: unknown, listElement = false): string | undefined {
...
function notesBaseQuotedNotePropertyReference(value: string) {
...
function notesBasePropertyReference(value: string) {
  return (
    /^[A-Za-z_][\w-]*$/u.test(value)
    || /^(?:file|formula|note)\.[A-Za-z_][\w-]*$/u.test(value)
  ) ? value : notesBaseQuotedNotePropertyReference(value);
}

function evaluateNotesBaseThisFile(
  expression: string,
...
function splitNotesBaseQuotedBracketMember(expression: string) {
...
function isNotesBaseFilePropertiesReceiver(expression: string) {
  return expression === "this.file.properties"
    || /^file\([\s\S]*\)\.properties$/u.test(expression);
}

function splitNotesBaseObjectMemberAccess(expression: string) {
  ...
  const receiverIsSupported = (
    notesBasePropertyReference(receiver) !== null
...
function evaluateNotesBaseObjectMemberAccess(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult | null {
...
function evaluateNumericExtremum(
  args: string[],
  source: string,
  resolveProperty: (property: string) => unknown,
  initialValue: number,
...
function evaluateStringQuery(
  call: { receiver: string; args: string },
  resolveProperty: (property: string) => unknown,
  matches: (source: string, query: string) => boolean,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult {
  const args = splitFormulaArgs(call.args);
...
function isNotesBaseScalar(value: unknown) {
  return value == null
    || typeof value === "string"
...
function isBoundedNotesBaseScalar(value: unknown) {
  return isNotesBaseScalar(value)
    && (typeof value !== "string" || value.length <= MAX_NOTES_BASE_FORMULA_STRING_LENGTH);
}

type NotesBaseCompositeLiteralBudget = {
  valueCount: number;
  stringLength: number;
};

function evaluateNotesBaseCompositeLiteralValue(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context: NotesBaseFormulaContext | undefined,
  depth: number,
  budget: NotesBaseCompositeLiteralBudget,
): NotesBaseFormulaResult {
...
function evaluateNotesBaseListIndex(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult | null {
...
function evaluateNotesBaseListLiteral(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
  depth = 1,
  budget: NotesBaseCompositeLiteralBudget = { valueCount: 0, stringLength: 0 },
): NotesBaseFormulaResult | null {
...
function splitNotesBaseObjectEntry(source: string) {
  let quote = "";
  let depth = 0;
...
function evaluateNotesBaseObjectLiteral(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
  depth = 1,
...
function evaluateListSort(
  ...
  return {
    supported: true,
    value: [...receiver.value].sort((left, right) => (
...
function evaluateListFlat(
  ...
  const pending: Array<
    ...
    | { type: "leave"; value: unknown[] }
  > = [{ type: "leave", value: receiver.value }];
  for (let index = receiver.value.length - 1; index >= 0; index -= 1) {
    pending.push({ type: "value", value: receiver.value[index] });
  }
  const activeArrays = new WeakSet<object>([receiver.value]);
...
function evaluateListCallback(
  ...
  if (
    expression.length > MAX_NOTES_BASE_FORMULA_STRING_LENGTH
  ...
  for (let index = 0; index < receiver.value.length; index += 1) {
    ...
    const result = evaluateArg(expression, (property) => {
      ...
      if (property === "index") return index;
...
function evaluateContains(
  call: { receiver: string; args: string },
  resolveProperty: (property: string) => unknown,
  arity: "one" | "one-or-more" = "one",
  quantifier: "all" | "any" = "all",
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult {
  const args = splitFormulaArgs(call.args);
  if (
  ...
  const scalarValues = new Set<unknown>();
...
function evaluateNumberToFixed(
  call: { receiver: string; args: string },
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult {
...
function titleCaseNotesBaseString(source: string) {
...
function evaluateStringReplace(
  call: { receiver: string; args: string },
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult {
  const evaluateNestedArg = (arg: string, resolver: (property: string) => unknown) => (
    evaluateArg(arg, resolver, context)
  );
  ...
  if (
    !receiver.supported || typeof receiver.value !== "string"
...
export function evaluateNotesBaseFormula(
  expression: string,
  resolveProperty: (property: string) => unknown,
  context?: NotesBaseFormulaContext,
): NotesBaseFormulaResult {
  ...
  if (dateField) {
    ...
    if (!date) return { supported: false };
    const field = dateField[2] as keyof typeof NOTES_BASE_DATE_FIELD_ACCESSORS;
  ...
  if (formatCall) {
    ...
    if (!args || args.length !== 1 || /,\s*$/u.test(formatCall.args)) return { supported: false };
  ...
  if (asLinkCall) {
    ...
    let activeFileProperty: "file.path" | "file.file" | null = null;
    ...
    const projectedFile = activeFileProperty
      ? { supported: true as const, value: resolveProperty(activeFileProperty) }
  ...
  if (hasPropertyCall) {
    ...
    if (!name.supported || typeof name.value !== "string") return { supported: false };
  ...
  if (name === "min" || name === "max") {
    return evaluateNumericExtremum(
      ...
      call[2] ?? "",
      resolveProperty,
      name === "min" ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY,
      name === "min" ? Math.min : Math.max,
      context,
    );
  }
  ...
  if (name === "upper" && resolved.length === 1) return { supported: true, value: text(resolved[0]).toUpperCase() };
  if (name === "lower" && resolved.length === 1) return { supported: true, value: text(resolved[0]).toLowerCase() };
  ...
  if (name === "random" && resolved.length === 0) return { supported: true, value: Math.random() };
  if (name === "today" && resolved.length === 0) return { supported: true, value: localCalendarDate() };
  if (name === "now" && resolved.length === 0) return { supported: true, value: new Date().toISOString() };
...
export function evaluateNotesBaseSummary<Row>(expression: string, rows: Row[], resolveProperty: (row: Row, property: string) => unknown): NotesBaseFormulaResult {
  const trimmed = expression.trim();
  ...
  if (call[1] === "empty" || call[1] === "filled") {
    ...
    for (const row of rows) {
      ...
      if (!result.supported) return { supported: false };
      if (result.value === expectedEmpty) count += 1;
    }
  ...
  if (call[1] === "checked" || call[1] === "unchecked") {
    ...
    let count = 0;
    for (const row of rows) {
      if (resolveProperty(row, call[2] ?? "") === expected) count += 1;
    }
    return { supported: true, value: count };
  }
  if (call[1] === "range") {
  ...
  if (call[1] === "earliest" || call[1] === "latest") {
    ...
    for (const row of rows) {
      ...
      if (date) timestamp = findEarliest ? Math.min(timestamp, date.getTime()) : Math.max(timestamp, date.getTime());
    }
