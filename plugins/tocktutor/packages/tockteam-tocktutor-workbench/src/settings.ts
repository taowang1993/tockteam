...
const DEFAULT_SETTINGS: TockTutorSettings = Object.freeze({
...
function settingsKey(vaultId: string): string {
...
function normalizeSettings(value: unknown): TockTutorSettings {
...
function writeJson(storage: KeyValueStorage, key: string, value: unknown): boolean {
  try {
...
export function loadTockTutorSettings(storage: KeyValueStorage, vaultId: string): TockTutorSettings {
  if (!validVaultId(vaultId)) return { ...DEFAULT_SETTINGS }
  return normalizeSettings(readJson(storage, settingsKey(vaultId)))
}

export function saveTockTutorSettings(
  storage: KeyValueStorage,
  vaultId: string,
  change: Partial<TockTutorSettings>,
): TockTutorSettings {
  if (!validVaultId(vaultId)) return { ...DEFAULT_SETTINGS }
...
function normalizeWorkspace(value: unknown, vaultId: string): NamedWorkspace | null {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return null
  const record = value as Record<string, unknown>
  if (typeof record.id !== 'string' || !/^[a-z0-9](?:[a-z0-9-]{0,63})$/u.test(record.id)) return null
  if (typeof record.name !== 'string' || record.name.trim().length === 0 || record.name.length > 100) return null
...
export function createNamedWorkspace(
  current: readonly NamedWorkspace[],
  name: string,
  session: WorkbenchSession,
  createdAt = Date.now(),
  focusMode = false,
  ...
  let id = base
  for (let index = 2; used.has(id) && index <= MAX_TOCKTUTOR_WORKSPACES + 1; index += 1) id = `${base.slice(0, 60)}-${String(index)}`
...
export function loadWorkbenchState(storage: KeyValueStorage, vaultId: string): PersistedWorkbenchState {
...
function safeSnippetId(value: string): string {
  return value.toLocaleLowerCase().replace(/[^a-z0-9_-]+/gu, '-').replace(/^-|-$/gu, '').slice(0, 64) || 'snippet'
}

export function compileTockTutorCssSnippet(id: string, source: string): string | null {
  ...
  while (cursor < stripped.length) {
    ...
    if (selectors.some(value => value === '' || value.length > 1_000)) return null
    output.push(`${selectors.map(value => `.tocktutor-editor-scope ${value}`).join(', ')} { ${body} }`)
    rules += 1
    if (rules > 1_000) return null
    cursor = close + 1
