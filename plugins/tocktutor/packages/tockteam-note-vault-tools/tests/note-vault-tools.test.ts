...
test('notes_search adapts the active generation into TockDriver citations without exposing source details', async () => {
  ...
  try {
    ...
    assert.deepEqual(loaded.noteVault.searches.at(-1), {
      args: { query: 'launch', mode: 'related', limit: 4, [MARKDOWN_ONLY_SEARCH]: true },
      expectedVault: { generation: 7, id: 'vault:test' },
      signal,
    })
...
test('notes_search trims padded queries before runtime normalization', async () => {
  const loaded = await load()
  try {
    loaded.noteVault.notesSearchResult = {
      query: 'launch',
      matches: [{ path: 'Notes/Launch.md', kind: 'content', line: 3, preview: 'launch plan' }],
      truncated: false,
      cursor: null,
      scan: { bytes: 32, entries: 1, files: 1 },
      truncationReason: null,
      warnings: [],
      generation: 7,
    }
    ...
    assert.ok(tool)
    const signal = new AbortController().signal
    const result = await tool.execute({ query: '  launch  ' }, execution(signal))
    assert.deepEqual(result, {
      vaultId: 'vault:test',
      query: 'launch',
      citations: [{
    ...
    assert.deepEqual(loaded.noteVault.searches.at(-1), {
      args: { query: 'launch', mode: 'literal', limit: 10, [MARKDOWN_ONLY_SEARCH]: true },
      expectedVault: { generation: 7, id: 'vault:test' },
      signal,
    })
  } finally {
