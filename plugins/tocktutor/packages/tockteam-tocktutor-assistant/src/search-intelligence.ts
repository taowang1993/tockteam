...
function assertQuickAnswerCandidates(candidates: readonly AssistantQuickAnswerCandidate[]): void {
  if (candidates.length > MAX_ANSWER_CANDIDATES) throw new TypeError('Quick Answer candidates exceed their bound.')
...
function parseQuickAnswer(value: string, candidates: ReadonlyMap<string, AssistantQuickAnswerCandidate>): { answer: string; citations: AssistantQuickAnswerCitation[] } {
  if (value.length > MAX_ANSWER_CHARS) throw new TypeError('Quick Answer output is too large.')
  const parsed: unknown = JSON.parse(value)
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) throw new TypeError('Quick Answer output must be an object.')
  const record = parsed as Record<string, unknown>
  if (Object.keys(record).some(key => key !== 'answer' && key !== 'citations') || typeof record.answer !== 'string' || !Array.isArray(record.citations) || record.citations.length > MAX_ANSWER_CITATIONS) throw new TypeError('Quick Answer output is invalid.')
  const seen = new Set<string>()
  const citations = record.citations.map(id => {
    if (typeof id !== 'string' || !candidates.has(id) || seen.has(id)) throw new TypeError('Quick Answer citation is unknown or duplicated.')
    seen.add(id)
  ...
  if (answer.length > MAX_ANSWER_CHARS) throw new TypeError('Quick Answer output is too large.')
...
export async function answerSearchQuery(
  ...
  if (llm === undefined) return { status: 'provider-unavailable', answer: '', citations: [] }
  if (signal.aborted) return { status: 'cancelled', answer: '', citations: [] }
  const query = redactBoundaryText(request.query.trim())
  if (query.length === 0 || query.length > MAX_QUERY_CHARS) return { status: 'invalid-output', answer: '', citations: [] }
