...
const ERROR_MESSAGES: Record<ProposalErrorCode, string> = {
...
function fail(code: ProposalErrorCode): never {
  throw new ProposalError(code)
}

function assertPlainRecord(value: unknown): asserts value is Record<string, unknown> {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) fail('CORRUPT_QUEUE')
  const prototype = Object.getPrototypeOf(value) as unknown
  if (prototype !== Object.prototype && prototype !== null) fail('CORRUPT_QUEUE')
}

function assertKeys(value: Record<string, unknown>, allowed: readonly string[], code: ProposalErrorCode): void {
  const keys = new Set(allowed)
  if (Object.keys(value).some(key => !keys.has(key))) fail(code)
}

function assertOpaqueId(value: unknown, code: ProposalErrorCode = 'INVALID_PROPOSAL'): asserts value is string {
  if (typeof value !== 'string' || !OPAQUE_ID_PATTERN.test(value)) fail(code)
}

function assertEpoch(value: unknown, code: ProposalErrorCode = 'INVALID_PROPOSAL'): asserts value is number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) fail(code)
}

function assertRoute(value: unknown, maximum: number, code: ProposalErrorCode = 'INVALID_PROPOSAL'): asserts value is string {
  if (typeof value !== 'string' || value.length > maximum || !ROUTE_PATTERN.test(value)) fail(code)
}

function assertGeneration(value: unknown, code: ProposalErrorCode = 'INVALID_PROPOSAL'): asserts value is number {
  if (!Number.isSafeInteger(value) || (value as number) < 1) fail(code)
}

function assertTimestamp(value: unknown, code: ProposalErrorCode = 'CORRUPT_QUEUE'): asserts value is number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) fail(code)
}

function assertDigest(value: unknown, code: ProposalErrorCode = 'INVALID_PROPOSAL'): asserts value is string {
  if (typeof value !== 'string' || !DIGEST_PATTERN.test(value)) fail(code)
}

function safePath(value: unknown, code: ProposalErrorCode): asserts value is string {
...
function expectedTarget(value: unknown, operation: ProposalOperation, code: ProposalErrorCode): ExpectedTarget {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) fail(code)
  const target = value as Record<string, unknown>
  assertKeys(target, ['exists', 'identity', 'modifiedAt'], code)
  if (typeof target.exists !== 'boolean') fail(code)
  if (target.identity !== undefined) assertOpaqueId(target.identity, code)
  if (target.modifiedAt !== undefined) assertTimestamp(target.modifiedAt, code)
  if (operation === 'create' && (target.exists || target.identity !== undefined || target.modifiedAt !== undefined)) fail(code)
  if (operation === 'update' && (!target.exists || target.identity === undefined)) fail(code)
  return {
    exists: target.exists,
    ...target.identity === undefined ? {} : { identity: target.identity },
    ...target.modifiedAt === undefined ? {} : { modifiedAt: target.modifiedAt as number },
  }
}

function proposalSource(value: unknown, code: ProposalErrorCode): ProposalSource | undefined {
...
function warning(entry: unknown, code: ProposalErrorCode): string {
  if (typeof entry !== 'string' || entry.length > MAX_WARNING_CHARS) fail(code)
  const redacted = boundToolText(entry, MAX_WARNING_CHARS)
  if (code === 'CORRUPT_QUEUE' && redacted !== entry) fail(code)
  return redacted
}

function recordSummary(record: ConsumedProposal): ProposalSummary {
  return {
    version: VERSION,
...
function parseRecord(value: unknown): ConsumedProposal {
  ...
  if (value.version !== VERSION) fail('CORRUPT_QUEUE')
  ...
  if (value.operation !== 'create' && value.operation !== 'update') fail('CORRUPT_QUEUE')
  ...
  if (typeof value.content !== 'string' || value.content.includes('\0')) fail('CORRUPT_QUEUE')
  const bytes = Buffer.byteLength(value.content, 'utf8')
  if (value.content.length > MAX_CONTENT_CHARS || bytes > MAX_CONTENT_BYTES) fail('CORRUPT_QUEUE')
  assertDigest(value.contentDigest, 'CORRUPT_QUEUE')
  if (sha256(value.content) !== value.contentDigest) fail('DIGEST_MISMATCH')
  if (value.contentBytes !== bytes || value.contentChars !== value.content.length) fail('DIGEST_MISMATCH')
  ...
  if (value.writePermission !== 'propose') fail('CORRUPT_QUEUE')
...
function approvalMismatch(
  ...
  now: number,
): ProposalErrorCode | undefined {
  if (now >= record.expiresAt) return 'EXPIRED'
  if (
    sha256(record.content) !== record.contentDigest
    || Buffer.byteLength(record.content, 'utf8') !== record.contentBytes
    || record.content.length !== record.contentChars
  ) return 'DIGEST_MISMATCH'
  if (context.vaultId !== record.vaultId || context.vaultGeneration !== record.vaultGeneration) return 'STALE_VAULT'
  if (context.childInstanceId !== record.childInstanceId) return 'CHILD_REPLACED'
  if (
    context.writePermission !== record.writePermission
    || context.permissionEpoch !== record.permissionEpoch
  ) return 'PERMISSION_CHANGED'
  if (context.turnId !== record.turnId || context.requestId !== record.requestId) return 'TURN_MISMATCH'
  if (context.provider !== record.provider || context.model !== record.model) return 'PROVIDER_MISMATCH'
  return undefined
}

function parseAudit(value: unknown): ProposalAuditEntry {
  assertPlainRecord(value)
  assertKeys(value, [
  ...
  if (![
    ...
    'applied',
    'rejected',
  ].includes(value.outcome as string)) fail('CORRUPT_QUEUE')
  assertOpaqueId(value.vaultId, 'CORRUPT_QUEUE')
  assertGeneration(value.vaultGeneration, 'CORRUPT_QUEUE')
  safePath(value.destination, 'CORRUPT_QUEUE')
  if (value.operation !== 'create' && value.operation !== 'update') fail('CORRUPT_QUEUE')
  ...
  const target = expectedTarget(value.expectedTarget, value.operation, 'CORRUPT_QUEUE')
  assertDigest(value.contentDigest, 'CORRUPT_QUEUE')
  if (!Number.isSafeInteger(value.contentBytes) || (value.contentBytes as number) < 0 || (value.contentBytes as number) > MAX_CONTENT_BYTES) fail('CORRUPT_QUEUE')
  assertOpaqueId(value.childInstanceId, 'CORRUPT_QUEUE')
  assertOpaqueId(value.turnId, 'CORRUPT_QUEUE')
  assertOpaqueId(value.requestId, 'CORRUPT_QUEUE')
  assertRoute(value.provider, 128, 'CORRUPT_QUEUE')
  ...
  if (value.writePermission !== 'propose') fail('CORRUPT_QUEUE')
  assertEpoch(value.permissionEpoch, 'CORRUPT_QUEUE')
  if (value.reason !== undefined && (typeof value.reason !== 'string' || value.reason.length > MAX_REASON_CHARS || redactBoundaryText(value.reason) !== value.reason)) fail('CORRUPT_QUEUE')
  return {
    version: VERSION,
...
export class ProposalQueue {
  ...
  private readonly proposals = new Map<string, ConsumedProposal>()
  private readonly approvals = new Map<string, ConsumedProposal>()
  private audits: ProposalAuditEntry[] = []
  ...
  constructor(options: ProposalQueueOptions = {}) {
    ...
    if (!Number.isSafeInteger(this.pendingLimit) || this.pendingLimit < 1 || this.pendingLimit > 1_000) fail('INVALID_PROPOSAL')
    if (!Number.isSafeInteger(this.auditLimit) || this.auditLimit < 1 || this.auditLimit > 10_000) fail('INVALID_PROPOSAL')
  }

  stage(input: StageProposalInput): ProposalSummary {
    this.pruneExpired()
    if (this.proposals.size >= this.pendingLimit) fail('QUEUE_FULL')
    assertOpaqueId(input.vaultId)
    assertGeneration(input.vaultGeneration)
    safePath(input.destination, 'INVALID_PROPOSAL')
    if (input.operation !== 'create' && input.operation !== 'update') fail('INVALID_PROPOSAL')
    ...
    if (typeof input.content !== 'string' || input.content.includes('\0')) fail('INVALID_PROPOSAL')
    const contentBytes = Buffer.byteLength(input.content, 'utf8')
    if (input.content.length > MAX_CONTENT_CHARS || contentBytes > MAX_CONTENT_BYTES) fail('INVALID_PROPOSAL')
    ...
    if (input.writePermission !== 'propose') fail('PERMISSION_CHANGED')
    assertEpoch(input.permissionEpoch)
    const expiresInMs = input.expiresInMs ?? DEFAULT_EXPIRY_MS
    if (!Number.isSafeInteger(expiresInMs) || expiresInMs < 1 || expiresInMs > MAX_EXPIRY_MS) fail('INVALID_PROPOSAL')
  ...
  recordApprovalOutcome(
    candidate: ConsumedProposal,
    outcome: 'applied' | 'approval-failed',
    reason?: string,
  ): void {
    const record = this.approvals.get(candidate.proposalId)
    if (
  ...
  invalidateVault(current: { id: string; generation: number } | null): number {
    let invalidated = 0
    for (const [token, record] of this.proposals) {
      if (
        current !== null
  ...
  invalidatePermission(permission: ProposalWritePermission, epoch: number): number {
    if (permission !== 'read-only' && permission !== 'propose') fail('INVALID_PROPOSAL')
    assertEpoch(epoch)
    let invalidated = 0
    for (const [token, record] of this.proposals) {
      if (record.writePermission === permission && record.permissionEpoch === epoch) continue
      this.proposals.delete(token)
  ...
  async invalidateRestored(
    validate: (proposal: ProposalSummary) => Promise<ProposalErrorCode | undefined>,
  ): Promise<number> {
    this.pruneExpired()
    let invalidated = 0
    for (const [token, record] of [...this.proposals]) {
      const mismatch = await validate(recordSummary(record))
      if (mismatch === undefined || this.proposals.get(token) !== record) continue
      this.proposals.delete(token)
      this.appendAudit(record, 'approval-denied', mismatch)
      invalidated += 1
    }
  ...
  serialize(): string {
    ...
    if (Buffer.byteLength(serialized, 'utf8') > MAX_PERSISTED_BYTES) fail('CORRUPT_QUEUE')
    return serialized
  }

  static hydrate(serialized: string, options: ProposalQueueOptions = {}): ProposalQueue {
    if (typeof serialized !== 'string' || Buffer.byteLength(serialized, 'utf8') > MAX_PERSISTED_BYTES) fail('CORRUPT_QUEUE')
    let parsed: unknown
    try {
    ...
    if (
      ...
      || (parsed.auditDropped as number) < 0
    ) fail('CORRUPT_QUEUE')
    const queue = new ProposalQueue(options)
    if (parsed.proposals.length > queue.pendingLimit || parsed.audits.length > queue.auditLimit) fail('CORRUPT_QUEUE')
    ...
    for (const entry of parsed.proposals) {
      ...
      if (record.expiresAt > maximumExpiry) fail('CORRUPT_QUEUE')
      if (queue.proposals.has(record.token) || proposalIds.has(record.proposalId)) fail('CORRUPT_QUEUE')
      proposalIds.add(record.proposalId)
      queue.proposals.set(record.token, record)
    }
    const auditIds = new Set<string>()
    queue.auditDropped = parsed.auditDropped as number
    queue.audits = parsed.audits.map((entry) => {
      const audit = parseAudit(entry)
      if (auditIds.has(audit.auditId)) fail('CORRUPT_QUEUE')
  ...
  private appendAudit(record: ConsumedProposal, outcome: AuditOutcome, reason?: string): void {
