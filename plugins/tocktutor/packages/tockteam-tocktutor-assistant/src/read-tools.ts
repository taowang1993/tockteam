...
const UNAVAILABLE_READ_TOOLS = new Set<PennivoReadTool>(
  REVIEWED_PENNIVO_READ_TOOLS.filter(tool => (
    tool !== 'read_file'
    && tool !== 'list_files'
    && tool !== 'search'
    && tool !== 'find_backlinks'
...
function failure(code: ReadToolErrorCode): ReadToolError {
  const messages: Record<ReadToolErrorCode, string> = {
...
function snapshotBinding(value: ReadBinding): ReadBinding {
  if (
    !isPlainRecord(value)
...
function emptyArguments(args: unknown): void {
...
interface PennivoFolderEntry {
  name: string
  path: string
  type: 'folder'
  children?: PennivoEntry[]
}

type PennivoEntry = PennivoFileEntry | PennivoFolderEntry

function listFilesArguments(args: unknown): ListFilesArguments {
  if (!isPlainRecord(args) || Object.keys(args).some(key => key !== 'path' && key !== 'recursive')) {
    throw failure('INVALID_ARGUMENTS')
  }
  if (args.path !== undefined && typeof args.path !== 'string') throw failure('INVALID_ARGUMENTS')
...
type ProjectedSearchMatch = Readonly<{
  path: string
  line: number
  preview: string
}>

function searchArguments(args: unknown): SearchArguments {
  if (
    !isPlainRecord(args)
...
function isSupportedMarkdown(path: string): boolean {
...
function validateSearchPage(
  ...
  for (const warning of value.warnings) {
    if (typeof warning !== 'string' || warning.length > 1_024) throw failure('INVALID_RESULT')
  }
...
function searchPayload(
  ...
  let payload: object
  do {
    const byPath = new Map<string, ProjectedSearchMatch[]>()
    for (const match of selected) {
      const group = byPath.get(match.path)
      if (group === undefined) byPath.set(match.path, [match])
    ...
    if (text.length <= MAX_READ_RESULT_CHARS) return { text, truncated }
    selected.pop()
...
type ProjectedBacklink = Readonly<{
  path: string
  line: number
  linkText: string
  url: string
...
function validateRelativeResultPath(value: unknown): void {
  if (typeof value !== 'string') throw failure('INVALID_RESULT')
  try {
...
function validateLinkRecord(
  ...
  if (
    ...
    || (value.fragment !== null && (
      typeof value.fragment !== 'string' || value.fragment.length > MAX_LINK_METADATA_CHARS
    ))
    || !['canvas-file', 'embed', 'image', 'image-reference', 'markdown', 'reference', 'tag', 'wiki'].includes(value.kind as string)
    ...
    || !['ambiguous', 'resolved', 'unresolved'].includes(value.status as string)
  ) throw failure('INVALID_RESULT')
  validateRelativeResultPath(value.sourcePath)
  if (value.resolvedPath !== null) validateRelativeResultPath(value.resolvedPath)
  if (backlink && (value.status !== 'resolved' || value.resolvedPath !== requestedPath)) {
    throw failure('INVALID_RESULT')
  }
}

function validateLinksPage(
  value: LinksPage,
  requestedPath: string,
  binding: ReadBinding,
): LinksPage {
  if (
    ...
    || ![null, 'byte-limit', 'entry-limit', 'file-limit', 'metadata-limit', 'result-limit'].includes(value.truncationReason)
    || (value.cursor !== null && (
      typeof value.cursor !== 'string' || value.cursor.length === 0 || value.cursor.length > 4_096
    ))
    || (!value.truncated && (value.cursor !== null || value.truncationReason !== null))
    || (value.truncated && value.truncationReason === null)
  ) throw failure('INVALID_RESULT')
  validateRelativeResultPath(value.path)
  for (const path of [...value.outgoing, ...value.backlinks]) validateRelativeResultPath(path)
  for (const record of value.outgoingDetails) validateLinkRecord(record, requestedPath, false)
  for (const record of value.backlinkDetails) validateLinkRecord(record, requestedPath, true)
  for (const relation of value.tagRelations) {
    ...
    for (const path of relation.paths) validateRelativeResultPath(path)
  }
  for (const warning of value.warnings) {
    if (typeof warning !== 'string' || warning.length > 1_024) throw failure('INVALID_RESULT')
  }
...
type OutlineResult = VaultInspectionRuntimeResult<VaultOutlineResult>

type ProjectedHeading = Readonly<{
  level: number
  text: string
  line: number
}>
...
function validateTreeEntry(entry: VaultTreeEntry): VaultTreeEntry {
  if (!isPlainRecord(entry) || typeof entry.path !== 'string') throw failure('INVALID_RESULT')
  try {
  ...
  if (entry.kind !== 'document' && entry.kind !== 'attachment') throw failure('INVALID_RESULT')
  if (
...
function validateTreePage(value: VaultTreePage, binding: ReadBinding): VaultTreePage {
  if (
    !isPlainRecord(value)
  ...
  for (const entry of value.entries) validateTreeEntry(entry)
  return value
}

function sortPennivoEntries(entries: PennivoEntry[]): PennivoEntry[] {
...
function projectTree(
  allEntries: VaultTreeEntry[],
  scope: string,
  recursive: boolean,
): { entries: PennivoEntry[]; truncated: boolean } {
  ...
  const selected: VaultTreeEntry[] = []
  let pathChars = 0
  for (const entry of relevant) {
    const cost = entry.path.length + entryName(entry.path).length
    if (
      selected.length >= MAX_PROJECTED_ENTRIES
  ...
  const documents = selected.filter((entry): entry is Extract<VaultTreeEntry, { kind: 'document' }> => (
    entry.kind === 'document'
  ))

  const children = (directory: string): PennivoEntry[] => {
    const folders: PennivoFolderEntry[] = [...directories]
    ...
    const files: PennivoFileEntry[] = documents
...
function validateTrashList(value: TrashListResult, binding: ReadBinding): TrashListResult {
  if (
    !isPlainRecord(value)
    || value.generation !== binding.vaultGeneration
    || !Array.isArray(value.entries)
...
function runtimeError(error: unknown, signal: AbortSignal): ReadToolError {
...
function boundedText(value: string): { text: string; truncated: boolean } {
  ...
  if (redacted.length <= MAX_READ_RESULT_CHARS) return { text: redacted, truncated: false }
  return {
...
export class PennivoReadAdapter {
  private readonly runtime: RuntimeDocumentReader
  private readonly isCurrent: (binding: ReadBinding) => boolean
  ...
  async execute(
    ...
    if (
      ...
      if (UNAVAILABLE_READ_TOOLS.has(tool as PennivoReadTool)) throw failure('TOOL_UNAVAILABLE')
    ...
    if (tool === 'list_files') {
      let listArgs: ListFilesArguments
      try {
    ...
    if (tool === 'find_backlinks') return this.findBacklinks(path, binding, signal)
    if (tool === 'get_outline') return this.getOutline(path, binding, signal)
    if (tool === 'list_snapshots') return this.listSnapshots(path, binding, signal)
    ...
    return Object.freeze({
      result: Object.freeze({
        content: Object.freeze([Object.freeze({ type: 'text' as const, text })]) as readonly [
  ...
  private async getOutline(
    ...
    this.assertCurrent(binding)
    if (signal.aborted) throw failure('ABORTED')
    if (this.runtime.outline === undefined) throw failure('READ_UNAVAILABLE')
    ...
    return Object.freeze({
      result: Object.freeze({
        content: Object.freeze([Object.freeze({ type: 'text' as const, text: projected.text })]) as readonly [
          { readonly type: 'text'; readonly text: string },
        ],
      }),
      source: null,
  ...
  private async findBacklinks(
    ...
    this.assertCurrent(binding)
    if (signal.aborted) throw failure('ABORTED')
    if (this.runtime.links === undefined) throw failure('READ_UNAVAILABLE')
    const backlinks: ProjectedBacklink[] = []
    ...
    for (let pageIndex = 0; pageIndex < MAX_SEARCH_PAGES; pageIndex += 1) {
      ...
      for (const record of page.backlinkDetails) {
        if (!isSupportedMarkdown(record.sourcePath)) continue
        ...
        if (seen.has(key)) throw failure('INVALID_RESULT')
        seen.add(key)
      ...
      if (cursors.has(page.cursor)) throw failure('INVALID_RESULT')
      cursors.add(page.cursor)
      cursor = page.cursor
    }
    if (cursor !== null) adapterTruncated = true
  ...
  private async search(
    ...
    this.assertCurrent(binding)
    if (signal.aborted) throw failure('ABORTED')
    if (this.runtime.search === undefined) throw failure('READ_UNAVAILABLE')
    ...
    for (let pageIndex = 0; pageIndex < MAX_SEARCH_PAGES; pageIndex += 1) {
      ...
      for (const match of page.matches) {
        ...
        if (seenMatches.has(key)) throw failure('INVALID_RESULT')
        seenMatches.add(key)
        if (!isSupportedMarkdown(match.path) || match.line === null) continue
      ...
      if (seenCursors.has(page.cursor)) throw failure('INVALID_RESULT')
  ...
  private async listTrash(
    ...
    this.assertCurrent(binding)
    if (signal.aborted) throw failure('ABORTED')
    if (this.runtime.listTrash === undefined) throw failure('READ_UNAVAILABLE')
    let raw: TrashListResult
    try {
    ...
    if (text.length > MAX_READ_RESULT_CHARS) throw failure('INVALID_RESULT')
    return Object.freeze({
      result: Object.freeze({
        content: Object.freeze([Object.freeze({ type: 'text' as const, text })]) as readonly [
          { readonly type: 'text'; readonly text: string },
  ...
  private async listSnapshots(
    ...
    this.assertCurrent(binding)
    if (signal.aborted) throw failure('ABORTED')
    if (this.runtime.listSnapshots === undefined) throw failure('READ_UNAVAILABLE')
    let raw: SnapshotListResult
    try {
    ...
    if (signal.aborted) throw failure('ABORTED')
    let snapshots: SnapshotListResult
    try {
  ...
  private async listFiles(
    ...
    for (let pageIndex = 0; pageIndex < MAX_TREE_PAGES; pageIndex += 1) {
      ...
      for (const entry of page.entries) {
        if (paths.has(entry.path)) throw failure('INVALID_RESULT')
      ...
      for (const warning of page.warnings) {
        if (warnings.length >= MAX_TREE_WARNINGS) break
  ...
  private assertCurrent(binding: ReadBinding): void {
    let matches: boolean
    try {
    ...
    if (!matches) throw failure('STALE_CONTEXT')
  }
}
