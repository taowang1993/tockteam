...
const workspace = await mkdtemp(join(tmpdir(), 'tocktutor-desktop-typert-'))
const packageRoot = join(workspace, 'packages', 'desktop-adapter')
...
try {
  ...
  if (artifact.remote === undefined) throw new Error('Typert did not emit the Desktop Remote contribution')
