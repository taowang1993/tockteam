...
import { buildMarkdownExportDocument, collectEmbedTargets, resolveEmbedGraph, } from '@tockteam/tocktutor-workbench';
...
async function resolveExportEmbeds(runtime, source, sourcePath, expectedVault, signal) {
    ...
    const resolved = await resolveEmbedGraph({
        ...
        async readAttachment(path) {
            ...
            if (entry?.kind !== 'attachment' || aggregateBytes >= 6_000_000)
            ...
            assertCurrentVault(runtime, expectedVault);
            if (preview.generation !== expectedVault.generation || preview.path !== path || preview.data.byteLength > 1_500_000)
                throw new Error('Embed unavailable.');
            aggregateBytes += preview.data.byteLength;
            if (aggregateBytes > 6_000_000)
                throw new Error('Embed budget exceeded.');
            return { dataBase64: Buffer.from(preview.data).toString('base64'), mimeType: preview.mimeType, path };
        },
        async readDocument(path) {
            if (!entries.some(entry => entry.path === path && entry.kind === 'document') || aggregateBytes >= 6_000_000)
                throw new Error('Embed unavailable.');
