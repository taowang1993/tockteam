...
const MARKDOWN_ONLY_SEARCH = Symbol.for('tockbot-note-vault.markdown-only-search')
...
async function collectInspectionInventory(input, config, signal, position, options) {
  ...
  let providerPages = 0
  const filterMarkdown = options.documentKind === 'markdown'
  while (entriesRead < config.maxSearchEntries) {
    ...
    for (const rawEntry of page.entries) {
      ...
      if (filterMarkdown && !isMarkdown(entry.path)) continue
      entries.push(entry)
    }
    if (page.cursor === null) {
...
async function searchVault(input, query, scope, options, limit, config, signal, cursor) {
  ...
  const key = JSON.stringify({ directory: start.prefix, documentKind: options.documentKind, modifiedFrom: options.modifiedFrom, modifiedTo: options.modifiedTo, query: needle, scope, titleOnly: options.titleOnly })
  ...
  const state = await scanVault(input, config, signal, (document) => {
    ...
    return false
  }, { cursor: position.sourceCursor, directory: start.path, documentKind: options.documentKind, key, operation: 'search' })
...
function validCandidateResult(result, limit, documentKind) {
  if (
  ...
  for (const value of result.entries) {
    ...
    if (documentKind === 'markdown' && !isMarkdown(candidate)) continue
...
async function searchRelatedVault(input, query, options, limit, config, signal, cursor) {
  ...
  const state = await scanVault(input, config, signal, document => {
    ...
    return false
  }, { cursor: sourceCursor, key, operation: 'related-source', directory: start.path, documentKind: options.documentKind })
  ...
  return Object.freeze({
    async search(args, signal) {
      ...
      const searchOptions = {
        ...
        documentKind: args[MARKDOWN_ONLY_SEARCH] === true ? 'markdown' : 'all',
