...
test('private Markdown-only search bounds malformed and oversized candidate inventories before filtering', async () => {
  const markdownOnly = Symbol.for('tockbot-note-vault.markdown-only-search')
  const contents = new Map([
    ['a.canvas', JSON.stringify({ nodes: [], edges: [] })],
    ['b.base', 'filters:\n  - project\n'],
    ['note.md', '---\ntags: [project]\n---\n# Note\n'],
  ])
  const entries = [...contents].map(([path, content]) => ({
  ...
  let candidateResult = null
  let listCalls = 0
  let candidateEntryReads = 0
  const input = {
    async list({ cursor = null, limit }) {
      listCalls += 1
      const start = cursor === null ? 0 : entries.findIndex(entry => entry.path === cursor) + 1
      const page = entries.slice(start, start + Math.min(limit, 2))
      const next = start + page.length < entries.length ? page.at(-1).path : null
      return {
        entries: page,
        cursor: next,
        complete: next === null,
        truncated: next !== null,
        truncationReason: next === null ? null : 'result-limit',
        warnings: [],
      }
    },
    async read(path, maxBytes, signal) {
    ...
    async searchCandidates() { return candidateResult },
  }
  const inspection = createVaultInspection(input, { ...limits, maxSearchEntries: 2, maxSearchResults: 1 })
  const search = () => inspection.search({ mode: 'query', query: 'tag:project', limit: 1, [markdownOnly]: true }, new AbortController().signal)

  const oversizedEntries = entries.map(({ modifiedMs, path, revision }) => ({ modifiedMs, path, revision }))
  Object.defineProperty(oversizedEntries, 0, {
    configurable: true,
    enumerable: true,
    get() {
      candidateEntryReads += 1
      return { modifiedMs: 1, path: 'a.canvas', revision: 'revision:a.canvas' }
    },
  })
  candidateResult = { complete: true, epoch: 'oversized-index', entries: oversizedEntries }
  const oversized = await search()
  assert.deepEqual(oversized.matches, [])
  assert.equal(oversized.scan.entries, 2)
  assert.equal(oversized.truncated, true)
  assert.equal(listCalls, 1)
  assert.equal(candidateEntryReads, 0, 'raw candidate quota is checked before iterating untrusted entries')

  listCalls = 0
  candidateResult = {
    complete: true,
    epoch: 'malformed-index',
    entries: [{ modifiedMs: 1, path: 'note.md', revision: 42 }],
  }
  const malformed = await search()
  assert.deepEqual(malformed.matches, [])
  assert.equal(malformed.scan.entries, 2)
...
test('private Markdown-only search filters document kinds before result pagination', async () => {
  const markdownOnly = Symbol.for('tockbot-note-vault.markdown-only-search')
  const contents = new Map([
    ['needle.canvas', JSON.stringify({ nodes: [{ id: 'canvas', type: 'text', text: 'Canvas body' }], edges: [] })],
    ['needle.base', 'needle in Base body\n'],
    ['note-c.md', '# Note C\nneedle in Markdown body\n'],
    ['note-d.md', '# Note D\nneedle in Markdown body\n'],
  ])
  const entries = [...contents].map(([path, content]) => ({
    createdMs: 1,
  ...
  const input = {
    async list({ cursor = null, limit }) {
      const start = cursor === null ? 0 : entries.findIndex(entry => entry.path === cursor) + 1
      const page = entries.slice(start, start + Math.min(limit, 1))
      const next = start + page.length < entries.length ? page.at(-1).path : null
      return {
        entries: page,
        cursor: next,
  ...
  const allKinds = await inspection.search({ query: 'needle', limit: 1 }, signal)
  assert.equal(allKinds.matches[0]?.path, 'needle.base')
  assert.equal(allKinds.matches[0]?.kind, 'path')
  assert.equal(allKinds.scan.files, entries.length)
  const readsBeforeMarkdownOnly = readPaths.length

  const first = await inspection.search({ query: 'needle', limit: 1, [markdownOnly]: true }, signal)
  assert.deepEqual(first.matches.map(match => match.path), ['note-c.md'])
  assert.equal(first.truncated, true)
  assert.notEqual(first.cursor, null)
  const second = await inspection.search({
    query: 'needle',
  ...
  assert.deepEqual(second.matches.map(match => match.path), ['note-d.md'])
  assert.equal(second.cursor, null)
  assert.deepEqual(readPaths.slice(readsBeforeMarkdownOnly), ['note-c.md', 'note-d.md', 'note-c.md', 'note-d.md'])
