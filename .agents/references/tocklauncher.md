...
Expand **TockLauncher** in the Settings sidebar. **General** opens shared search, appearance, lifecycle, browser, storage and update controls. Expand **Extensions** to reveal its search field and all individual extension buttons; selecting one opens its dedicated page on the right. The sidebar contains every built-in identity in `LAUNCHER_COMPOSITION` and all three reviewed compatibility descriptors (27 destinations at this baseline). Search accepts names and setting labels; the sidebar and forms are localized in English and Chinese. Provider editors and DeepL's write-only key belong to their individual extension pages. There is no second set of navigation tabs or oversized extension list inside the content area.
