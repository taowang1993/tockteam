...
- Must stop every Electron app, browser session, web server, and descendant process started for verification unless the user asks to keep it running. Verify that the full process tree stopped; terminating only a wrapper or parent process is insufficient.
