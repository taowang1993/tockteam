...
test('cached state persists and admits legacy stored shapes through the compatibility hook', async () => {
  ...
  try {
    ...
    const output = await build({ stdin: { contents: `export { useCachedState } from './src/trusted-raycast-translate-compat-utils.ts'`, resolveDir: resolve('.') }, bundle: true, write: false, format: 'esm', platform: 'node', plugins: [{ name: 'isolated-hook-scheduler', setup(builder) {
      builder.onResolve({ filter: /^react$/ }, () => ({ path: 'hooks', namespace: 'test' }))
      builder.onResolve({ filter: /^@tockteam\/trusted-raycast-translate-catalog$/ }, () => ({ path: 'catalog', namespace: 'test' }))
      builder.onLoad({ filter: /^catalog$/, namespace: 'test' }, () => ({ contents: `export const supportedLanguagesByCode = ${JSON.stringify({ auto: {}, en: {}, 'zh-CN': {}, fr: {} })}` }))
    ...
    const translateState = readFileSync(join(resolve('.'), 'src', 'trusted-raycast-translate-state.ts'), 'utf8')
    assert.match(translateState, /atomicWrite\(path,/)
    assert.doesNotMatch(translateState, /writeFileSync\(path,/)
    assert.doesNotMatch(output.outputFiles[0]!.text, /src\/trusted-raycast-compat-utils\.ts/)
  } finally {
...
test('built Translate child rejects oversized state before source boot and preserves it', { timeout: 30000 }, async t => {
  ...
  const work = mkdtempSync(join(tmpdir(), 'raycast-translate-state-child-'))
  const stateFile = join(work, 'state.json')
  const contents = JSON.stringify({ selectedLanguageSet: null, padding: 'x'.repeat(MAX_TRANSLATE_STATE_BYTES + 1) })
  ...
  try {
    ...
    await manager.start({ webContentsId: 1 }, {
      extensionId: 'google-translate', sessionId: 'state-child', generation: '1', command: 'translate',
      preferences: { ...TRUSTED_RAYCAST_PREFERENCE_DEFAULTS, autoInput: false },
    })
