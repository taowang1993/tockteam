...
for (const mode of ['symlink', 'symlink-on-open', 'file-on-open', 'symlink-on-read', 'growth'] as const) {
  test(`bounded reads fail closed and release their descriptor: ${mode}`, t => {
    ...
    try {
      assert.throws(() => readBoundedRegularFile(selected, 4), /invalid|changed|symlink|size limit/i)
