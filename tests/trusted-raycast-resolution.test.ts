...
test('private child resolution permits Node builtins but forbids ambient module fallback', () => {
  ...
  try {
    ...
    const allowed = spawnSync(process.execPath, ['--import', hookUrl, '-e', 'import("node:fs")'], { timeout: 2000 })
    assert.equal(allowed.status, 0, allowed.stderr.toString())
    const forbidden = spawnSync(process.execPath, ['--import', hookUrl, '-e', `import(${JSON.stringify(pathToFileURL(module).href)})`], { timeout: 2000 })
