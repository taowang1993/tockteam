...
test('main assembles one launcher owner without branching the DSH workbench factory', () => {
  ...
  assert.match(main, /ttlMsForSource/u)
  assert.match(main, /TOCKTEAM_OS_FIXTURE/u)
  assert.match(main, /TOCKTEAM_TERMINAL_FIXTURE/u)
  assert.match(main, /TOCKTEAM_BROWSER_FIXTURE/u)
  assert.match(main, /launcherBrowserFixtureMarker/u)
  ...
  assert.match(main, /searchInstant: async searchTerm => \{[\s\S]+networkResults\.lastError[\s\S]+fileResults\.lastError/u)
  assert.doesNotMatch(main, /getIndexedError: \(\) => \{[\s\S]+network\.getLastError\(\)[\s\S]+fileSearch\.getLastError\(\)[\s\S]+os\.getLastError\(\)/u)
...
test('fixture smoke reads host-owned effect counters instead of renderer authority', () => {
  ...
  assert.match(electronSmoke, /browser-fixture-marker\.json/u)
  ...
  assert.match(electronSmoke, /fileSearchMenuItemClicked/u)
  assert.match(electronSmoke, /Show in Finder/u)
  assert.match(electronSmoke, /window\.hideWindowOn/u)
