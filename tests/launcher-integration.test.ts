...
test('main assembles one launcher owner without branching the DSH workbench factory', () => {
  ...
  assert.match(main, /os\.loadIndexedItems\(signal\)/u)
  assert.match(main, /if \(await os\.executeAction\(record\)\)/u)
  assert.match(main, /os\.invalidate\(\)/u)
  assert.match(main, /onWindowCleared/u)
  assert.match(main, /os\.waitForIdle\(\)/u)
  ...
  assert.match(main, /launcherCoreFlush = async \(\) => \{[\s\S]+await fileSearch\.close\(\)[\s\S]+await networkClose[\s\S]+await osClose[\s\S]+await coreSearch\.close\(\)/u)
