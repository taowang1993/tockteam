...
test('main assembles one launcher owner without branching the DSH workbench factory', () => {
  ...
  assert.match(main, /getIndexedError: \(\) => network\.getLastError\(\) \?\? fileSearch\.getLastError\(\)/u)
  assert.match(main, /launcherNetworkFixtureEnabled = !app\.isPackaged && process\.env\.TOCKTEAM_NETWORK_FIXTURE === '1'/u)
