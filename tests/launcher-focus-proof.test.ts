...
function setup(windows: FakeWindow[] = []) {
  const app = new FakeApp(); const channel = new FakeChannel(); const scheduled: Array<() => void> = []
  const proof = installLauncherFocusProof({ app, channel, enabled: true, getAllWindows: () => windows, nonce, scheduleExit: callback => { scheduled.push(callback) }, shutdown: code => { app.exit(code) } })
  return { app, channel, proof, scheduled, windows }
}
...
test('normal Desktop mode installs no proof listeners or IPC authority', () => {
  ...
  const proof = installLauncherFocusProof({ app, channel, enabled: false, getAllWindows: () => [], nonce: undefined, scheduleExit: callback => callback(), shutdown: code => { app.exit(code) } })
...
test('installs app and per-window listeners before READY and latches pre-handshake activation', () => {
  ...
  installLauncherFocusProof({ app, channel, enabled: true, getAllWindows: () => [window], nonce, scheduleExit: callback => callback(), shutdown: code => { app.exit(code) } })
