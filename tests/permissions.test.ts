...
test('every info and privileged IPC handler invokes the trusted-main guard', () => {
  ...
  for (const channel of ['choose-workspace', 'get-info', 'get-runtime-snapshot', 'tocktutor-authorize', 'tocktutor-dispatch-next', 'tocktutor-dispatch-complete', 'plugin-marketplace-snapshot', 'plugin-marketplace-dispatch', 'web-clip-authorize-document', 'open-external']) {
