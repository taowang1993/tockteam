...
test('desktop profile pins the released TockTutor runtime and peer package', () => {
  ...
  assert.equal(desktopAdapter.version, '0.1.0')
  assert.equal(desktopAdapter.peerDependencies['@tockteam/desktop'], '>=0.1.6 <0.2.0')
  assert.equal(desktopAdapter.peerDependencies['@tockteam/tocktutor-workbench'], '0.1.4')
