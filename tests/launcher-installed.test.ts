...
test('installed report validation requires complete platform lifecycle evidence', () => {
  ...
  const rendererFor = (rendererAppPath: string) => ({
    launcher: { apiKeys: ['cancelAction', 'dismiss', 'getLocalExtensionSettings', 'getSurfaceSettings', 'getTheme', 'getTrustedRaycastTrust', 'invokeAction', 'onLocale', 'onTheme', 'onTrustedRaycastView', 'openSettings', 'recordSearch', 'rescan', 'search', 'trustedRaycastClose', 'trustedRaycastEvent', 'trustedRaycastTrustAction'], csp: "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'; object-src 'none'", hasNodeProcess: false, hasRequire: false, notificationPermission: 'denied', ready: 'true', title: 'TockLauncher' },
