...
test('usePromise fences results before debounce and recovers from current service failure', async () => {
  const output = await build({ stdin: { contents: `export { usePromise } from './src/trusted-raycast-compat-utils.ts'; export { advanceQuery } from './src/trusted-raycast-compat-api.ts'`, resolveDir: resolve('.') }, bundle: true, write: false, format: 'esm', platform: 'node', plugins: [{ name: 'isolated-hook-scheduler', setup(builder) {
    ...
    builder.onLoad({ filter: /.*/, namespace: 'test' }, () => ({ contents: `let state, dependency, cleanup; const ref = {current:0}; export default {useState(initial) {state ??= (typeof initial === 'function' ? initial() : initial); return [state,next=>{state=typeof next==='function'?next(state):next}]},useRef(value){return {current:value}},useEffect(effect,deps){if(JSON.stringify(deps)!==dependency){cleanup?.();dependency=JSON.stringify(deps);cleanup=effect()}},createContext(value){return {Provider:()=>null,value}},useContext(){return null},Children:{toArray:children=>children}}` }))
  } }] })
