...
test('the installed-app gate exercises all bundled commands and restart persistence', () => {
  ...
  const proof = readFileSync('scripts/trusted-raycast-can-i-use-installed-proof.mjs', 'utf8')
  const bundled = proof.slice(proof.indexOf('export async function runBundledTrustedRaycastInstalledSmoke'), proof.indexOf('/** Reopen the same packaged app'))
  const restart = proof.slice(proof.indexOf('export async function runTrustedRaycastInstalledRestartSmoke'))
  ...
  assert.match(installed, /runBundledTrustedRaycastInstalledSmoke/u)
  assert.match(installed, /runTrustedRaycastInstalledRestartSmoke/u)
  assert.match(proof, /TRUSTED_RAYCAST_EXTENSION_IDS/u)
  assert.match(bundled, /assertBundledTrust[\s\S]*google-translate[\s\S]*kaomoji-search/u)
  assert.match(bundled, /Disable Extension/u)
