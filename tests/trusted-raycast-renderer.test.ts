...
test('first-run preferences use the Raycast-like centered hierarchy and keyboard submit', async () => {
  ...
  assert.ok(continueButton.className.includes('!px-2'), 'setup action uses compact horizontal padding')
  ...
  assert.ok(shortcutGroup?.className.includes('ml-0.5'), 'setup action and keycaps use compact spacing')
  ...
  assert.ok(keycaps.every(node => node.className.includes('box-border') && node.className.includes('size-[1.375rem]') && node.className.includes('justify-center')), 'setup keycaps are enlarged and center their glyphs')
