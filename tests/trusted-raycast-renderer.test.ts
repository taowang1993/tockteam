...
test('Translate collections expose paging, full-language actions and search without losing focus', async () => {
  const nodes: Element[] = []; const sent: TrustedRaycastViewEvent[] = []
  const document = { get activeElement() { return Element.activeElement }, createElement(tag: string) { const node = new Element(); node.tagName = tag.toUpperCase(); nodes.push(node); return node } } as unknown as Document
  const view = createTrustedRaycastView(document, { async trustedRaycastEvent(event: TrustedRaycastViewEvent) { sent.push(event) } } as unknown as LauncherPreloadBridge, () => {})
  const message = (revision: number, page: number): TrustedRaycastViewMessage => ({ ...projection(revision), root: { type: 'root', props: { languageCollection: true, collectionPage: page, collectionPages: 6, collectionCount: 129, collectionTotal: 129, navigationDepth: 1, searchEventId: `search-${revision}` }, children: [
    { type: 'raycast-list', props: { searchText: '', searchBarPlaceholder: 'Search Language Sets' }, children: [
      { type: 'raycast-list-item', props: { title: 'English →', subtitle: 'French, French…' }, children: [
        { type: 'raycast-action', props: { title: 'Select', actionEventId: `select-${revision}` }, children: [] },
        { type: 'raycast-action', props: { title: 'View All Languages', collectionControl: 'expand', actionEventId: `expand-${revision}` }, children: [] },
      ] },
      { type: 'raycast-action-panel', props: {}, children: [
        { type: 'raycast-action', props: { title: 'Next Page', collectionControl: 'next', actionEventId: `next-${revision}` }, children: [] },
        { type: 'raycast-action', props: { title: 'Remove All', actionEventId: `remove-${revision}` }, children: [] },
      ] },
    ] },
  ] } })
  view.update(message(0, 1))
  assert.ok(nodes.some(node => node.textContent === 'Search Language Sets'))
  const bar = nodes.find(node => node.getAttribute('aria-label') === 'Language Set Pages')!
  assert.ok(bar && !bar.hidden)
  assert.ok(nodes.some(node => node.textContent === '129 Language Sets · Page 1 of 6'))
  const next = nodes.find(node => node.getAttribute('data-collection-control') === 'next')!
  next.focus(); next.dispatchEvent(new Event('click')); await flush()
  assert.equal(sent.at(-1)?.eventId, 'next-0')
  view.update(message(1, 2))
  assert.equal(Element.activeElement?.getAttribute('data-collection-control'), 'next')
  assert.notEqual(Element.activeElement, next)
  view.update({ type: 'outcome', extensionId: 'google-translate', sessionId: 's', generation: 'g', revision: 1, eventId: 'next-0', succeeded: true, message: '' })
  const expand = nodes.findLast(node => node.getAttribute('data-collection-control') === 'expand')!
  assert.ok(expand)
  expand.dispatchEvent(new Event('click')); await flush()
  assert.equal(sent.at(-1)?.eventId, 'expand-1')
  assert.ok(nodes.some(node => node.textContent === 'Remove All'), 'collection-wide actions remain available with rows present')
  view.dispose()
})

test('Translate expansion Back restores the inspected row and its primary action', () => {
  const nodes: Element[] = []
  const document = { get activeElement() { return Element.activeElement }, createElement(tag: string) { const node = new Element(); node.tagName = tag.toUpperCase(); nodes.push(node); return node } } as unknown as Document
  const view = createTrustedRaycastView(document, { async trustedRaycastEvent() {} } as unknown as LauncherPreloadBridge, () => {})
  const collection = (revision: number): TrustedRaycastViewMessage => ({ ...projection(revision), root: { type: 'root', props: { navigationDepth: 1, languageCollection: true, collectionDetail: false, collectionPage: 1, collectionPages: 1, collectionCount: 3, collectionTotal: 3 }, children: [{ type: 'raycast-list', props: {}, children: ['Add New', 'First Set', 'Inspected Set'].map(title => ({ type: 'raycast-list-item', props: { title }, children: [{ type: 'raycast-action', props: { title: `Select ${title}`, actionEventId: title }, children: [] }] })) }] } })
  view.update(collection(0))
  const row = nodes.filter(node => node.getAttribute('data-selected') !== null)[2]!
  row.focus(); row.dispatchEvent(new Event('focusin'))
  view.update({ ...projection(1), root: { type: 'root', props: { navigationDepth: 2, languageCollection: true, collectionDetail: true, collectionPage: 1, collectionPages: 1 }, children: [{ type: 'raycast-list', props: { isShowingDetail: true }, children: [{ type: 'raycast-list-item', props: { title: 'All Languages' }, children: [] }] }] } })
  view.update(collection(2))
  const restored = nodes.filter(node => node.getAttribute('data-selected') !== null).slice(-3)
  assert.equal(restored[2]!.getAttribute('data-selected'), 'true')
  assert.equal(Element.activeElement, restored[2])
  assert.ok(nodes.some(node => node.tagName === 'BUTTON' && node.getAttribute('aria-label') === 'Select Inspected Set'))
  view.dispose()
})
