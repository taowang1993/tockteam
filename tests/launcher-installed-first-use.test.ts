...
test('installed proof separates verified UI errors from expected connection teardown', () => {
  const observer = createInstalledBrowserErrorProof()
  observer.beginShutdown()
  observer.consoleError('Failed to load resource: net::ERR_INCOMPLETE_CHUNKED_ENCODING')
  observer.finishShutdown()
  assert.deepEqual(observer.proof.consoleErrors, [])
  assert.deepEqual(observer.proof.shutdownConsoleErrors, ['Failed to load resource: net::ERR_INCOMPLETE_CHUNKED_ENCODING'])
  for (const phase of ['active', 'shutdown']) {
    const unexpected = createInstalledBrowserErrorProof()
    if (phase === 'shutdown') unexpected.beginShutdown()
    unexpected.consoleError('Unhandled component failure')
    assert.throws(() => phase === 'active' ? unexpected.beginShutdown() : unexpected.finishShutdown())
  }
  const pageFailure = createInstalledBrowserErrorProof()
  pageFailure.beginShutdown(); pageFailure.pageError('Uncaught exception')
  assert.throws(() => pageFailure.finishShutdown())
})
...
test('installed first-use harness is inactive, authenticated, direct, and first-use-only', async () => {
  ...
  const canIUseStart = canIUse.indexOf('export async function runCanIUseInstalledSmoke')
  const canIUseFlow = canIUse.slice(canIUse.indexOf('const wait', canIUseStart), canIUse.indexOf('const paths', canIUseStart))
