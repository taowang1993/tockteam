...
test('native translation requests admit only bounded Copy and the exact Google Translate URL', () => {
  ...
  const base = { type: 'native', sessionId: 's', generation: 'g', revision: 2, eventId: 'a', requestId: 'n' }
...
test('native denial is correlated, replay and query replacement cannot perform effects', async () => {
  ...
  const request = { type: 'native', sessionId: 's', generation: 'g', revision: 2, eventId: 'action', requestId: 'n', kind: 'copy', text: 'controlled' }
  ...
  assert.deepEqual(JSON.parse(writes.at(-1)!), { type: 'nativeOutcome', requestId: 'browser-result', succeeded: false, message: 'Browser denied' })
