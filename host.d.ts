...
export declare const TOCKTEAM_DESKTOP_MICROPHONE_SERVICE: "tockTeamDesktopMicrophone";
export declare const TOCKTEAM_DESKTOP_PRINT_EXPORT_SERVICE: "tockTeamDesktopPrintExport";
...
export declare const MAX_DESKTOP_SOURCE_ENTRIES = 100000;
export declare const MAX_DESKTOP_SOURCE_DEPTH = 128;
export declare const MAX_DESKTOP_SOURCE_ENTRY_BYTES: number;
export declare const MAX_DESKTOP_SOURCE_TOTAL_BYTES: number;
export declare const MAX_DESKTOP_SOURCE_RELATIVE_PATH_BYTES = 4096;
export declare const MAX_DESKTOP_SOURCE_PAGE_ENTRIES = 256;
export declare const MAX_DESKTOP_DESTINATION_CHUNK_BYTES: number;
export declare const MAX_DESKTOP_GRANT_SESSION_MS: number;
export declare const DESKTOP_DESTINATION_PLAN_VERSION: 1;
export declare const MAX_PRINT_EXPORT_HTML_BYTES: number;
export declare const MAX_PRINT_EXPORT_TITLE_BYTES = 512;
export declare const MAX_PRINT_EXPORT_RESOURCE_REFERENCES = 256;
export declare const MAX_PRINT_EXPORT_RESOURCE_URL_BYTES: number;
...
export type DesktopSourcePurpose = 'markdown-folder' | 'markdown-zip' | 'html' | 'csv' | 'apple-journal' | 'bear-backup' | 'evernote' | 'google-keep' | 'roam-research' | 'textbundle' | 'restore-backup';
export type DesktopExportPurpose = 'export-html' | 'export-pdf' | 'vault-backup';
export type DesktopPickerRequest = {
...
export type DesktopSourceRejectionReason = 'depth-limit' | 'entry-limit' | 'hardlink' | 'invalid-name' | 'special-file' | 'symlink' | 'total-bytes-limit';
...
export type DesktopSourceFileEntry = Extract<DesktopSourceEntry, {
    kind: 'file';
}>;
...
export type StatDesktopSourceResult = Exclude<DesktopSourceEntry, {
    kind: 'rejected';
}>;
...
export type DesktopCleanupEvidence = {
    ...
    residualLabels: DesktopPickerLabel[];
    status: 'scrubbed' | 'retained' | 'residual';
};
...
export interface TockTeamDesktopPickerService {
    ...
    statSource(request: StatDesktopSourceRequest, signal: AbortSignal): Promise<StatDesktopSourceResult>;
...
export interface TockTeamDesktopVaultSelection {
    adopt(input: TockTeamDesktopVaultSelectionAdoptInput, signal: AbortSignal): Promise<TockTeamDesktopVaultSelectionAdoptResult>;
    bind(input: TockTeamDesktopVaultSelectionBindInput, signal: AbortSignal): Promise<TockTeamDesktopVaultSelectionBindResult>;
    consume(input: TockTeamDesktopVaultSelectionConsumeInput, signal: AbortSignal): Promise<TockTeamDesktopVaultSelectionConsumeResult>;
...
export interface TockTeamDesktopDispatch {
    complete(request: DesktopDispatchCompletionRequest, signal: AbortSignal): Promise<DesktopDispatchCompletionResult>;
...
export interface NativeOwnerLifetime {
    ...
    run<Result>(operation: (signal: AbortSignal) => Promise<Result>, signal?: AbortSignal): Promise<Result>;
}
/** Own pending adapter calls and settle all of them before plugin unload returns. */
export declare function createNativeOwnerLifetime(): NativeOwnerLifetime;
export {};
