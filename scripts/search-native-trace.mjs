...
export function finishNativeTrace(directory, pid, control = false) {
  ...
  const markers = traceEvents.filter(event => event.ph === 'b' && event.name.startsWith('bon:'))
    .map(event => ({ pid: event.pid, ts: event.ts, ...JSON.parse(decodeURIComponent(event.name.slice(4))) }))
