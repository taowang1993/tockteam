...
try {
  ...
  const result = await cli('run-code', `async page => {
    ...
    const capture = await page.evaluate(() => ({ layout: [...document.querySelectorAll('html, body, [role="dialog"], [role="dialog"] > nav, [data-testid="tocklauncher-settings"]')].map(node => ({ tag: node.tagName, width: node.clientWidth, scrollWidth: node.scrollWidth })), route: location.pathname, extensionId: document.querySelector('[data-testid="tocklauncher-extension-detail"]').dataset.extensionId, mode: 'Hidden Electron with Pinned DSH Settings Shell', advancedExpanded: document.querySelector('[data-testid="tocklauncher-preferences-google-translate"] details').open, saved: document.body.innerText.includes('Saved. Changes apply'), scrollY, colorScheme: document.documentElement.style.colorScheme, skin: document.documentElement.dataset.tockteamSkin ?? null }));
