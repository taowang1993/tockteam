...
export async function proveTrustedRaycast({ port, root, workbenchConnection, userData }) {
  ...
  try {
    ...
    await cli('run-code', `async page => {
      ...
      const alignment = await setup.evaluate(section => {
        const centerX = node => { const rect = node.getBoundingClientRect(); return rect.left + rect.width / 2; };
        const close = section.querySelector('header button'); const closeIcon = close.querySelector('svg');
        const summary = section.querySelector('summary'); const summaryText = document.createRange(); summaryText.selectNode(summary.firstChild);
        const info = summary.querySelector('[data-about-info]');
        const intro = section.querySelector('.launcher-command-content > p'); const firstLabel = section.querySelector('form label > span'); const firstSelect = section.querySelector('form select');
        const action = section.querySelector('footer .launcher-command-footer-action'); const shortcut = action.querySelector('span'); const keys = [...action.querySelectorAll('kbd')];
        const actionText = document.createRange(); actionText.selectNode(action.firstChild); const textRect = actionText.getBoundingClientRect(); const shortcutRect = shortcut.getBoundingClientRect();
        const closeStyle = getComputedStyle(close); const actionStyle = getComputedStyle(action);
        return { backCenterDelta: Math.abs(centerX(close) - centerX(closeIcon)), backBackground: closeStyle.backgroundColor, backBorder: closeStyle.borderTopColor, logoWidth: section.querySelector('img[alt="Google Translate"]').getBoundingClientRect().width, infoGap: info.getBoundingClientRect().left - summaryText.getBoundingClientRect().right, introFontSize: getComputedStyle(intro).fontSize, labelFontSize: getComputedStyle(firstLabel).fontSize, selectorPaddingLeft: getComputedStyle(firstSelect).paddingLeft, actionPaddingInline: actionStyle.paddingInlineStart, actionGap: actionStyle.gap, labelShortcutGap: shortcutRect.left - textRect.right, shortcutMarginLeft: getComputedStyle(shortcut).marginLeft, keyGap: getComputedStyle(shortcut).gap, keySizes: keys.map(key => [key.getBoundingClientRect().width, key.getBoundingClientRect().height]), keyFontSizes: keys.map(key => getComputedStyle(key).fontSize), actionCenterDelta: Math.abs(centerX(action) - ((Math.min(textRect.left, shortcutRect.left) + Math.max(textRect.right, shortcutRect.right)) / 2)) };
      });
      if (alignment.backCenterDelta > 0.5 || !['rgba(0, 0, 0, 0)', 'transparent'].includes(alignment.backBackground) || !['rgba(0, 0, 0, 0)', 'transparent'].includes(alignment.backBorder) || alignment.logoWidth !== 32 || alignment.infoGap < 5 || alignment.introFontSize !== alignment.labelFontSize || alignment.selectorPaddingLeft !== '12px' || alignment.actionPaddingInline !== '8px' || alignment.actionGap !== '4px' || Math.abs(alignment.labelShortcutGap - 4) > 0.5 || alignment.shortcutMarginLeft !== '0px' || alignment.keyGap !== '1px' || alignment.keySizes.some(([width, height]) => width !== 20 || height !== 20) || alignment.keyFontSizes.some(size => size !== '14px') || alignment.actionCenterDelta > 1) throw new Error('Preference alignment, sizing, or spacing drifted: ' + JSON.stringify(alignment));
      const geometry = await setup.evaluate(section => { const footerTop = section.querySelector('footer').getBoundingClientRect().top; return [...section.querySelectorAll('form label')].map(label => { const range = document.createRange(); range.selectNode(label.firstChild); const text = range.getBoundingClientRect(); const select = label.querySelector('select').getBoundingClientRect(); const arrow = label.querySelector('svg').getBoundingClientRect(); return { labelRight: text.right, selectX: select.x, selectRight: select.right, selectTop: select.top, selectWidth: select.width, selectHeight: select.height, selectBottom: select.bottom, arrowRight: arrow.right, footerTop }; }); });
      const expectedTops = [283, 339, 395];
      if (geometry.some((row, index) => Math.abs(row.labelRight - 171) > 2 || Math.abs(row.selectX - 194) > 2 || Math.abs(row.selectTop - expectedTops[index]) > 2 || Math.abs(row.selectWidth - 362) > 2 || Math.abs(row.selectHeight - 34) > 1 || Math.abs(row.selectRight - row.arrowRight - 12) > 1 || row.selectBottom > row.footerTop)) throw new Error('Preference rows drifted from the 750x475 Raycast geometry, arrow gutter, or footer: ' + JSON.stringify(geometry));
      const footerGeometry = await setup.locator('footer').evaluate(footer => { const identity = footer.querySelector('.launcher-command-footer-identity'); const action = footer.querySelector('.launcher-command-footer-action'); const identityStyle = getComputedStyle(identity); const actionStyle = getComputedStyle(action); const keycaps = [...action.querySelectorAll('kbd')]; return { identityPadding: identityStyle.paddingInlineStart, matchingMaterial: identityStyle.backgroundColor === actionStyle.backgroundColor && identityStyle.borderTopColor === actionStyle.borderTopColor, keycapCount: keycaps.length, keycapsDistinct: keycaps.every(key => { const style = getComputedStyle(key); return style.borderTopStyle === 'solid' && style.backgroundColor !== actionStyle.backgroundColor; }) }; });
      if (footerGeometry.identityPadding !== '10px' || !footerGeometry.matchingMaterial || footerGeometry.keycapCount !== 2 || !footerGeometry.keycapsDistinct) throw new Error('Preference footer controls lost their shared material, widened identity, or distinct keycaps: ' + JSON.stringify(footerGeometry));
    ...
    await cli('run-code', `async page => { const launcher = page.context().pages().find(p => p.url().endsWith('/launcher.html')); await launcher.waitForFunction(() => document.documentElement.style.colorScheme === 'light'); const setup = launcher.locator('section[data-view="preference-setup"]'); const colors = await setup.evaluate(section => [section.querySelector('.launcher-preference-logo'), section.querySelector('summary'), section.querySelector('select'), section.querySelector('.launcher-command-footer-identity'), section.querySelector('footer .launcher-command-footer-action')].map(node => getComputedStyle(node).backgroundColor)); if (new Set(colors).size !== 1) throw new Error('Light preference materials do not share one color: ' + JSON.stringify(colors)); await launcher.screenshot({ path: ${JSON.stringify(join(evidence, 'preference-setup-light.png'))} }); return { theme: 'light', sharedMaterial: colors[0] }; }`)
    ...
    const reshowPlacementOutput = await cli('run-code', `async page => { const launcher = page.context().pages().find(p => p.url().endsWith('/launcher.html')); await launcher.waitForFunction(() => document.visibilityState === 'visible'); const placement = await launcher.evaluate(() => ({ screenX: window.screenX, screenY: window.screenY, availLeft: window.screen.availLeft })); if (${String(!process.env.CI)} && (Math.abs(placement.screenX - ${Number(launcherPlacement.screenX)}) > 2 || Math.abs(placement.screenY - ${Number(launcherPlacement.screenY)}) > 2)) throw new Error('Reshown launcher moved displays: ' + JSON.stringify(placement)); return { reshownForPaste: true, placement }; }`)
    const reshowPlacement = cliResult(reshowPlacementOutput).placement
    await writeFile(join(evidence, 'display-placement.json'), JSON.stringify({ launcher: launcherPlacement, privateBrowser: browserPlacement, reshownLauncher: reshowPlacement }), { mode: 0o600 })
    await cli('run-code', `async page => {
      const launcher = page.context().pages().find(p => p.url().endsWith('/launcher.html'));
      await launcher.bringToFront();
      await launcher.locator('#launcher-search').fill('Translate');
