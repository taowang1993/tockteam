...
export async function proveTrustedRaycast({ port, root, workbenchConnection, userData }) {
  ...
  try {
    ...
    await cli('run-code', `async page => {
      ...
      const footerGeometry = await setup.locator('footer').evaluate(footer => { const identity = footer.querySelector('.launcher-command-footer-identity'); const action = footer.querySelector('.launcher-command-footer-action'); const keycaps = [...action.querySelectorAll('kbd')]; return { identityPadding: getComputedStyle(identity).paddingInlineStart, keycapCount: keycaps.length, keycapsDistinct: keycaps.every(key => { const style = getComputedStyle(key); return style.borderTopStyle === 'solid' && style.backgroundColor !== getComputedStyle(action).backgroundColor; }) }; });
      if (footerGeometry.identityPadding !== '10px' || footerGeometry.keycapCount !== 2 || !footerGeometry.keycapsDistinct) throw new Error('Preference footer controls lost their widened identity or distinct keycaps: ' + JSON.stringify(footerGeometry));
