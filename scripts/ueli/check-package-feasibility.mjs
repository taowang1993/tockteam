...
const UELI_DERIVED_DEPENDENCIES = new Set(['color', 'fuse.js', 'fuzzysort', 'mathjs', 'uuid', 'better-sqlite3', 'sharp'])
const FORBIDDEN_IDENTITY = /tockbot|works\.tockbot|OliverSchwendener\.Ueli|\bueli\b/iu
...
export function inspectLauncherPackageFeasibility(inputs) {
  const { contract, packageJson, mainSource } = inputs
  const failures = []
  ...
  const packageIdentitySource = JSON.stringify({
  ...
  addFailure(failures, !FORBIDDEN_IDENTITY.test(`${packageIdentitySource}\n${mainSource ?? ''}`), 'forbidden launcher identity leakage')
  ...
  for (const dependencyName of Object.keys(dependencies)) {
    addFailure(failures, !UELI_DERIVED_DEPENDENCIES.has(dependencyName), `Ueli-derived dependency is admitted in package inputs: ${dependencyName}`)
  }
