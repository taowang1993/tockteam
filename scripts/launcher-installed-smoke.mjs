...
import { runBundledTrustedRaycastInstalledSmoke, runCanIUseInstalledSmoke, runTrustedRaycastInstalledRestartSmoke } from './trusted-raycast-can-i-use-installed-proof.mjs'
export {
...
import { waitForTrustedProofPage } from './trusted-raycast-proof-pages.ts'
...
export function createInstalledBrowserErrorProof() {
  let shuttingDown = false
  const proof = { pageErrors: [], consoleErrors: [], shutdownPageErrors: [], shutdownConsoleErrors: [] }
  return {
    proof,
    pageError: message => (shuttingDown ? proof.shutdownPageErrors : proof.pageErrors).push(message),
    consoleError: message => (shuttingDown ? proof.shutdownConsoleErrors : proof.consoleErrors).push(message),
    beginShutdown() {
      assert.deepEqual(proof.pageErrors, [])
      assert.deepEqual(proof.consoleErrors, [])
      shuttingDown = true
      Object.freeze(proof.pageErrors); Object.freeze(proof.consoleErrors)
    },
    finishShutdown() {
      assert.deepEqual(proof.shutdownPageErrors, [])
      assert.ok(proof.shutdownConsoleErrors.every(message => message === 'Failed to load resource: net::ERR_INCOMPLETE_CHUNKED_ENCODING'), 'Unexpected console error during app shutdown')
    },
  }
}

export function installedPlaywrightPackagePath(inventory) {
  const paths = [...new Set(inventory.flatMap(item => {
    const candidate = item.dependencies?.['@playwright/cli']?.path
    return typeof candidate === 'string' ? [candidate] : []
  }))]
  assert.equal(paths.length, 1, 'Expected one installed Playwright CLI package; install @playwright/cli globally')
  assert.ok(isAbsolute(paths[0]), 'Playwright package path must be absolute')
  return join(paths[0], 'package.json')
}

export async function loadInstalledPlaywright() {
  const { stdout } = await execFileAsync('pnpm', ['list', '--global', '@playwright/cli', '--depth', '0', '--json'], { timeout: 10_000, maxBuffer: 64 * 1024 })
  const requirePlaywright = createRequire(await realpath(installedPlaywrightPackagePath(JSON.parse(stdout))))
  const chromium = requirePlaywright('playwright').chromium
  assert.equal(typeof chromium?.connectOverCDP, 'function', 'Installed Playwright must expose the Chromium CDP API')
  return chromium
}
...
async function installedFirstUseSession(executable, userData, inventory, options = {}) {
  ...
  try {
    ...
    browser = await options.chromium.connectOverCDP(`http://127.0.0.1:${port}`, { timeout: 15_000 })
    const context = browser.contexts()[0]
    context.setDefaultTimeout(10_000)
    const page = await waitForTrustedProofPage(() => context.pages(), 'launcher')
    const workbench = await waitForTrustedProofPage(() => context.pages(), 'workbench')
    browserErrors = createInstalledBrowserErrorProof()
    for (const observed of [page, workbench]) {
      observed.on('pageerror', error => browserErrors.pageError(error.message))
      observed.on('console', message => { if (message.type() === 'error') browserErrors.consoleError(message.text()) })
    }
    await workbench.evaluate(() => window.dshDesktop.syncLauncherTheme({ mode: 'dark', skinId: null }))
    const cdp = await context.newCDPSession(page)
    await cdp.send('Emulation.setDeviceMetricsOverride', { width: 1512, height: 949, deviceScaleFactor: 2, mobile: false })
    const geometry = () => page.evaluate(() => ({ width: innerWidth, height: innerHeight, dpr: devicePixelRatio, colorScheme: document.documentElement.style.colorScheme, skin: document.documentElement.dataset.tockteamSkin ?? null }))
    const expectedGeometry = { width: 1512, height: 949, dpr: 2, colorScheme: 'dark', skin: null }
    await page.waitForFunction(() => document.documentElement.style.colorScheme === 'dark' && !document.documentElement.dataset.tockteamSkin)
    const initialGeometry = await geometry()
    assert.deepEqual(initialGeometry, expectedGeometry)
    const launcher = { evaluate: expression => page.evaluate(expression), pressKey: key => page.keyboard.press(key) }
    const clickExactText = async (page, text) => await page.evaluate(`(() => { const button = [...document.querySelectorAll('button, summary')].find(element => element.textContent?.trim() === ${JSON.stringify(text)} && element.getClientRects().length && !element.matches(':disabled')); if (!(button instanceof HTMLElement)) return false; button.click(); return true })()`)
    const clickSelector = selector => launcher.evaluate(`(() => { const element = document.querySelector(${JSON.stringify(selector)}); if (!(element instanceof HTMLElement) || element.hidden || !element.getClientRects().length || element.matches(':disabled')) return false; element.focus(); if (document.activeElement !== element) return false; element.click(); return true })()`)
    const proof = options.proof === undefined
      ? {
          bundled: await runBundledTrustedRaycastInstalledSmoke(launcher, userData, { waitFor, clickExactText }, { clickSelector }),
          canIUse: await runCanIUseInstalledSmoke(launcher, userData, { waitFor, clickExactText }, { clickSelector, firstUseOnly: true }),
        }
      : { proof: await options.proof(launcher) }
    const finalGeometry = await geometry()
    assert.deepEqual(finalGeometry, expectedGeometry)
    assert.deepEqual(browserErrors.proof.pageErrors, [])
    assert.deepEqual(browserErrors.proof.consoleErrors, [])
    ...
    return Object.freeze({
      ...
      browser,
      browserErrors,
      browserProof: { driver: 'Playwright over Loopback CDP', initialGeometry, finalGeometry, ...browserErrors.proof },
    ...
    try {
      await closeInstalledSession({ launched, browser, browserErrors, processEvidence }, executable, options.installRoot)
    } catch (cleanupError) {
...
async function closeInstalledSession(session, executable = undefined, installRoot = undefined) {
  const errors = []
  try { session.browserErrors?.beginShutdown() } catch (error) { errors.push(error) }
  ...
  // Closing the app first also disconnects the secondary Playwright transport.
  try { await session.browser?.close() } catch (error) { errors.push(error) }
  try { session.browserErrors?.finishShutdown() } catch (error) { errors.push(error) }
  let processEvidence
...
async function runInstalledFirstUseSmoke(artifact, chromium) {
  ...
  await mkdir(artifact.userData, { recursive: true })
  await writeFile(join(artifact.userData, 'skins.json'), JSON.stringify({ activeId: null, fallbackTheme: 'dark' }), { flag: 'wx' })
  const first = await installedFirstUseSession(identity.executable, artifact.userData, inventory, { installRoot: destination, chromium })
  let cleanup
  const firstProof = await withInstalledSession(first, async session => ({ canIUse: session.canIUse, bundled: session.bundled }), async session => { cleanup = await closeInstalledSession(session, identity.executable, destination); return cleanup })
  assert.ok(cleanup?.processEvidence !== undefined, 'installed first-use proof is missing process evidence')
  assert.equal(cleanup.processEvidence.residuePids.length, 0, 'installed first-use process tree was not fully cleaned before restart')
  const restarted = await installedFirstUseSession(identity.executable, artifact.userData, inventory, {
    installRoot: destination,
    chromium,
    proof: async launcher => await runTrustedRaycastInstalledRestartSmoke(launcher, artifact.userData, { waitFor }),
  })
  let restartCleanup
  const restartProof = await withInstalledSession(restarted, async session => session.proof, async session => { restartCleanup = await closeInstalledSession(session, identity.executable, destination); return restartCleanup })
  assert.ok(restartCleanup?.processEvidence !== undefined, 'installed restart proof is missing process evidence')
  assert.equal(restartCleanup.processEvidence.residuePids.length, 0, 'installed restart process tree was not fully cleaned')
  assert.notEqual(first.launch.pid, restarted.launch.pid, 'persisted disablement proof did not relaunch the packaged app')
  ...
  return Object.freeze({
    ...
    installed: {
      ...
      firstUse: firstProof.canIUse,
      bundled: firstProof.bundled,
      restart: restartProof,
      installRoot: destination,
      launch: first.launch,
      restartLaunch: restarted.launch,
      package: inventory,
      checkpoints: first.checkpoints,
      restartCheckpoints: restarted.checkpoints,
      browserProof: first.browserProof,
      restartBrowserProof: restarted.browserProof,
      cleanupEvidence: cleanup.processEvidence,
      restartCleanupEvidence: restartCleanup.processEvidence,
      observedPids: [...cleanup.processEvidence.observedPids, ...restartCleanup.processEvidence.observedPids],
      processTreesGone: cleanup.processEvidence.residuePids.length === 0 && restartCleanup.processEvidence.residuePids.length === 0,
...
async function main() {
  ...
  try {
    ...
    // Resolve the existing tool before packaging or starting any app.
    const chromium = process.argv.includes(LAUNCHER_INSTALLED_FIRST_USE_FLAG) ? await loadInstalledPlaywright() : undefined
    const artifact = await preparePackagedArtifact({ smokeRoot })
    evidence = process.argv.includes(LAUNCHER_INSTALLED_FIRST_USE_FLAG)
      ? await runInstalledFirstUseSmoke(artifact, chromium)
