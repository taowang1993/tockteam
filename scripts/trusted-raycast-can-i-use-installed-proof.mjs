...
export async function runCanIUseInstalledSmoke(launcher, userData, { waitFor, clickExactText }) {
  ...
  await open('Trusted Extensions', 'trust')
  await click('Can I Use · Trusted Raycast')
  ...
  if (!initial.installed) { await click('Install Reviewed Extension'); await click('Approve & Install') }
  await waitFor(trust, state => state.installed && state.digestApproved, 15000)
  if (!(await trust()).enabled) await click('Enable Extension')
