...
export async function runCanIUseInstalledSmoke(launcher, userData, { waitFor, clickExactText }, { firstUseOnly = false, clickSelector = selector => launcher.clickSelector(selector) } = {}) {
  ...
  assert.equal(initial.installed, false, 'cold first-use proof requires an uninstalled Can I Use')
  await open('Can I Use', 'setup:can-i-use')
  await wait(`([...document.querySelectorAll('button')].some(button => button.textContent === 'Approve and Open' && !button.disabled))`)
  await launcher.pressKey('Enter')
  await waitFor(trust, state => state.installed && state.digestApproved && state.enabled, 15000)
