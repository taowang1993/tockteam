...
The lower-volume correction has not been tested in another Windows capture. A new capture can explain only a newly observed interval; it cannot reconstruct the historical cancelled process. Another Windows invocation requires a new explicit decision, not an automatic retry.
