async page => {
  ...
  const sourceLocator = () => page.locator('.cm-content:visible').first()
  const liveLocator = () => page.locator('[contenteditable="true"]:visible').first()
  const readSource = async () => {
    const editor = sourceLocator()
    await editor.waitFor({ state: 'visible', timeout: 10000 })
    return normalize((await editor.locator('.cm-line').allTextContents()).join('\n'))
  }
  const readLive = async () => {
    const editor = liveLocator()
    await editor.waitFor({ state: 'visible', timeout: 10000 })
  ...
  const state = async (name, mode, text) => {
