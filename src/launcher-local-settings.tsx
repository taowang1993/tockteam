...
export function LauncherLocalSettings({ busy, extensionId, save, snapshot }: LocalSettingsProps): ReactNode {
  ...
  const isOpen = (id: string, defaultOpen = false): boolean | undefined => extensionId === undefined ? (defaultOpen ? true : undefined) : extensionId === id
  return <section className="min-w-0 space-y-3" data-testid="tocklauncher-local-settings">
    <div hidden={extensionId !== undefined}><h2 className="text-base font-semibold text-foreground">{launcherFixedText('Local Transformation Extensions')}</h2><p className="mt-1 text-xs text-muted-foreground">{launcherFixedText('These bounded controls configure the seven local search providers.')}</p></div>
    <details hidden={isHidden('Base64Conversion')} open={isOpen('Base64Conversion', true)}><summary className="cursor-pointer text-sm font-medium">{launcherFixedText('Base64 Conversion')}</summary><div className="pl-3">
    ...
    <details hidden={isHidden('Calculator')} open={isOpen('Calculator')}><summary className="cursor-pointer text-sm font-medium">{launcherFixedText('Calculator')}</summary><div className="pl-3">
