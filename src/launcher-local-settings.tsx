...
export function LauncherLocalSettings({ busy, save, snapshot }: LocalSettingsProps): ReactNode {
  ...
  return <section className="min-w-0 space-y-3" data-testid="tocklauncher-local-settings">
    <div><h2 className="text-base font-semibold text-foreground">{launcherFixedText('Local Transformation Extensions')}</h2><p className="mt-1 text-xs text-muted-foreground">{launcherFixedText('These bounded controls configure the seven local search providers.')}</p></div>
    <details open><summary className="cursor-pointer text-sm font-medium">{launcherFixedText('Base64 Conversion')}</summary><div className="pl-3">
    ...
    <details><summary className="cursor-pointer text-sm font-medium">{launcherFixedText('Calculator')}</summary><div className="pl-3">
