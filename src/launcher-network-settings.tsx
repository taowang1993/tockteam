...
export function LauncherNetworkSettings({ busy, save, snapshot }: NetworkSettingsProps): ReactNode {
  ...
  const updateCurrencies = (raw: string): void => {
    ...
    void save('extension[CurrencyConversion].currencies', next).then(saved => {
      if (!saved) setCurrencyError(fixed('Currency codes could not be saved.'))
    }, () => setCurrencyError(fixed('Currency codes could not be saved.')))
  }
  const updateTarget = (raw: string): void => {
    if (!isLauncherRendererSettingValue('extension[CurrencyConversion].defaultTargetCurrency', raw)) {
      setTargetError(fixed('Default target currency must be a lowercase currency code.'))
      return
    }
    setTargetError(undefined)
    void save('extension[CurrencyConversion].defaultTargetCurrency', raw).then(saved => {
      if (!saved) setTargetError(fixed('Default target currency could not be saved.'))
  ...
  return <section className="space-y-3" data-testid="tockteam-network-settings">
    ...
    <Field label="Custom search engines" description="JSON records are validated as a whole; only one query placeholder and HTTPS public-host templates are accepted."><div className="flex flex-col items-end gap-1"><Textarea aria-label={fixed('Custom web search engines JSON')} aria-describedby={customError === undefined ? undefined : 'tockteam-custom-search-error'} aria-invalid={customError !== undefined} className="min-h-28 max-w-md text-xs" maxLength={32_768} spellCheck={false} disabled={busy} value={customJson} onChange={event => setCustomJson(event.target.value)} onBlur={() => { try { const parsed: unknown = JSON.parse(customJson); if (!isLauncherRendererSettingValue('extension[CustomWebSearch].customSearchEngines', parsed)) throw new Error(); setCustomError(undefined); void save('extension[CustomWebSearch].customSearchEngines', parsed).then(saved => { if (!saved) setCustomError(fixed('Custom search engines could not be saved.')) }, () => setCustomError(fixed('Custom search engines could not be saved.'))) } catch { setCustomError(fixed('Custom search engines could not be saved because the JSON or URL policy is invalid.')) } }} /><FieldError id="tockteam-custom-search-error" className="max-w-80 text-right text-xs">{customError}</FieldError></div></Field>
    <Field label="DeepL source language" description="Auto lets DeepL detect the source language."><LauncherSyncedNativeSelect aria-label={fixed('DeepL source language')} size="sm" disabled={busy} defaultValue={source} onChange={event => { if (isLauncherRendererSettingValue('extension[DeeplTranslator].defaultSourceLanguage', event.target.value)) void save('extension[DeeplTranslator].defaultSourceLanguage', event.target.value) }}>{SOURCE_LANGUAGES.map(item => <NativeSelectOption key={item} value={item}>{item}</NativeSelectOption>)}</LauncherSyncedNativeSelect></Field>
    <Field label="DeepL target language"><LauncherSyncedNativeSelect aria-label={fixed('DeepL target language')} size="sm" disabled={busy} defaultValue={language} onChange={event => { if (isLauncherRendererSettingValue('extension[DeeplTranslator].defaultTargetLanguage', event.target.value)) void save('extension[DeeplTranslator].defaultTargetLanguage', event.target.value) }}>{TARGET_LANGUAGES.map(item => <NativeSelectOption key={item} value={item}>{item}</NativeSelectOption>)}</LauncherSyncedNativeSelect></Field>
    <Field label="Web Search provider"><LauncherSyncedNativeSelect aria-label={fixed('Web Search provider')} size="sm" disabled={busy} defaultValue={engine} onChange={event => { if (event.target.value === 'Google' || event.target.value === 'DuckDuckGo') void save('extension[WebSearch].searchEngine', event.target.value) }}><NativeSelectOption value="Google">Google</NativeSelectOption><NativeSelectOption value="DuckDuckGo">DuckDuckGo</NativeSelectOption></LauncherSyncedNativeSelect></Field>
    <Field label="Web Search locale"><LauncherSyncedNativeSelect aria-label={fixed('Web Search locale')} size="sm" disabled={busy} defaultValue={locale} onChange={event => { if (LOCALES.includes(event.target.value as typeof LOCALES[number])) void save('extension[WebSearch].locale', event.target.value) }}>{LOCALES.map(item => <NativeSelectOption key={item} value={item}>{item}</NativeSelectOption>)}</LauncherSyncedNativeSelect></Field>
    <Field label="Show instant Web Search result" description="When enabled, ordinary launcher terms create one main-owned URL action without fetching suggestions."><Switch aria-label={fixed('Show instant Web Search result')} disabled={busy} checked={instant} onCheckedChange={checked => { void save('extension[WebSearch].showInstantSearchResult', checked) }} /></Field>
    <Field label="DeepL API key" description="The write-only key field remains in the Security section and is encrypted in Electron main."><KeyRound aria-hidden="true" className="size-4 text-muted-foreground" /><span className="text-xs text-muted-foreground">{snapshot.missingSensitiveKeys.includes('extension[DeeplTranslator].apiKey') ? fixed('Not configured') : fixed('Stored securely')}</span></Field>
    <Label className="sr-only">{fixed('Network settings complete')}</Label>
  </section>
}
