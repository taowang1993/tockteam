...
export function LauncherNetworkSettings({ busy, save, snapshot }: NetworkSettingsProps): ReactNode {
  ...
  return <section className="space-y-3" data-testid="tockteam-network-settings">
    ...
    <Field label="Show instant Web Search result" description="When enabled, ordinary launcher terms create one main-owned URL action without fetching suggestions."><Switch aria-label={fixed('Show instant Web Search result')} disabled={busy} defaultChecked={instant} onCheckedChange={checked => { void save('extension[WebSearch].showInstantSearchResult', checked) }} /></Field>
