...
export function LauncherTerminalSettings({ busy, save, snapshot }: LauncherTerminalSettingsProps): ReactNode {
  ...
  const [prefix, setPrefix] = useLauncherDraft(value(snapshot, 'extension[TerminalLauncher].prefix', '>'))
  ...
  if (platform === 'Linux') {
    return <div data-testid="tocklauncher-terminal-settings"><Badge variant="outline">{launcherFixedText('Terminal Launcher is unavailable on Linux.')}</Badge></div>
  }

  const savePrefix = (): void => {
    const next = prefix.trim()
    if (!isLauncherTerminalPrefix(prefix) || next !== prefix) {
      setError(launcherFixedText('Use a nonempty prefix up to 32 characters without line breaks.'))
      return
    }
    ...
    <div className="space-y-2" data-testid="tocklauncher-terminal-settings">
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-border/60 py-3">
        <div>
          ...
          <div className="mt-1 text-xs text-muted-foreground">{launcherFixedText('Type the prefix followed by a command. Every command requires native approval.')}</div>
        </div>
        <Input aria-describedby={error ? 'tocklauncher-terminal-prefix-help tocklauncher-terminal-prefix-error' : 'tocklauncher-terminal-prefix-help'} aria-invalid={error !== undefined} aria-label={launcherFixedText('Terminal Launcher command prefix')} className="w-24" disabled={busy} maxLength={32} value={prefix} onChange={event => setPrefix(event.target.value)} onBlur={savePrefix} />
      </div>
      <p className="text-xs text-muted-foreground" id="tocklauncher-terminal-prefix-help">{launcherFixedText('The command runs in the main-owned home directory through the selected terminal.')}</p>
      {definitions.map(definition => (
        <div className="flex items-center justify-between border-b border-border/60 py-3 last:border-b-0" key={definition.id}>
          ...
          <Switch aria-label={`${launcherFixedText('Enable Terminal Launcher')} ${definition.name}`} checked={terminalIds.includes(definition.id)} disabled={busy} onCheckedChange={checked => toggle(definition.id, checked)} />
        </div>
      ))}
