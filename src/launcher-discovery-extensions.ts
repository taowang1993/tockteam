...
export function createLauncherDiscoveryExtensions(options: LauncherDiscoveryOptions): Readonly<{
  ...
  const scan = async (extensionId: LauncherDiscoveryExtensionId, parentSignal: AbortSignal, scanErrors: Set<LauncherDiscoveryExtensionId>): Promise<readonly LauncherDiscoveryEntry[]> => {
    ...
    try {
      ...
      const operation = options.scanners[extensionId](context(extensionId, controller.signal, scanErrors))
  ...
  const loadIndexedItems = async (signal: AbortSignal, preserveSignal?: AbortSignal): Promise<readonly LauncherInternalResultItem[]> => {
    ...
    const vscode = new Map<string, Readonly<{ command: string; entry: Extract<LauncherDiscoveryEntry, { kind: 'vscode' }>; executableIdentity: LauncherDiscoveryIdentity | undefined; identity: LauncherDiscoveryIdentity | undefined }>>()
