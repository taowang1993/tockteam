...
function initializeLauncher(): void {
  ...
  const workflow = createLauncherWorkflow({
    ...
    effects: {
      ...
      executeCommand: async request => {
        ...
        return await runBoundedWorkflowCommand({ command: request.command, platform, signal: request.signal, workingDirectory: request.workingDirectory }, { runtimeRoot: runtimePaths().runtimeRoot })
      },
  ...
  const trustedRaycastCatalogAvailability = () => Object.freeze({
    'google-translate': trustedRaycast?.available === true,
    'kaomoji-search': trustedRaycast?.availableFor('kaomoji-search') === true,
    'can-i-use': trustedRaycast?.availableFor('can-i-use') === true,
  })
  ...
  trustedRaycastFirstUse = createTrustedRaycastFirstUse({
    ...
    launch: async (webContentsId, extensionId, check) => {
      ...
      const item = trustedRaycastCatalog(trustedRaycastChannel.active, trustedRaycastTrust!.status(), trustedRaycastKaomojiTrust?.status(), trustedRaycastCanIUseTrust?.status(), trustedRaycastCatalogAvailability()).find(item => item.id === command.id)
