...
import {
  DESKTOP_COPY_PATH_CHANNEL_PATH,
...
export interface DesktopCopyPathChannelEnvironment {
  endpoint: string
  token: string
}

export interface DesktopCopyPathChannelOptions {
  isAvailable(): boolean
  onCopy(input: DesktopCopyPathInput, signal: AbortSignal): Promise<DesktopCopyPathResult>
}
...
async function bodyOf(request: IncomingMessage, signal: AbortSignal): Promise<RequestBody> {
  return await new Promise<RequestBody>((resolve, reject) => {
    ...
    const onData = (chunk: Buffer | string): void => {
      ...
      size += buffer.length
      if (size > MAX_DESKTOP_COPY_PATH_BODY_BYTES) {
        request.pause()
...
/** One authenticated, clipboard-only request channel for the current DSH child. */
export class DesktopCopyPathChannel {
  private readonly options: DesktopCopyPathChannelOptions
  private server: Server | undefined
  private environmentValue: DesktopCopyPathChannelEnvironment | undefined
  ...
  constructor(options: DesktopCopyPathChannelOptions) {
    this.options = options
  }

  get environment(): DesktopCopyPathChannelEnvironment | undefined {
    return this.environmentValue
  }

  async start(): Promise<DesktopCopyPathChannelEnvironment> {
    if (this.server !== undefined || this.starting) throw new Error('Desktop copy-path channel is already running')
    this.starting = true
    this.stopping = false
    ...
    try {
      ...
      if (this.stopping) {
        ...
        throw new Error('Desktop copy-path channel was stopped while starting')
      }
      const address = server.address()
      if (address === null || typeof address === 'string') {
        server.close()
        throw new Error('Desktop copy-path channel did not receive a TCP address')
      }
      this.server = server
      this.environmentValue = {
        endpoint: `http://127.0.0.1:${String(address.port)}${DESKTOP_COPY_PATH_CHANNEL_PATH}`,
        token,
      }
      return this.environmentValue
    } finally {
  ...
  private async handle(
    ...
    if (request.method !== 'POST' || request.url !== DESKTOP_COPY_PATH_CHANNEL_PATH) {
  ...
  private async handleAuthorized(
    ...
    const input = validateDesktopCopyPathInput(raw)
    if (input === undefined) {
      response.writeHead(400).end()
      return
    }
    ...
    let result: DesktopCopyPathResult
    try {
      result = await this.options.onCopy(input, signal)
    } catch {
      result = signal.aborted
        ? { operationId: input.operationId, status: 'cancelled' }
        : { operationId: input.operationId, status: 'unavailable' }
    }
    const validated = validateDesktopCopyPathResult(result)
    ...
    if (Buffer.byteLength(rendered) > MAX_DESKTOP_COPY_PATH_RESULT_BYTES) {
      jsonResponse(response, 200, { operationId: input.operationId, status: 'unavailable' })
      return
    }
