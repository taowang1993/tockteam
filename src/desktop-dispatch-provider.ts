...
function validEvent(value: unknown): value is DesktopDispatchEvent {
  ...
  if (request.vaultGeneration !== undefined && request.vaultId === undefined) return false
  if (request.vaultId !== undefined
    && (typeof request.vaultId !== 'string' || !/^vault:[0-9a-f]{64}$/u.test(request.vaultId)
      || request.vaultGeneration !== undefined && (typeof request.vaultGeneration !== 'number'
        || !Number.isSafeInteger(request.vaultGeneration) || request.vaultGeneration < 0))) return false
  return true
}
