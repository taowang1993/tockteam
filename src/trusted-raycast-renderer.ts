...
export function createTrustedRaycastView(document: Document, bridge: LauncherPreloadBridge, onClose: () => void, locale = 'en-US'): { dispose(): void; element: HTMLElement; focus(): void; refreshTheme(): void; update(message: TrustedRaycastViewMessage): void } {
  ...
  const collectionBar = document.createElement('nav'); collectionBar.className = 'flex flex-wrap items-center gap-2 border-b border-[var(--dsw-alias-border-l1,CanvasText)] px-4 py-2'; collectionBar.setAttribute('aria-label', zh ? '语言集分页' : 'Language Set Pages'); collectionBar.hidden = true
  let collectionButtons: HTMLButtonElement[] = []
  header.append(searchRow); content.append(hero, intro, back, status, collectionBar, panelActions, results, formArea, error)
  ...
  const syncIdentity = (): void => {
    ...
    close.setAttribute('aria-label', current?.root?.props.languageCollection === true ? (zh ? '返回' : 'Back') : (zh ? '返回结果' : 'Back to Results'))
    element.setAttribute('aria-label', value.title); title.textContent = value.title; titleIcon.setAttribute('src', value.image)
    searchLabel.textContent = current?.extensionId === 'google-translate' && current.root?.props.languageCollection === true ? (zh ? '搜索语言集' : 'Search Language Sets') : current?.extensionId === 'can-i-use' ? 'Search Web Features' : current?.extensionId === 'kaomoji-search' ? 'Search Kaomoji' : (zh ? '要翻译的文本' : 'Text to Translate')
    results.setAttribute('aria-label', current?.extensionId === 'google-translate' && current.root?.props.languageCollection === true ? (zh ? '语言集' : 'Language Sets') : current?.extensionId === 'can-i-use' ? 'Web Features' : current?.extensionId === 'kaomoji-search' ? 'Kaomoji Results' : (zh ? '翻译结果' : 'Translations'))
  ...
  input.addEventListener('input', () => {
    ...
    status.textContent = current?.extensionId === 'can-i-use' || current?.extensionId === 'kaomoji-search' || current?.root?.props.languageCollection === true ? 'Searching…' : (zh ? '正在翻译…' : 'Translating…')
  ...
  const render = (root: TrustedRaycastViewNode): void => {
    ...
    collectionButtons = []; collectionBar.replaceChildren()
    const languageCollection = current?.extensionId === 'google-translate' && root.props.languageCollection === true
    setHidden(collectionBar, !languageCollection)
    const collectionButton = (action: TrustedRaycastViewNode): HTMLButtonElement => {
      const button = document.createElement('button'); button.type = 'button'; button.className = buttonClass
      button.textContent = String(action.props.title ?? ''); button.setAttribute('data-collection-control', String(action.props.collectionControl))
      button.disabled = !action.props.actionEventId || action.props.unavailable === true
      button.addEventListener('click', () => invoke(action)); collectionButtons.push(button); return button
    }
    if (languageCollection) {
      const count = document.createElement('span'); count.className = 'mr-auto text-xs text-[var(--dsw-alias-label-secondary,CanvasText)]'; count.setAttribute('aria-live', 'polite'); count.setAttribute('aria-atomic', 'true')
      const total = root.props.collectionCount === root.props.collectionTotal ? String(root.props.collectionCount) : `${root.props.collectionCount} of ${root.props.collectionTotal}`
      count.textContent = root.props.collectionDetail === true ? (zh ? '全部语言' : 'All Languages') : (zh ? `${total} 个语言集` : `${total} Language Sets`)
      if (Number(root.props.collectionPages) > 1) count.textContent += zh ? ` · 第 ${root.props.collectionPage} / ${root.props.collectionPages} 页` : ` · Page ${root.props.collectionPage} of ${root.props.collectionPages}`
      collectionBar.append(count)
      for (const action of descendants(root, 'raycast-action')) if (['previous', 'next', 'back'].includes(String(action.props.collectionControl))) collectionBar.append(collectionButton(action))
    }
    ...
    setHidden(status, preferenceSetup || (languageCollection && status.textContent === ''))
    syncSourceSearch(root)
    renderDropdown(root)
    const depth = typeof root.props.navigationDepth === 'number' ? root.props.navigationDepth : 0
    setHidden(back, preferenceSetup || languageCollection || depth <= 0)
    ...
    const rootActions = descendants(root, 'raycast-action').filter(action => !itemActions.has(action) && (!languageCollection || !action.props.collectionControl))
    ...
    items.forEach((node, index) => {
      ...
      if (languageCollection) {
        const expand = ownActions.find(action => action.props.collectionControl === 'expand')
        if (expand) item.append(collectionButton(expand))
      }
      const owner = createActionOwner(languageCollection ? [...ownActions, ...rootActions] : ownActions, item)
    ...
    if (items.length === 0) {
      ...
      const emptyText = document.createElement('p'); emptyText.className = 'm-0 text-sm font-medium'; emptyText.textContent = emptyTitle || (zh ? '无结果' : 'No Results'); empty.append(emptyIcon, emptyText)
      if (typeof emptyProjection?.props.description === 'string') { const description = document.createElement('p'); description.className = 'm-0 max-w-md text-center text-xs text-[var(--dsw-alias-label-secondary,CanvasText)]'; description.textContent = emptyProjection.props.description; empty.append(description) }
  ...
  return {
    ...
    update(message) {
      ...
      if (message.type === 'outcome') {
        ...
        if (message.succeeded) { setHidden(error, true); actionFeedback = current?.root?.props.languageCollection === true ? '' : zh ? '操作已完成' : 'Action Completed'; if (toastText === '') status.textContent = actionFeedback; setHidden(status, current?.root?.props.languageCollection === true && status.textContent === '') }
      ...
      const wasCollectionDetail = current?.root?.props.languageCollection === true && current.root.props.collectionDetail === true
      const isCollectionDetail = message.root?.props.languageCollection === true && message.root.props.collectionDetail === true
      const returningToCollection = wasCollectionDetail && !isCollectionDetail && message.root?.props.languageCollection === true
      if (!wasCollectionDetail && isCollectionDetail) collectionSelected = selected
      if (returningToCollection) selected = collectionSelected
      const focusedCollectionControl = document.activeElement?.getAttribute?.('data-collection-control')
      const collectionChanged = current?.root?.props.languageCollection !== message.root?.props.languageCollection
      ...
      if (collectionChanged && message.root) { input.value = String(descendants(message.root, 'raycast-list')[0]?.props.searchText ?? ''); setHidden(error, true) }
      if (message.root) render(message.root)
      if (returningToCollection) rows[selected]?.item.focus()
      else if (focusedCollectionControl) {
        const restored = collectionButtons.find(button => button.getAttribute('data-collection-control') === focusedCollectionControl)
          ?? collectionButtons.find(button => button.getAttribute('data-collection-control') !== 'expand')
        if (restored) restored.focus(); else focus()
      } else if (restoreRow || depthChanged) rows[selected]?.item.focus()
      sendLatest()
