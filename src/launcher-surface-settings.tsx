...
export function LauncherSurfaceSettingsSection({ busy, platform, save, section, snapshot }: LauncherSurfaceSettingsProps): ReactNode {
  ...
  const sectionContent: Readonly<Record<LauncherSurfaceSection, ReactNode>> = {
    ...
    browser: <section aria-labelledby="tocklauncher-browser-heading"><h2 id="tocklauncher-browser-heading" className="flex items-center gap-2 text-base font-semibold text-foreground"><SlidersHorizontal aria-hidden="true" className="size-4" />Browser and Shortcuts</h2><p className="mt-1 text-xs text-muted-foreground">Browser grants and global shortcuts are main-owned. Selection and revocation use native operations.</p>{renderRows(sections.browser)}<Field label="Custom browser grant" description="Use the native Choose/Revoke controls in Storage and Privacy; executable paths and arguments are never editable here."><Badge variant="outline">Status-only</Badge></Field><Field label="Language" description="The DSH locale service owns launcher language selection."><Badge variant="outline">DSH-owned</Badge></Field><Field label="Global shortcut" description="Electron main owns registration and conflict handling."><Badge variant="outline">Main-owned</Badge></Field></section>,
