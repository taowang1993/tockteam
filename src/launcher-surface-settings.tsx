...
function Field({ label, description, children }: Readonly<{ label: string; description?: string; children: ReactNode }>): ReactNode {
  return <div className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-border/60 py-3 last:border-b-0"><div className="min-w-0 flex-1"><div className="text-sm font-medium text-foreground">{label}</div>{description ? <div className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">{description}</div> : null}</div><div className="flex min-w-0 max-w-full flex-wrap items-center gap-2">{children}</div></div>
}
...
function SettingValue({ keyName, value, busy, save }: Readonly<{ keyName: string; value: unknown; busy: boolean; save: LauncherSurfaceSettingsProps['save'] }>): ReactNode {
  ...
  if (keyName === 'window.hideWindowOn') {
    ...
    return <div className="flex flex-wrap gap-2">{(['blur', 'afterInvocation', 'escapePressed'] as const).map(reason => <label className="flex items-center gap-2 text-xs"><input aria-label={`${labelFor(keyName)} ${reason}`} checked={options.includes(reason)} disabled={busy} type="checkbox" onChange={event => { const next = event.target.checked ? [...options, reason] : options.filter(item => item !== reason); if (isLauncherRendererSettingValue(keyName, next)) void save(keyName, next) }} />{reason}</label>)}</div>
  }
...
export function LauncherSurfaceSettingsSection({ busy, platform, save, snapshot }: LauncherSurfaceSettingsProps): ReactNode {
