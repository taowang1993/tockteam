    ...
    <div className="rounded-md border border-border/60 p-3">
      ...
      <div className="mt-2 space-y-2">
        {folders.length === 0 ? <p className="py-2 text-xs text-muted-foreground">{launcherFixedText('No Simple File Search roots configured.')}</p> : folders.map(folder => <div key={folder.id} className="rounded-md border border-border/60 p-2">
          <div className="flex min-w-0 flex-wrap items-center gap-2"><LauncherSyncedInput aria-label={`${launcherFixedText('Path for')} ${folder.id}`} aria-describedby={`path-help-${folder.id}`} className="min-w-0 flex-1" maxLength={4096} disabled={busy} defaultValue={folder.path} aria-invalid={folder.path.length > 0 && !absolutePath(folder.path.trim())} onBlur={event => updateFolder(folder.id, { path: event.target.value })} /><Button type="button" size="icon" variant="ghost" aria-label={`${launcherFixedText('Remove')} ${folder.id}`} disabled={busy} onClick={() => removeFolder(folder.id)}><Trash2 aria-hidden="true" /></Button><span className="w-full text-xs text-muted-foreground" id={`path-help-${folder.id}`}>{launcherFixedText('Use an absolute home-contained path.')}</span></div>
