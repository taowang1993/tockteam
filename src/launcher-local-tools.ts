...
function setOutput(output: HTMLTextAreaElement, error: HTMLElement, value: string, status?: HTMLElement, locale?: LauncherLocale): void {
  if (value.length > MAX_LOCAL_TOOL_OUTPUT_LENGTH) {
    setError(output, error, launcherText(locale, 'outputTooLong', 'Output exceeds the 16,384 character limit.'), status, locale)
  } else {
    output.value = value
    error.textContent = ''
    error.hidden = true
...
export function createLauncherLocalTool(options: Readonly<{
  ...
  generate.addEventListener('click', update); version.addEventListener('change', update); quantity.addEventListener('change', update); update(); content.append(labeled(document, text('uuidVersion', 'UUID version'), version), labeled(document, text('uuids', 'Number of UUIDs'), quantity), generate, labeled(document, text('generatedUuids', 'Generated UUIDs'), output)); queueMicrotask(() => generate.focus()); return tool
}
