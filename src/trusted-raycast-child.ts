...
const serialize = (node: Node | string, feature?: string): unknown => {
  ...
  const props = Object.fromEntries(Object.entries(node.props).filter(([key, value]) => key !== 'children' && value !== undefined && typeof value !== 'function' && (typeof value !== 'object' || value === null)))
...
let querySequence = 0
let ready = false
const translateCollection = extensionId === 'google-translate' ? createTranslateCollectionProjection(() => emit()) : undefined
...
const emit = () => {
  ...
  const projected = projectTrustedRaycastRoot(rootNode, extensionId)
  let root = serialize(translateCollection?.project(projected) ?? projected)
  if (translateCollection && navigationDepth() === 0 && !isTrustedRaycastViewMessage({ type: 'ready', extensionId, sessionId, generation, revision: revision + 1, root })) {
    // A large successful translation must not take away the user's recovery controls.
    handles.clear(); fieldHandles.clear()
    root = serialize(translateCollection.overflow(projected))
  }
...
const emitToast = (toast: { title: string; message: string; style: string }): void => {
  // Source toasts include full language lists; keep notifications compact, not cached values.
  if (extensionId === 'google-translate' && toast.message.length > 400) toast = { ...toast, message: `${toast.message.slice(0, 397)}…` }
...
process.stdin.on('data', chunk => {
  ...
  while ((end = pending.indexOf('\n')) >= 0) {
    ...
    if (message.kind === 'searchChanged') {
      querySequence++
      if (translateCollection?.search(message.value)) continue
    ...
    if (message.kind === 'navigation') {
      ...
      if (translateCollection?.pop()) continue
      showingPreferenceSetup = false
      popView()
      continue
    }
    if (message.kind === 'fieldChanged') {
      ...
      if (typeof callback === 'function') {
        try { callback(message.value!) }
        catch (error) {
          if (extensionId !== 'google-translate') throw error
          emitToast({ style: 'failure', title: 'Translate state update failed', message: error instanceof Error ? error.message.slice(0, 256) : 'Translate state update was rejected' })
          emit()
        }
      }
      continue
